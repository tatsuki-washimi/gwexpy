...
def test_register_timeseries_format_auto_adapt(tmp_path):
    ...
    register_timeseries_format(
        "mock_fmt", reader_dict=mock_reader_dict, extension="mockext"
    )
    ...
    assert io_registry.identify_format(
        "read", TimeSeriesDict, "file.mockext", None, (), {}
    ) == ["mock_fmt"]
    assert io_registry.identify_format(
        "read", TimeSeries, "file.mockext", None, (), {}
    ) == ["mock_fmt"]
    assert io_registry.identify_format(
        "read", TimeSeriesMatrix, "file.mockext", None, (), {}
    ) == ["mock_fmt"]
