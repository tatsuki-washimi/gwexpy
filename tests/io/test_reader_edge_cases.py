...
class TestCsvEdgeCases:
    def test_multi_sample_roundtrip(self, tmp_path):
        ts = TimeSeries(
            np.arange(10, dtype=float), dt=0.5, t0=100, unit="strain", name="multi"
        )
...
class TestHdf5EdgeCases:
    ...
    def test_dict_roundtrip_channel_names(self, tmp_path):
        d = TimeSeriesDict(
            {
                "X1:CHANNEL_A": TimeSeries(
                    np.ones(10), dt=1, t0=0, name="X1:CHANNEL_A"
                ),
                "X1:CHANNEL_B": TimeSeries(
                    np.zeros(10), dt=1, t0=0, name="X1:CHANNEL_B"
                ),
            }
        )
