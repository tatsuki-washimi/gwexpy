...
def generate_ndscope_hdf5(filename):
    ...
    with h5py.File(filename, "w") as f:
        for ch_name in channels:
            ...
            grp.attrs["unit"] = unit
            grp.create_dataset(
                "raw", data=np.random.randn(n_samples).astype(np.float32)
            )
...
def generate_diaggui_xml(filename):
    ...
    ET.SubElement(
        arr_asd, "Stream", Encoding="LittleEndian,base64"
    ).text = _to_base64_stream(asd_A.astype(np.float32))

    with open(filename, "wb") as f_out:
        f_out.write(b'<?xml version="1.0" encoding="utf-8" ?>\n')
        f_out.write(
            b'<!DOCTYPE LIGO_LW SYSTEM "http://ldas-sw.ligo.caltech.edu/doc/util/ligo_lw.dtd">\n'
        )
...
def generate_davis_db(filename):
    ...
    cursor = conn.cursor()
    cursor.execute(
        "CREATE TABLE archive (dateTime INTEGER PRIMARY KEY, barometer REAL, outTemp REAL)"
    )
...
def generate_seismic_mseed(filename):
    ...
    tr = Trace(
        ...
        header={"starttime": UTCDateTime("2024-01-01"), "sampling_rate": 10.0},
    )
...
def generate_all():
    ...
    if allow_zarr:
        ...
        print(
            "  [SKIP] Zarr: set GWEXPY_ALLOW_ZARR=1 to enable Zarr fixture generation"
        )
