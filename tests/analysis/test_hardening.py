...
def test_frequency_alignment_interpolate(sample_data):
    ...
    assert (
        th_interp.threshold(psd_tgt_inj, psd_tgt_bkg, bkg_table=st_small).shape
        == psd_tgt_inj.value.shape
    )
...
def test_response_hardening_parallel(sample_data):
    ...
    res = analysis.compute(wit, tgt, segments=segments, fftlength=1.0, n_jobs=2)
