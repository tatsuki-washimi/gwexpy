...
def test_collection_type_preservation():
    ...
    msg = []
    if not is_gwex_dict:
        msg.append(f"Dict.crop returned {type(tsd_crop)}")
    if not is_gwex_dict_res:
        msg.append(f"Dict.resample returned {type(tsd_res)}")
    if not is_gwex_list:
        msg.append(f"List.crop returned {type(tsl_crop)}")
...
def test_monkey_patching():
    ...
    return log_result(
        "Monkey Patch (csd_matrix on gwpy.TimeSeriesDict)",
        has_csd,
        "Patch applied" if has_csd else "Patch missing",
    )
...
if __name__ == "__main__":
    ...
    if not test_collection_type_preservation():
        failures.append("Collection Type Preservation")
    if not test_time_array_handling():
        failures.append("Time Array Handling")
    if not test_monkey_patching():
        failures.append("Monkey Patching")
