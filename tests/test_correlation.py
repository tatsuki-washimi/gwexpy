...
def test_mix_resample():
    ...
    ts2 = TimeSeries(np.interp(np.arange(0, 200, 0.5), np.arange(0, 200), ts1.value), dt=0.5)
...
def test_correlation_vector():
    ...
    assert df.iloc[-1]['channel'] == 'ch1' # ch2 (neg 1) has high abs corr? sort_values(key=abs) was used
