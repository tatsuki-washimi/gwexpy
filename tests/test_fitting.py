...
def test_fitting_missing_p0():
    ...
    y = damped_oscillation(t, A=1, tau=0.2, f=50, phi=0.5) + np.random.normal(
        0, 0.05, len(t)
    )
    ts = TimeSeries(y, times=t)
...
def test_power_law_fitting():
    ...
    y = power_law(frequencies, A=10, alpha=-1.5) * (
        1 + 0.05 * np.random.normal(size=len(frequencies))
    )
...
def test_fitting_sigma():
    ...
    # Crop to middle part
    result_crop = ts.fit(
        "gaussian", x_range=(4, 6), sigma=sigma_full, p0={"A": 10, "mu": 5, "sigma": 1}
    )
