...
def test_export_data_dialog_flow(window):
    ...
    with patch.object(
        QtWidgets.QFileDialog,
        "getSaveFileName",
        return_value=("test.h5", "HDF5 (*.h5 *.hdf5)"),
    ):
    ...
    with patch.object(
        QtWidgets.QFileDialog,
        "getSaveFileName",
        return_value=("test_export.h5", "HDF5 (*.h5 *.hdf5)"),
    ):
        with patch.object(QtWidgets.QMessageBox, "information") as mock_info:
            window.export_data()
            # Check if write was called
            mock_data.write.assert_called_with(
                "test_export.h5", format="hdf5", overwrite=True
            )
