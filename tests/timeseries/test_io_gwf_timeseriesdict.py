...
@pytest.mark.skipif(not FIXTURE_DATA.exists(), reason="test.gwf fixture not found")
@pytest.mark.skipif(not has_gwf_backend(), reason="gwf backend not available")
def test_read_gwf_timeseriesdict_multiple_files_single_file_interval(tmp_path):
    files, _ = write_gwf_parts(tmp_path)

    tsd = TimeSeriesDict.read(files, CHANNEL, start=1000.25, end=1000.75)

    assert len(tsd[CHANNEL]) == 8
    assert float(tsd[CHANNEL].t0.value) == pytest.approx(1000.25)
    np.testing.assert_allclose(tsd[CHANNEL].value, np.arange(4, 12, dtype=float))
