...
def test_lock_in_constant_freq():
    ...
    mag, ph = data.lock_in(
        f0=f0, stride=stride, output="amp_phase", deg=False, singlesided=True
    )
...
def test_singlesided_scaling():
    ...
    out_double = data.lock_in(
        phase=phases, stride=stride, output="complex", singlesided=False
    )
    out_single = data.lock_in(
        phase=phases, stride=stride, output="complex", singlesided=True
    )
...
def test_fdot_phase_model():
    ...
    out = data.lock_in(
        f0=f0, fdot=fdot, stride=stride, output="complex", singlesided=True
    )
...
def test_phase_epoch():
    ...
    out2 = data.lock_in(
        f0=f0, phase_epoch=1000.0, stride=stride, output="complex", singlesided=True
    )
    ...
    data_test = TimeSeries(
        amp * np.cos(2 * np.pi * f_test * t_rel), unit="", times=t_abs
    )

    # Aligned: epoch=1000
    out_aligned = data_test.lock_in(
        f0=f_test, phase_epoch=1000.0, stride=stride, output="complex", singlesided=True
    )
    ...
    out_shifted = data_test.lock_in(
        f0=f_test, phase_epoch=999.0, stride=stride, output="complex", singlesided=True
    )
