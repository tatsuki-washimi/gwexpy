...
class TestBuildPhaseSeries:
    ...
    def test_quadratic_chirp_fddot(self):
        ...
        t_rel = dt * np.arange(n_samples)
        expected = phase0 + 2 * np.pi * (
            f0 * t_rel + 0.5 * fdot * t_rel**2 + (1 / 6) * fddot * t_rel**3
        )
...
class TestMixDown:
    ...
    def test_singlesided_doubles_amplitude(self):
        ...
        np.testing.assert_allclose(
            result_double.value, 2.0 * result_single.value, rtol=1e-10
        )
    ...
    def test_mix_down_preserves_metadata(self):
        ...
        ts = TimeSeries(
            np.ones(100), dt=0.01, unit="m", channel="TEST", name="test_signal"
        )
...
class TestHeterodyneLockIn:
    ...
    def test_lock_in_amplitude_recovery(self):
        ...
        mag, phase = ts.lock_in(
            f0=f0, stride=stride, singlesided=True, output="amp_phase", deg=False
        )
    ...
    def test_lock_in_singlesided_scaling(self):
        ...
        out_single = ts.lock_in(
            f0=f0, stride=stride, singlesided=False, output="complex"
        )
        out_double = ts.lock_in(
            f0=f0, stride=stride, singlesided=True, output="complex"
        )

        # singlesided=True should give 2x amplitude
        ratio = np.median(np.abs(out_double.value)) / np.median(
            np.abs(out_single.value)
        )
