...
def test_torch_window_dataset_shapes():
    ...
    ds = torch_dataset.TimeSeriesWindowDataset(
        ts, window=4, stride=2, horizon=1, labels=labels
    )
    assert len(ds) == 3
...
def test_torch_dataset_importerror(monkeypatch):
    ...
    def fake_require(name):
        raise ImportError(
            "The 'torch' package is required for this feature but is not installed."
        )
