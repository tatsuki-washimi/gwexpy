"""Tests for Field4D slicing behavior - always maintains 4D."""
...
class TestField4DSlicingMaintains4D:
    """Test that Field4D __getitem__ always returns 4D."""

    @pytest.fixture
    def field(self):
        """Create a test Field4D."""
    ...
    def test_int_index_axis0_maintains_4d(self, field):
        """Test field[0, :, :, :] returns Field4D with axis0 length=1."""
        sliced = field[0, :, :, :]

        assert isinstance(sliced, Field4D)
        assert sliced.ndim == 4
        assert sliced.shape == (1, 8, 6, 4)
        assert sliced.shape[0] == 1

    def test_int_index_axis3_maintains_4d(self, field):
        """Test field[:, :, :, 0] returns Field4D with axis3 length=1."""
        sliced = field[:, :, :, 0]
...
class TestField4DSlicingDomain:
    ...
    def test_axis0_domain_preserved_frequency(self):
        ...
        field = Field4D(np.zeros((10, 4, 4, 4)), axis0_domain="frequency")
        sliced = field[0, :, :, :]
...
class TestField4DSlicingAxisNames:
...
class TestField4DIselMaintains4D:
    """Test isel with Field4D maintains 4D."""
