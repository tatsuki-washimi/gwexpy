...
def test_frozen_b0_evidence_validates_against_its_immutable_fixed_tree() -> None:
    ...
    evidence_path = (
        ...
        / "evidence"
        / "v0.2.0-b0"
        / "series_matrix_b0.json"
    )
    evidence = json.loads(evidence_path.read_text())

    benchmark._validate_evidence(
        evidence, expected_phase="B0", target_root=repository_root
    )
...
@pytest.mark.parametrize(
    ...
    [
        (
            lambda record: record.update(path="gwexpy/_bootstrap.py"),
            "unique|does not match",
        ),
        (lambda record: record.update(path="gwexpy/missing.py"), "does not match"),
        (
            lambda record: record.update(name="gwexpy._bootstrap"),
            "unique|does not match",
        ),
    ],
)
def test_frozen_b0_rejects_forged_or_nonexistent_module_mappings(
    ...
    evidence = json.loads(evidence_path.read_text())
    mutation(evidence["imported_gwexpy_source_modules"][0])
...
def test_b0_rejects_consistent_valid_module_substitution_against_fixed_sha_closure() -> (
    None
):
    repository_root = Path(__file__).parents[2]
    evidence_path = (
    ...
    evidence = json.loads(evidence_path.read_text())
    fixed_sha = cast(str, evidence["fixed_sha"])
...
"""
    with benchmark.isolated_worktree(repository_root, fixed_sha) as fixed_root:
        ...
        fixed_closure = set(map(tuple, json.loads(completed.stdout)))
        replacement_source_exists = (
            fixed_root / "gwexpy" / "analysis" / "__init__.py"
        ).is_file()

    assert ("gwexpy._warnings", "gwexpy/_warnings.py") in fixed_closure
    replacement = ("gwexpy.analysis", "gwexpy/analysis/__init__.py")
    assert replacement not in fixed_closure
    assert replacement_source_exists

    def substitute(records: list[dict[str, object]]) -> None:
        for record in records:
            if record["name"] == "gwexpy._warnings":
                record.update(name=replacement[0], path=replacement[1])

    substitute(
        cast(list[dict[str, object]], evidence["imported_gwexpy_source_modules"])
    )
    ...
    for raw_result in results.values():
        ...
        for attempt in attempts:
            ...
            for sample in samples:
                substitute(cast(list[dict[str, object]], sample["source_modules"]))

    with pytest.raises(ValueError, match="fixed.*closure|authoritative.*module"):
        benchmark._validate_evidence(
            evidence, expected_phase="B0", target_root=repository_root
        )
...
def test_b1_packet_preserves_phase_a_runtime_and_separates_failure_groups() -> None:
    repository_root = Path(__file__).parents[2]
    evidence_root = repository_root / "docs" / "plans" / "evidence" / "v0.2.0-b1"
    decision = (evidence_root / "series_matrix_b1_decision.md").read_text()
    ledger = (evidence_root / "completion-ledger.md").read_text()

    assert (
        "No #637 composition candidate runtime was copied into integration" in decision
    )
    assert "Phase A" in decision
    assert "SpectrogramMatrix" in decision
    assert "raw-ndarray add/sub" in decision
    assert "atomic-`TypeError`" in decision
    assert "adopted: false" in decision
    assert "B0 benchmark" in decision
    assert "fallback semantics" in decision
    assert "No #637 composition candidate runtime was copied into integration" in ledger
    assert "Phase A" in ledger
    assert "raw-ndarray add/sub" in ledger
    assert "atomic-`TypeError`" in ledger
    assert "B0 benchmark/fallback semantics" in ledger
    assert "No integration runtime changes were made" not in ledger
    assert "integration runtime remains the frozen B0 runtime" not in decision

    def section(document: str, heading: str) -> str:
        start = document.index(f"### {heading}")
        remainder = document[start + len(heading) + 4 :]
        end = remainder.find("\n### ")
        return remainder if end == -1 else remainder[:end]

    typed_view = section(decision, "Intentional typed-view incompatibilities")
    assert "two intentional" in typed_view
    assert "excluded from the filtered Spectrogram compatibility run" in typed_view
    assert (
    ...
    assert "Result: `2 failed`" in typed_view

    reflected_quantity = section(
        decision, "Reflected Quantity-left addition limitations"
    )
    assert "two genuine" in reflected_quantity
    assert (
    ...
    legacy = section(decision, "Legacy operator failures")
    assert "339 passed, 33 failed" in legacy
    assert "30 are" in legacy
    assert "3 are" in legacy
    assert "test_direct_ufunc_application_raises" in legacy
    assert "test_modulo_and_floor_divide_do_not_ignore_units" in legacy
