...
| `gh_pr_comment`    | `gh pr comment` で検証済みのローカル所見を GitHub PR に投稿する                   |
...
| `audit_api_elements`    | リポジトリ内のクラス・関数を抽出し、公開 API / 内部実装として分類する       |
...
| `verify_hardening` | Non-ASCII 検知、Sphinx 厳格ビルド、Doctest をまとめて検証する |
...
| スキル                 | 説明                                                                         |
| ---------------------- | ---------------------------------------------------------------------------- |
| `setup_plan`           | ユーザーの要求から具体的な作業計画を作成し、必要に応じて multi-agent gate を参照する |
...
| `catalog_legacy`       | 既存コードや旧資産を棚卸しし、移行・保守対象を分類する                      |
