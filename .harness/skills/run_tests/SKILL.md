...
`gwexpy` リポジトリでは、`pytest` を直接叩かずに先に `gwexpy_conda_jobs` を使う。
標準は `bash .agent/skills/gwexpy_conda_jobs/scripts/run_job.sh start pytest ...`。

## Quick Start

```bash
# 全テスト実行
bash .agent/skills/gwexpy_conda_jobs/scripts/run_job.sh start pytest

# 高速モード（GUIを除く）
bash .agent/skills/gwexpy_conda_jobs/scripts/run_job.sh start pytest --ignore=tests/gui/

# 特定のモジュール
bash .agent/skills/gwexpy_conda_jobs/scripts/run_job.sh start pytest tests/fields/
```
