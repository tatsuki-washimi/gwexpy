...
#   - all tags pushed:     git push --tags   (tags v0.1.0..v0.1.6 must exist on origin)
#
# Behaviour:
...
#   - Only v0.1.6 is marked as the latest release.
#
# Usage:
...
for v in "${VERSIONS[@]}"; do
  ...
  if [[ "$tag" == "$LATEST_TAG" ]]; then latest="true"; else latest="false"; fi

  if gh release view "$tag" --repo "$REPO" >/dev/null 2>&1; then
    echo ">> editing existing release $tag (latest=$latest)"
    run gh release edit "$tag" --repo "$REPO" \
      --title "$tag" --notes-file "$notes" --latest="$latest"
  else
    echo ">> creating new release $tag (latest=$latest)"
