...
def test_timeseries_attr_preservation():
    ...
    print(f"Original: _gwex_custom_attr = {getattr(ts, '_gwex_custom_attr', 'MISSING')}")

    # 2. Slice operation
    ts_slice = ts[1:4]
    
    print(f"Slice type: {type(ts_slice)}")
    print(f"Slice: _gwex_custom_attr = {getattr(ts_slice, '_gwex_custom_attr', 'MISSING')}")
    ...
    ts_math = ts * 2
    print(f"Math: _gwex_custom_attr = {getattr(ts_math, '_gwex_custom_attr', 'MISSING')}")
