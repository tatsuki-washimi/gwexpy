...
TUTORIAL_DIRS = ["docs/web/ja/user_guide/tutorials", "docs/web/en/user_guide/tutorials"]
...
def patch_notebook(file_path):
    ...
    if first_cell and first_cell.get("cell_type") == "code":
        ...
        if "%pip install" in source or "pip install" in source:
            ...
            first_cell["source"] = [
                line + "\n" for line in INSTALL_CELL_CONTENT[:-1]
            ] + [INSTALL_CELL_CONTENT[-1]]
...
def main():
    ...
    print(
        f"Done. Inserted {inserted_count} new cells, modified {modified_count} existing cells."
    )
