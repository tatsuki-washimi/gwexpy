...
EXTERNAL_CALL_PATTERN = re.compile(r"(np\.|numpy\.|scipy\.|signal\.|linalg\.|fft)")
...
class DiffResult:
    ...
    lineno_deviation: list[tuple[LedgerEntry, FunctionDef, int]] = field(
...
def compare_ledger_to_code(
    ledger: list[LedgerEntry], code_funcs: list[FunctionDef], line_tolerance: int = 30
) -> DiffResult:
...
def check_a2_classification(
    ledger: list[LedgerEntry], code_funcs: list[FunctionDef], repo_root: Path
) -> list[ClassificationSuspect]:
    ...
    for entry in ledger:
        ...
        source = get_function_source(
            str(file_path), func.source_lines[0], func.source_lines[1]
        )
        ...
        if entry.a2_flag == "No":
            ...
            if (
                numpy_count >= 3
                or scipy_count >= 1
                or (numpy_count >= 1 and len(found_funcs) >= 2)
            ):
                suspects.append(
                    ClassificationSuspect(
                        qualified_name=entry.qualified_name,
        ...
        if entry.a2_flag == "Yes":
            ...
            if len(body_lines) <= 3:
                ...
                if any(
                    line.startswith("return ") and "(" in line for line in body_lines
                ):
                    method_calls = re.findall(
                        r"\.([a-z_][a-z0-9_]*)\s*\(", source_lower
                    )
    ...
    with open(output_path, "w", encoding="utf-8") as f:
        ...
        if diff.in_ledger_not_code:
            ...
            for entry in diff.in_ledger_not_code:
                reason = "削除/改名?"
                f.write(
                    f"| `{entry.qualified_name}` | {entry.file} | {entry.lineno} | {reason} |\n"
                )
        else:
            f.write("*なし*\n")
        f.write("\n")
        ...
        if diff.in_code_not_ledger:
            ...
            for func in diff.in_code_not_ledger:
                reason = "新規追加?"
                f.write(
                    f"| `{func.qualified_name}` | {func.file} | {func.lineno} | {reason} |\n"
                )
        else:
            f.write("*なし*\n")
        f.write("\n")
        ...
        if diff.file_mismatch:
            ...
            for entry, func in diff.file_mismatch:
                reason = "移動/誤記?"
                f.write(
                    f"| `{entry.qualified_name}` | {entry.file} | {func.file} | {reason} |\n"
                )
        else:
            f.write("*なし*\n")
        f.write("\n")
        ...
        if diff.lineno_deviation:
            ...
            for entry, func, dev in diff.lineno_deviation:
                f.write(
                    f"| `{entry.qualified_name}` | {entry.lineno} | {func.lineno} | {dev} |\n"
                )
        else:
            f.write("*なし*\n")
        f.write("\n")
        ...
        f.write("## Summary\n\n")
        f.write(
            f"- 台帳にあるが実コードに存在しない: {len(diff.in_ledger_not_code)} 件\n"
        )
...
def generate_suspect_report(
    suspects: list[ClassificationSuspect], output_path: Path, commit_hash: str
) -> None:
    ...
    with open(output_path, "w", encoding="utf-8") as f:
        ...
        f.write(f"**対象 Commit Hash:** `{commit_hash}`\n\n")
        f.write(
            "**注意:** このレポートは機械的ヒューリスティックによる疑義リストであり、"
        )
        ...
        for suspect_type, items in by_type.items():
            ...
            for item in items:
                f.write(
                    f"| `{item.qualified_name}` | {item.file}:{item.lineno} | {item.reason} |\n"
                )
            f.write("\n")
...
def main():
    ...
    parser.add_argument("--csv", required=True, help="Path to the ledger CSV file")
    parser.add_argument(
        "--package",
        required=True,
    ...
    if needs_update:
        ...
        with open(diff_summary_path, "w") as f:
            f.write("Ledger update summary\n")
            f.write("=====================\n\n")
            f.write(
                f"Removed entries (marked status=removed): {len(diff.in_ledger_not_code)}\n"
            )
