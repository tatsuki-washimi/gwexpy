...
def test_type_preservation():
    ...
    # Crop
    ts_crop = ts.crop(1, 2)
    assert isinstance(ts_crop, GwexTimeSeries), (
        f"Expected GwexTimeSeries, got {type(ts_crop)}"
    )

    # Append
    ts2 = GwexTimeSeries(np.random.randn(1000), t0=ts.t0 + ts.duration, sample_rate=100)
    ts_app = ts.append(ts2, inplace=False)
    assert isinstance(ts_app, GwexTimeSeries), (
        f"Expected GwexTimeSeries, got {type(ts_app)}"
    )
