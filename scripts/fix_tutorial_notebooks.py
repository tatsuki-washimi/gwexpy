...
def _clean_text_output(text: str, *, lang: str | None = None) -> str:
    ...
    text = "\n".join(filtered_lines)
    text = text.replace(
        "Wrote /tmp/kagra_sus_itmx.xml", "Wrote synthetic DTT XML to a temporary file"
    )
    text = text.replace(
        "書き込み完了: /tmp/kagra_sus_itmx.xml",
    ...
    if "パイプラインアーカイブ:" in text and "_pipeline.h5" in text:
        text = text.replace(
            "パイプラインアーカイブ:", "パイプラインアーカイブ: 一時ファイル"
        )
        if "  (" in text:
    ...
    if lang == "ja" and "Saved to temporary MTH5 file:" in text:
        text = text.replace(
            "Saved to temporary MTH5 file:", "一時 MTH5 ファイルへ保存:"
        )
    return text + ("\n" if text else "")
...
def _fix_case_bruco_ica(lang: str) -> None:
    path = (
        ...
        / "user_guide"
        / "tutorials"
        / "case_bruco_ica_denoising.ipynb"
    )
    nb = _load(path)
    cell = nb["cells"][13]
    src = _cell_source(cell)
...
def _fix_advanced_hht(lang: str) -> None:
    path = (
        ROOT / "docs" / "web" / lang / "user_guide" / "tutorials" / "advanced_hht.ipynb"
    )
    nb = _load(path)
    for cell in nb["cells"]:
        if cell.get("cell_type") != "code":
    ...
    for name in files:
        ...
        for cell in nb["cells"]:
            ...
            src = src.replace(
                ...
                'print(f"Written synthetic_pem.gbd ({path.stat().st_size/1024:.1f} KB)")',
            )
            src = src.replace(
                'print(f"作成完了: {path}  ({path.stat().st_size/1024:.1f} KB)")',
                'print(f"synthetic_pem.gbd を作成しました ({path.stat().st_size/1024:.1f} KB)")',
            )
            _set_source(cell, src)
        _sanitize_outputs(nb, lang=lang)
        _save(path, nb)
...
def _fix_advanced_coupling() -> None:
    ...
    if "周波数帯域制限" not in _cell_source(ja["cells"][13]):
        ...
        _set_source(
            ja["cells"][16],
            _cell_source(ja["cells"][16]).replace(
                "## 5. 上限値（Upper Limit）", "## 6. 上限値（Upper Limit）"
            ),
        )
...
def _fix_case_seismic_obspy() -> None:
    ...
    nb = _load(ja_path)
    joined = "\n".join(
        _cell_source(c) for c in nb["cells"] if c.get("cell_type") == "markdown"
    )
    if "マルチチャンネル地震解析" not in joined:
        ...
        _set_source(
            nb["cells"][14],
            _cell_source(nb["cells"][14]).replace(
...
def _annotate_advanced_hht() -> None:
    path = (
        ROOT / "docs" / "web" / "ja" / "user_guide" / "tutorials" / "advanced_hht.ipynb"
    )
    nb = _load(path)
    note = (
