...
RISKY_FUNCS = {
    # Linear Algebra
    "inv", "det", "solve", "pinv", "lstsq",
    # Signal Processing
    "welch", "csd", "coherence", "spectrogram", "lfilter", "filtfilt",
    # Fitting/Optimization
    "curve_fit", "leastsq", "minimize",
    # Polynomials
    "polyfit", "roots",
...
DANGEROUS_CATCHES = {
    "Exception", "BaseException", "ArithmeticError", "ZeroDivisionError", "FloatingPointError", "ValueError"
}
...
class DeepRiskVisitor(ast.NodeVisitor):
    ...
    def visit_BinOp(self, node):
        ...
        if isinstance(node.op, ast.Mod) and isinstance(node.left, ast.Constant) and isinstance(node.left.value, str):
            ...
            if match:
                ...
                if precision < 10:
                    self._log(node, "Formatting Risk", f"Low precision format: %.{precision}f")
                 ...
                 if kw.arg == "dtype":
                     if isinstance(kw.value, ast.Constant) and isinstance(kw.value.value, str):
                         if "float32" in kw.value.value or "float16" in kw.value.value:
                              self._log(node, "Precision Risk", f"Creation with dtype={kw.value.value}")
                     if isinstance(kw.value, ast.Attribute) and kw.value.attr in ("float32", "float16"):
                          self._log(node, "Precision Risk", f"Creation with dtype={kw.value.attr}")
    ...
    def _check_format_spec(self, node, format_spec):
        ...
        if match:
            precision = int(match.group(1))
            if precision < 10: # 10 digits is reasonable safety, but typical 2 or 4 is bad for GW
                self._log(node, "Formatting Risk", f"Low precision format: .{precision}f")
