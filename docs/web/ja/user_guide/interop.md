...
| 方針 | 依存関係 | インストール案内 |
| --- | --- | --- |
| GWexpy extras で宣言済み | `zarr`, `netCDF4`, `xarray`, `obspy`, `mth5`, `lalsuite`, `gwinc`, `control`, `pydub` | [インストールガイド](installation.md) の PyPI extra 形式を使います。例: `pip install "gwexpy[control]"` |
