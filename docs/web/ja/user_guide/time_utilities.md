...
| 目的 | 推奨関数 | 入力の型 | 出力の型 (Scalar / Vector) | 共通引数 |
| :--- | :--- | :--- | :--- | :--- |
...
| **相互変換 (便利)** | `tconvert` | 上記すべて + `"now"` | 文脈に応じた型 (Scalar 優先) | — |
| **高精度オブジェ** | `LIGOTimeGPS` | `seconds`, `nanoseconds` | `LIGOTimeGPS` (秒+ナノ秒保持) | — |
...
### 閏秒 (Leap Seconds) はどのように扱われますか？

`gwexpy` は内部的に `astropy.time` および `gwpy` の変換ルール（IERS テーブル）に従います。

- **GPS 時刻**: 1980年1月6日 00:00:00 UTC を起点（0秒）とし、閏秒を含まない連続した秒数としてカウントされます。
- **変換**: UTC との変換時には、実行環境で利用可能な最新の閏秒テーブルが自動的に適用されます。
- **注意**: 未来の時刻や、直近で追加された閏秒を含む変換を行う場合は、`astropy` のデータ更新が必要になる場合があります。

### 非常に大きな配列を処理できますか？

はい。`to_gps` に pandas の `Series` や NumPy の `datetime64` 配列を渡すと、内部でループを回さずにベクトル演算を実行するため、10万要素を超えるような大量の時刻データも高速に変換可能です。

### タイムゾーンのない文字列の扱いは？

`"2015-09-14 09:50:45"` のようにタイムゾーンが指定されていない文字列を `to_gps` や `tconvert` に渡すと、**デフォルトで UTC** として解釈されます。ローカルタイムとして扱いたい場合は、必ずタイムゾーン名（例: `"Asia/Tokyo"`）を文字列に含めるか、タイムゾーン付きの `datetime` オブジェクトを渡してください。
