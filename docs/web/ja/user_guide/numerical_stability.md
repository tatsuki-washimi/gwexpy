...
ホワイトニングでは、`eps="auto"` と `eps=None` がデータ相対の正則化を選びます。
偏相関の `eps=None` は追加 ridge 項を使わない従来の挙動を維持し、`eps="auto"` は共分散相対の ridge 項を選びます。
明示値は有限かつ非負でなければなりません。
共分散に基づく演算で `eps=0` は追加する ridge 項だけを無効にし、shrinkage と疑似逆行列の扱いは独立しています。
Laplace の sigma ごとの正規化では、自動モードが有限かつ非ゼロの分母をそのまま使い、ゼロ振幅スライスはゼロのままです。
