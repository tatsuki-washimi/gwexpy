...
- [学習ロードマップ](#学習ロードマップ-learning-path)
- [次のステップ](#次のステップ-next-steps)
...
既に GWpy を使っている方向け。まずは差分レシピで移行の入口を掴み、必要になったら [GWpy 差分 API 一覧](gwpy_added_api_index_ja.md) で追加 API を引いてください。
...
* **高次元データ**: [Field API 入門](tutorials/field_scalar_intro.ipynb) / [スライス操作ガイド](scalarfield_slicing.md)
...
* [GWpy ユーザー向け移行ガイド](gwexpy_for_gwpy_users_ja.md) - 差分レシピから移行の入口を掴む
