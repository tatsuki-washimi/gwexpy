...
print(f"Space domains: {field_k.space_domains}")  # {'kx': 'k', 'ky': 'k', 'kz': 'k'} と表示されます
```
...
4次元の場から、任意の空間座標 $(x, y, z)$ における時系列データを 1 点または複数点まとめて抽出できます。指定した座標がグリッド上にない場合は、自動的に補間が行われます。

```python
# 座標 (0.5 m, 0.5 m, 0.5 m) での時系列を抽出
