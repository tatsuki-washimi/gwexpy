...
| **こんなときに読む** | notebook や network / optional backend 依存のテストがどう検証されるか知りたい、I/O 対応表とテストの関係を見たい、監査ノートの入口を知りたい、coverage 表示の意味と限界を確認したい |
| **検索ヒント** | verification, quality, coverage, notebook policy, Extended Verification, SUPPORTED_IO_MATRIX, codecov, audit trail |
...
検証の方法は対象ごとに異なります。network / optional backend 依存のテスト(I/O、Zarr、notebook)は手動トリガーの Extended Verification workflow で検証される部分もあれば、Notebook の中には CI で全実行されるものもあり、重い notebook のように構造確認中心のものもあります。optional dependency を持つテストは、環境によって skip される場合もあります。
:::
...
クロスプラットフォーム / optional backend gate | [Extended Verification workflow](https://github.com/tatsuki-washimi/gwexpy/blob/main/.github/workflows/extended-verification.yml) | macOS/Windows での import smoke test を行い、network-backed I/O・docs notebook 実行・Zarr I/O をゲートしている、手動トリガー CI の公開根拠 |
...
- network / optional backend 依存の I/O テスト(GWOSC datafind、Zarr 等)は、公開されている [extended verification workflow](https://github.com/tatsuki-washimi/gwexpy/blob/main/.github/workflows/extended-verification.yml) でゲートされますが、手動トリガーが必要です。
- **Light** notebook は、Notebook Policy 上では同じ workflow の docs-notebook gate で `papermill` により全実行される想定です。
...
- extended verification workflow により、network-backed I/O と Zarr I/O には自動ゲートがありますが、手動トリガー時のみ実行されます。
- 同じワークフローでも notebook の扱いは分類依存で、`Light` は `papermill`、`Heavy` は `nbval --nbval-lax` です。
...
- network-backed I/O や notebook 検証が、ドキュメント全体に対する単一の release-blocking gate だという意味でもありません。Extended Verification は手動トリガーであり、すべての PR で実行されるわけではないためです。
