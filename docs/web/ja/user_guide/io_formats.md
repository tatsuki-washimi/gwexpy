...
`to_*()` / `from_*()` による変換や、xarray / ROOT object / Zarr array との橋渡しは、このページでは扱いません。質問が「このオブジェクトを別ライブラリや別コンテナへどう変換するか」であれば、それは interop 側の話です。必要な場合は [他ライブラリ連携チュートリアル](../../en/user_guide/tutorials/intro_interop.ipynb) や [Interop API リファレンス](../reference/api/interop) を参照してください。
...
- **GWF** は `TimeSeries`、`TimeSeriesDict`、`StateVector`、`StateVectorDict` で `.gwf` ファイルの list / tuple 入力に対応します。ファイル名順ではなく、decode 後 payload の実際の時刻 span 順で結合し、同じ span は入力順を維持します。連続 span はそのまま結合し、ギャップは既定で失敗します。`pad=<値>` または `gap="pad"` で埋められ、`gap="ignore"` では埋めずに連結します。オーバーラップする span は既定または `gap="raise"` で失敗しますが、`gap="ignore"` では decode 後 span 順に連結し、オーバーラップの連結を許可します。`start` / `end` が実データの外側に伸びる場合も、既定の `gap="raise"` では失敗します。外側区間を埋めるには `pad=<値>` または `gap="pad"` を使ってください。`gap="pad"` を明示的な `pad=` なしで使うと、ギャップおよび外側区間は `NaN` で充填されます（`SeriesMatrix.append` と同じ挙動）。非浮動小数点チャンネルをこの方法で読むと `ValueError` になるため、整数チャンネルでは明示的に `pad=` を渡してください。`gap="ignore"` は内部ギャップも外側の `start` / `end` 区間も padding しません。複数ファイルでは `TimeSeries` と `TimeSeriesDict` は先頭 source から channel を自動検出できますが、multi-worker の `StateVector` と `StateVectorDict` には明示的な channel selector が必要です。
- **GWF parallel read:** 4 種の reader は `parallel=` と互換 alias の `nproc=` を受け取ります。`None`、`False`、`1` は serial、`True` は `min(CPU 数, source 数, 8)` worker、明示値は 2〜8 の整数です。両方を指定すると file/backend I/O 前に `TypeError` になります。実際に複数 worker を使う場合は個別の local `.gwf` frame path の list / tuple（`str`、`bytes`、`os.PathLike`）が必要です。public parallel boundary では各 `PathLike` を `os.fspath` で一度だけ immutable な `str` / `bytes` snapshot に変換し、その同じ snapshot を validation、scheduling、spawn worker に渡します。そのため `os.fspath` の例外は I/O 前に伝播し、`PathLike` が後から resolver / worker source を変えることはできません。existence で受理するのは local regular file（symlink を含む）に解決する正確な元の `os.fspath` spelling だけであり、percent-decode した counterpart を resolver / worker source に置換しません。未解決 spelling はまず raw token 化し、GWF/path/URI-like final token を持つ LAL cache-shaped 3〜5 token record を拒否します。header field に slash、backslash、drive、UNC-like text があっても同様です。続いて最大 4 回の percent decode の各段階で、cache、token boundary の URI（`//` なしの `file:` / `http:` を含む）、composite hazard を span/backend I/O 前に検査します。query/composite source、glob、file-like object も拒否されます。parallel mode では、2 つの `.gwf` 名を `+`、`|`、`;`、`@`、空白で連結した式（percent-encoding を含む）も意図的に拒否します。判定は path component 単位なので、これらの文字は通常の単一 filename または directory component 内では使用できます。Windows の drive / UNC の単一 path 表記は構造的に受理されます。ファイル名に時刻 segment がない場合、preflight は選択済み local backend に scheduling validation のためだけに問い合わせます。結合順は引き続き decode 後 payload span で決まります。spawn-safe process の backend 例外（`ImportError` を含む）はそのまま伝播し、daemon process からは preflight で拒否されます。これは既存の gwpy-style unified reader interface に対する GWF 固有の拡張です。
