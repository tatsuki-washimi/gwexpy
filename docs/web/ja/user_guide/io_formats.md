...
- **GWF** は `TimeSeries` と `TimeSeriesDict` で `.gwf` ファイルの list / tuple 入力に対応します。ファイルは時刻 span 順で結合されます。連続 span はそのまま結合し、ギャップは既定で失敗します。`pad=<値>` または `gap="pad"` で埋められ、`gap="ignore"` では埋めずに連結します。オーバーラップする span は既定または `gap="raise"` で失敗しますが、`gap="ignore"` では span 順に連結し、オーバーラップの連結を許可します。`start` / `end` が実データの外側に伸びる場合も、既定の `gap="raise"` では失敗します。外側区間を埋めるには `pad=<値>` または `gap="pad"` を使ってください。`gap="pad"` を明示的な `pad=` なしで使うと、ギャップおよび外側区間は `NaN` で充填されます（`SeriesMatrix.append` と同じ挙動）。非浮動小数点チャンネルをこの方法で読むと `ValueError` になるため、整数チャンネルでは明示的に `pad=` を渡してください。`gap="ignore"` は内部ギャップも外側の `start` / `end` 区間も padding しません。複数ファイル読み込みで channel 名を指定しない場合、自動検出は先頭ファイルを使い、残りのファイルにも互換 channel がある前提です。
- **FFL** (`.ffl`) は読み込み専用のローカル frame-file list です。GWexpy は、1 フィールドの `.gwf` エントリ、5 フィールドで path を先頭に置く FFL エントリ、3 フィールドの nested `.ffl` エントリを、それぞれ格納元の list を基準に解決し、`.gwf` list と同じ時刻 span マージを適用します。`.ffl` では `format="gwf"` を省略できます。include cycle は `ValueError` になります。空行は無視します。コメント、remote URL、その他の cache 構文はこの契約の対象外です。
