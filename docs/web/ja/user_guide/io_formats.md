...
- **GWF parallel read:** 4 種の reader は `parallel=` と互換 alias の `nproc=` を受け取ります。`None`、`False`、`1` は serial、`True` は `min(CPU 数, source 数, 8)` worker、明示値は 2〜8 の整数です。両方を指定すると file/backend I/O 前に `TypeError` になります。実際に複数 worker を使う場合は local frame path の list / tuple が必要で、spawn-safe process を使い worker 例外をそのまま伝播します。daemon process からは preflight で拒否されます。これは既存の gwpy-style unified reader interface に対する GWF 固有の拡張です。
