...
- **GWF parallel read:** 4 種の reader は `parallel=` と互換 alias の `nproc=` を受け取ります。`None`、`False`、`1` は serial、`True` は `min(CPU 数, source 数, 8)` worker、明示値は 2〜8 の整数です。両方を指定すると file/backend I/O 前に `TypeError` になります。実際に複数 worker を使う場合は個別の local `.gwf` frame path の list / tuple（`str`、`bytes`、`os.PathLike`）が必要です。regular local file（symlink を含む）は先に受理し、それ以外では token boundary の URI scheme（`//` なしの `file:` / `http:` も含む）と、scientific / absent / malformed segment field を含む LAL cache-shaped GWF record を、query/composite source、glob、file-like object と同様に span/backend I/O 前に拒否します。validation は percent-encoding を最大 4 回 decode し、なお escape が残る spelling を拒否するため、再帰 encoding が後段で frame-list expression に変わることはありません。parallel mode では、2 つの `.gwf` 名を `+`、`|`、`;`、`@`、空白で連結した式（percent-encoding を含む）も意図的に拒否します。判定は path component 単位なので、これらの文字は通常の単一 filename または directory component 内では使用できます。Windows の drive / UNC 表記は構造的に受理されます。ファイル名に時刻 segment がない場合、preflight は選択済み local backend に scheduling validation のためだけに問い合わせます。結合順は引き続き decode 後 payload span で決まります。spawn-safe process の backend 例外（`ImportError` を含む）はそのまま伝播し、daemon process からは preflight で拒否されます。これは既存の gwpy-style unified reader interface に対する GWF 固有の拡張です。
