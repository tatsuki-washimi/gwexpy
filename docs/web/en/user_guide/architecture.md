...
`gwexpy` builds advanced analysis pipelines by combining the following core components. For detailed mathematical and physical foundations, refer to {doc}`physics_models`.
...
## Related Documents
- {doc}`physics_models` — Detailed analysis theory and physics models (ICA/BruCo/MCMC)
