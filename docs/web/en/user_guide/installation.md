...
conda-forge package is in staged-recipes review and is not yet available as a
released conda package. You can still install `gwexpy` from PyPI inside a
Conda-managed environment. Do not use `conda install -c conda-forge gwexpy`
until the package appears on conda-forge and a fresh install smoke test has
passed.
:::
