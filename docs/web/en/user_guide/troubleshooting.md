...
**Solution:**
Use Conda (Miniforge, etc.) in a dedicated environment, then install GWexpy there.
```bash
...
If you started from the smallest example in [Quickstart](quickstart.md), note that `pygmt` is optional and is not part of the minimal install.
