...
- **GWF** reads accept a list or tuple of `.gwf` files for `TimeSeries`, `TimeSeriesDict`, `StateVector`, and `StateVectorDict`. Files are merged by each decoded payload's actual time span (not filename order); equal decoded spans retain input order. Contiguous spans join normally, gaps raise by default, `pad=<value>` or `gap="pad"` fills gaps, and `gap="ignore"` concatenates without filling. Overlapping spans are rejected by default or with `gap="raise"`, while `gap="ignore"` concatenates files in decoded-span order and permits overlap concatenation. If `start` or `end` extends beyond available data, the default `gap="raise"` behavior rejects the request; use `pad=<value>` or `gap="pad"` to fill the outer interval. When `gap="pad"` is used without an explicit `pad=`, gaps and outer intervals are filled with `NaN` (matching `SeriesMatrix.append`); reading a non-floating-point channel this way raises a `ValueError`, so pass an explicit `pad=` for integer channels. `gap="ignore"` never pads missing samples, including outer `start`/`end` ranges. `TimeSeries` and `TimeSeriesDict` may auto-discover channels from the first multi-file source; multi-worker `StateVector` and `StateVectorDict` reads require an explicit channel selector.
- **GWF parallel reads:** all four readers accept `parallel=` and the compatible `nproc=` alias. `None`, `False`, and `1` keep serial behavior; `True` selects `min(CPU count, source count, 8)` workers; explicit worker counts must be integers from 2 through 8. Supplying both aliases raises `TypeError` before file/backend I/O. Actual multi-worker reads require a list or tuple of individual local `.gwf` frame paths (`str`, `bytes`, or `os.PathLike`). At the public parallel boundary, each `PathLike` is converted with `os.fspath` exactly once into an immutable `str`/`bytes` snapshot; that same snapshot is validated, scheduled, and passed to the spawned worker. `os.fspath` errors therefore propagate before I/O, and a `PathLike` cannot change its later resolver or worker source. Only an exact original `os.fspath` spelling that is a local regular file (including a symlink) is accepted by existence; a percent-decoded counterpart is never substituted for the resolver or worker source. Unresolved spellings are first tokenized raw: LAL cache-shaped three-to-five-token records with a GWF/path/URI-like final token are rejected even when the header fields contain arbitrary slash, backslash, drive, or UNC-like text. Bounded percent decoding then checks every stage for cache, token-boundary URI (including `file:`/`http:` forms without `//`), and composite hazards before span/backend I/O. Query/composite sources, globs, and file-like objects are also rejected. In parallel mode, expressions that join two `.gwf` names with `+`, `|`, `;`, `@`, or whitespace (including percent-encoded spellings) are intentionally rejected. The check is path-component-aware: these characters remain valid inside an ordinary filename or directory component. Windows drive and UNC single-path spellings are accepted structurally. When a filename has no encoded time segment, preflight queries the selected local backend only for scheduling validation; decoded payload spans still determine merge order. Spawn-safe workers propagate backend exceptions—including `ImportError`—unchanged; daemon processes are rejected during preflight. This is a GWF-specific extension of the existing gwpy-style unified reader interface.
