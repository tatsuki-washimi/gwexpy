...
- **GWF parallel reads:** all four readers accept `parallel=` and the compatible `nproc=` alias. `None`, `False`, and `1` keep serial behavior; `True` selects `min(CPU count, source count, 8)` workers; explicit worker counts must be integers from 2 through 8. Supplying both aliases raises `TypeError` before file/backend I/O. Actual multi-worker reads require a list or tuple of individual local `.gwf` frame paths (`str`, `bytes`, or `os.PathLike`). A regular local file (including a symlink) is accepted first; otherwise URI schemes at token boundaries—including `file:`/`http:` forms without `//`—and LAL cache-shaped GWF records (including scientific, absent, and malformed segment fields) are rejected before span or backend I/O, alongside query/composite sources, globs, and file-like objects. Validation percent-decodes at most four rounds and rejects a spelling that still contains an escape, so recursive encoding cannot turn into a later frame-list expression. In parallel mode, expressions that join two `.gwf` names with `+`, `|`, `;`, `@`, or whitespace (including percent-encoded spellings) are intentionally rejected. The check is path-component-aware: these characters remain valid inside an ordinary filename or directory component. Windows drive and UNC spellings are accepted structurally. When a filename has no encoded time segment, preflight queries the selected local backend only for scheduling validation; decoded payload spans still determine merge order. Spawn-safe workers propagate backend exceptions—including `ImportError`—unchanged; daemon processes are rejected during preflight. This is a GWF-specific extension of the existing gwpy-style unified reader interface.
