...
- **GWF parallel reads:** all four readers accept `parallel=` and the compatible `nproc=` alias. `None`, `False`, and `1` keep serial behavior; `True` selects `min(CPU count, source count, 8)` workers; explicit worker counts must be integers from 2 through 8. Supplying both aliases raises `TypeError` before file/backend I/O. Actual multi-worker reads require a list or tuple of local frame paths, use spawn-safe processes, and propagate worker exceptions unchanged; daemon processes are rejected during preflight. This is a GWF-specific extension of the existing gwpy-style unified reader interface.
