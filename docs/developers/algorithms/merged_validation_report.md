...
優先度付きアクション一覧における各ステータスの定義は以下の通りです。

*   **Fixed**: 既存のコードを修正・置換し、問題が解決された状態（回帰試験または検証済み）。
*   **Implemented**: 新しい機能や経路として実装された状態（デフォルト経路への統合や互換性告知は今後の課題となり得る）。
*   **Done**: ドキュメントの更新や仕様の明確化のみで対応完了とした状態（コード変更なし）。
*   **Open**: 未対応、または要継続調査の状態（Issue化推奨）。
...
| DTT XML: 位相消失                       | A      | **独立検証済み** — subtype 6 で `.real` 使用を確認 |
...
## 補足分析（Gemini (Web) 2026-02-01）

検証レポートの指摘内容は物理的・統計的に正当ですが、これを実際の**ソフトウェア修正として適用する際の実務的なリスクと解決策**について、シニアエンジニアの視点から補足します。

### 1. MCMC計算のパフォーマンス最適化

**P1-1「対数尤度における log det Σ の追加」** は理論的には必須ですが、実装上は計算コストがボトルネックになります。

| 課題               | 詳細                                                                                                    |
| ------------------ | ------------------------------------------------------------------------------------------------------- |
| **計算コスト**     | MCMCの各ステップで Σ の行列式計算を行うと、計算時間が劇的に増大する                                     | 
| **数値的不安定性** | 共分散行列 Σ が悪条件（condition numberが大きい）の場合、`np.linalg.slogdet` は数値的に不安定になりがち |

**推奨策**:

- 単純な `slogdet` ではなく、**コレスキー分解** $\Sigma = LL^T$ を用いて $\log\det\Sigma = 2\sum_i \log L_{ii}$ と計算する
- GLS計算時にすでに $\Sigma^{-1}$ を求めるために分解を行っているはずなので、その中間生成物（$L$）を再利用することで、**計算コストをほぼゼロに抑えられる**

### 2. 「物理的正しさ」の自動テスト導入

指摘された物理単位やスケーリングの誤りは、コードレビューだけでは再発を防げません。

**推奨策**: **「既知の物理信号を用いた単体テスト (Physics Injection Test)」** をCIに追加する。

| テスト例                   | 内容                                                                                                                                                    |
| -------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **パーセバルの定理テスト** | 振幅 $A$、周波数 $f_0$ の正弦波を入力し、計算されたPSDの積分値（またはピーク値）が理論値（パーセバルの定理に基づくエネルギー）と一致することを `assert` |
| **位相復元テスト**         | 既知の位相を持つ複素信号を処理し、位相が正しく保持されることを確認                                                                                      |
| **次元復元テスト**         | 3D配列 → 変換 → 逆変換 → 元配列と一致することを確認                                                                                                     |

**効果**: 将来的なリファクタリング時にも物理的な整合性が保証される。

---

**備考**: これらのエンジニアリング観点を盛り込むことで、コードの品質だけでなく、ライブラリとしての信頼性と保守性を高めることができる。

---

## 補足分析（Perplexity 2026-02-01）

12種AIのクロス検証は信頼性が高く、誤判定の傾向分析も優れています。以下、**独立した追加視点**と**未指摘リスク**を補足します。

### 1. DTT XML「位相消失」の物理的帰結

**重大度**: 🔴 **High**（独立検証済み）

```python
parse_transfer(subtype=6): np.frombuffer(..., dtype='c8').real  # 虚部破棄！
```

| 項目           | 内容                                                                                     |
| -------------- | ---------------------------------------------------------------------------------------- |
| **物理的帰結** | 複素伝達関数 $|H(f)|e^{i\phi(f)}$ の位相 $\phi(f)$ 喪失 → **遅延・群速度解析完全不能** |
| **LIGO影響**   | KAGRA/LIGO共同解析で位相整合必須（LIGO-T1800044）                                        |
| **即時対応**   | `dtype=np.complex128` 保持 + `response = real + 1j*imag`                                 |

### 2. VIF+ブートストラップ「二重分散補正」メカニズム

**重大度**: 🟡 **Medium**（複数AIが指摘したがメカニズム未解析）

```python
if block_size: factor = 1.0  # VIF無効化
```

| 問題             | 詳細                                                                          |
| ---------------- | ----------------------------------------------------------------------------- |
| **未解決ケース** | `block_size=None`（標準ブートストラップ）時はVIF適用 → **相関無視の過小分散** |
| **統計的帰結**   | CI幅10-20%狭小 → 偽検出率↑                                                    |
| **参考文献**     | Percival & Walden (1993) Eq.(56) + Künsch (1989)                              |

### 3. 新規発見: Schumann共鳴「モード干渉無視」

**重大度**: 🔴 **High**（複数AIがNoise Models指摘したが詳細未深掘り）

```python
for f0, Q, A in modes:  # 逐次加算
    total_psd += lorentzian(f0, A, Q).value**2
```

| 項目         | 内容                                                                    |
| ------------ | ----------------------------------------------------------------------- |
| **物理誤り** | Schumann モードは**干渉する**（電磁共鳴）。逐次PSD加算 ≠ 実際スペクトル |
| **影響**     | モード間干渉によるスペクトル凹凸再現不能                                |
| **修正案**   | `total_field = sum(lorentzian_field); total_psd = \|total_field\|^2`    |
| **文献**     | Sentman, D.D. (1987) Schumann resonances                                |

**修正スニペット**:

```python
# Schumann干渉対応
fields = [lorentzian_field(f0, A, Q) for f0, Q, A in modes]
total_field = np.sum(fields, axis=0)
total_psd = np.abs(total_field)**2  # 干渉明示
```

### 4. 新規発見: HHT「IMF非直交性」深刻度

**重大度**: 🟡 **Medium**

| 項目           | 内容                                                      |
| -------------- | --------------------------------------------------------- |
| **数学的欠陥** | EMDは**直交性保証なし** → Hilbertスペクトルで偽周波数発生 |
| **推奨代替**   | VMD（Variational Mode Decomposition）の採用検討           |

### 5. 優先度再評価（Perplexity視点）

| 優先度         | 項目                      | 理由         |
| -------------- | ------------------------- | ------------ |
| **P0（即日）** | DTT XML位相消失           | 位相解析不能 |
| **P1**         | GLS logdet前提明示        | 全AI合意     |
| **P1**         | PCA cols消失              | 9/12合意     |
| **P2**         | Schumannモード干渉修正    | 物理誤り     |
| **P2**         | Transfer Function ε正則化 | 3AI指摘      |

### 6. AI間誤判定のメタ分析

| 誤判定パターン  | 該当AI     | 判定     | 根拠                           |
| --------------- | ---------- | -------- | ------------------------------ |
| VIF=回帰VIF混同 | I, E(一部) | **誤り** | Percival Ch.7 vs. Menard(1995) |
| leap-second懸念 | L(前回)    | **誤り** | LIGO GPS=TAI連続               |
| 複素FFT 2×問題  | L(前回)    | **誤り** | `rfft`実信号専用               |

### 7. 全体評価

| 項目                    | 評価                                        |
| ----------------------- | ------------------------------------------- |
| **現状品質**            | **優秀（85/100）** — 基本アルゴリズムは堅牢 |
| **即時対応3項目完了後** | **95/100** へ向上可能                       |
| **12AIクロス検証**      | 業界最高水準                                |

**最終推奨**: DTT XMLを最優先で修正 → GWOSC O4データでend-to-end検証
