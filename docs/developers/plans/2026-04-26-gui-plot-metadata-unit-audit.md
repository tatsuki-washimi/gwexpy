...
- Main worktree had uncommitted changes, so the original document was prepared in a separate worktree at `/tmp/gwexpy-issue252`; PR #261 review passes were prepared in `/tmp/gwexpy-pr261-address-review` and `/tmp/gwexpy-pr261-followup-review`.
...
| `HHTSpectrogram.plot()` from `TimeSeries.hht(output="spectrogram")` | `gwexpy/types/hht_spectrogram.py`, returned by `gwexpy/timeseries/_spectral_special.py` | Repo-owned fallback labels are `Time [{self.xunit}]` and `Frequency [{self.yunit}]`; defaults also set `xscale="auto-gps"` and `yscale="log"` | Repo-owned colorbar label starts as bare `Power` when `self.unit is None`; otherwise it infers `Power [unit]` when the unit string contains `^2` or `**2`, `Amplitude [unit]` otherwise, and `Amplitude/Power [unit]` for non-string unit fallback | `TimeSeries.hht()` preserves `name`, `channel`, and `epoch` on the returned object, but `plot()` does not display name/channel metadata | No explicit phase label; default log normalization assumes positive amplitude/power-like values | HHT tests cover spectrogram construction and options, but not plot labels/colorbars | Keep the HHT plot gap. Add headless tests for x/y labels, x/y scales, `unit is None` colorbar text, `weight="ia"`/`weight="ia2"` colorbar labels, and explicit-label preservation. The squared-token heuristic can misclassify non-power units if a unit string contains an exponent, for example acceleration-like `m/s^2` rendered with `^2`. |
...
| `Plot.plot_summary()` | `gwexpy/plot/plot.py` | Summary frequency axis uses `Frequency [Hz]`; spectrogram axis uses `Frequency [Hz]` | Spectrogram colorbar label uses `sg.unit.to_string("latex_inline")` | Matrix row keys or item names are used in titles | Coherence/ASD/PSD title type is inferred from unit/name, but phase-specific labels are not explicit | Not specifically label-tested | Add focused headless tests for titles, summary axes, and colorbar label. Real bug/gap: `sg.unit.to_string("latex_inline")`, `str(sg.unit)`, and `sg.unit.is_equivalent("")` assume `sg.unit` is not `None`; `plot_summary()` can raise for unitless spectrograms before or while adding the colorbar. |
| `plot_gauch_dashboard()` | `gwexpy/plot/gauch_dashboard.py` | GauCh/Rayleigh panels hard-code `Frequency [Hz]`; the time-series panel hard-codes `Time [s]` and formats `Amplitude [{ts.unit}]`, producing `Amplitude [None]` when `ts.unit is None` | Colorbars are explicit: `-log10(p-value)`, `Rayleigh Statistic`, or `KS Statistic (Dn)` | Titles are explicit dashboard labels; `ts.name`/channel metadata are not displayed | Not applicable; dashboard values are p-value/statistic/time-series amplitude views | Tutorial references only; no direct automated dashboard label tests found | Keep the dashboard plot gap. Add headless tests for panel titles, hard-coded frequency/time labels, the `ts.unit is None` label risk, colorbar labels, and Rayleigh-vs-GauCh fallback paths. |
...
| GUI streaming accumulator | `gwexpy/gui/streaming.py` | Returns raw arrays/dicts without unit fields | Value unit is stripped before rendering | Channel/name metadata not carried in result payloads | Phase/dB semantics are not represented in payload metadata | GUI tests cover render paths, not metadata | Same metadata-payload gap as non-streaming engine. Manual/follow-up item: `common_t0` is initialized from the first packet without an explicit lock around the check/set path, so concurrent streaming ingestion could race on the shared alignment reference. |
| GUI plot renderer | `gwexpy/gui/ui/plot_renderer.py` | Updates bottom time label only; no y-axis/value labels from data metadata | Applies display conversion but does not update value-unit/colorbar labels | Renderer receives no name/channel metadata from engine results | `Phase` display returns degrees for complex data and zeros for real data, but no label says degrees | `tests/gui/test_plot_renderer.py` covers dB conversion, relative time, log-y rect, and bottom time label | Add tests for current absence of y/value labels, then later metadata-driven label tests. Audit/follow-up candidate: spectrogram image levels use `np.min(data)`/`np.max(data)` after display conversion; NaN, inf, or all-invalid converted data can yield invalid image levels. |
...
- `Plot.plot_summary` unitless-spectrogram behavior around summary-title and colorbar label generation.
...
- Spectrogram image level handling when display conversion produces NaN, inf, or all-invalid data.
...
- Streaming concurrency review for `SpectralAccumulator.common_t0` alignment. The check/set path is not explicitly locked, so concurrent `add_chunk()` calls should be stress-tested or hardened in a follow-up PR.
...
- HHT colorbar amplitude/power classification is heuristic. Unitless labels and exponent-bearing non-power units should be reviewed before changing display semantics.
...
2. Headless label tests for existing Matplotlib contracts: add assertions for `FieldPlot.add_scalar`, `ScalarField.plot_map2d`, `ScalarField.plot_profile`, `ScalarField.plot_time_space_map`, `BifrequencyMap.plot`, `HHTSpectrogram.plot`, `Plot.plot_summary`, and `plot_gauch_dashboard`. Keep expected values aligned to current behavior, including current unitless-label and hard-coded-label risks where behavior is intentionally unchanged.
...
4. GUI payload contract tests: assert current engine/streaming result payloads and renderer label behavior, especially the current absence of unit/name/channel metadata. Include spectrogram level handling for non-finite converted data. This gives a safe baseline before later behavior work.
...
6. Streaming concurrency follow-up: review or harden `SpectralAccumulator.common_t0` initialization and alignment under concurrent `add_chunk()` calls.
7. Post-contract behavior PR: after #244/#246, add structured GUI result metadata such as x unit, y unit, value unit, display unit, name, and channel; then update renderer labels and tests.
8. Physics-reviewed complex/phase PR: after human review, decide whether angle/power map colorbar labels should use transformed units and whether GUI phase should label degrees explicitly.
