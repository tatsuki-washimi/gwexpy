...
`Field4D` クラスの強力な描画・解析機能をユーザーが直感的に利用できるよう、チュートリアルを整理・拡充し、同時に数値的正当性を保証する不変量チェックを導入する。

## 2. 目標

1. **ノートブックの分割**: `intro_Field4D.ipynb` (コア) と `plot_Field4D.ipynb` (描画) に分離し、保守性を向上。
...
#### 1. `intro_Field4D.ipynb` (Core Specifications)
...
| ファイル | 説明 | 状態 |
| :--- | :--- | :--- |
| `examples/tutorials/intro_Field4D.ipynb` | コア機能 + 数値的不変量チェック | ✅ 更新済み |
| `examples/tutorials/plot_Field4D.ipynb` | 描画機能の実践チュートリアル | ✅ 新規作成 |
