# ScalarField 向け信号処理拡張 計画書 (2026-01-21 17:51:40)

- 元ファイル: `docs/developers/plans/Signal Processing Extensions for ScalarField: PSD, Correlation and Coherence Utilities.docx`
...
# ScalarField フェーズ3：信号処理拡張 計画

本計画は、**ScalarField** に信号処理ユーティリティを追加するために必要なタスクを整理したものです。目的は、4D データセットに対してスペクトル解析、相関、コヒーレンス、遅延推定を行うためのツールを研究者に提供しつつ、これまでのフェーズで確立した設計原則（軸メタデータ、4D 形状の保持、単位の整合性）を維持することです。実装は、コアとなる計算メソッドと薄い描画ラッパーを中心に、包括的なテストとドキュメントを伴って進めます。

## 目的

- PSD（パワースペクトル密度）推定と周波数–空間マッピングにより、ScalarField データの周波数領域解析を可能にする
...
2.  固定シードと既知の波形（例：伝搬するガウシアン波、正弦波など）を用いて、決定論的な ScalarField を生成するヘルパー関数（例：`make_demo_field4d`）を実装する
...
9.  下流の描画へ渡しやすい形として、`Array2D` もしくは `ScalarField` の view を返す
...
9.  遅延値を保持する `ScalarField` スライスを返す
...
4.  バンドごとの計算か複数バンドかに応じて、`ScalarField` もしくは `ScalarFieldDict` を返す
...
2.  デモデータセット上で PSD、周波数–空間マップ、相互相関、遅延マップ、コヒーレンス解析を実演する `ScalarField_signal_processing.ipynb` を新規作成する
...
8.  ScalarField の 1D/2D プロットと、新しい信号処理プロットの差分を強調する
...
**“Signal Processing Extensions for ScalarField: PSD, Correlation and Coherence Utilities”**

このタイトルは、ScalarField にスペクトル解析と相関解析ツールを拡張するフェーズ3の要点を簡潔に表現しています。
