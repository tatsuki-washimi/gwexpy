...
2.  固定シードと既知の波形（例：伝搬するガウシアン波、正弦波など）を用いて、決定論的な ScalarField を生成するヘルパー関数（例：`make_demo_scalar_field`）を実装する
...
4.  バンドごとの計算か複数バンドかに応じて、`ScalarField` もしくは `FieldDict` を返す
