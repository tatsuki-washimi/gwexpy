...
Independent review found and closed the missing-`t0`, `Reference[n]` pair, and declared Array-dimension defects. The initial focused DTTXML suite passed 73 tests; full `pytest -q tests/` passed 12,980 with 307 skipped and 6 xfailed. Full Ruff, changed-file formatting, changed-module mypy, I/O contract, and I/O conformance gates passed. This evidence preceded the public-helper compatibility repair below and was requalified after the later changes.

The completion auditor judged the **pre-sample implementation complete** and **full Issue #726 acceptance partial** until the user's real TF XML could be inspected. The formal changed-file gate was partial because markdownlint was unavailable for two Markdown files and the two notebooks had no `.ipynb` quality-gate profile; direct notebook JSON and execution checks passed. Steps 7 and 8 were open at this stage for real-sample validation and final closure. No release was made.
...
The revised case studies describe the two backend value types explicitly. The focused DTTXML suite passed 74 tests at this stage. After the compatibility repair, the full suite passed 12,979 tests with 309 skipped and 6 xfailed; the I/O contract gate passed 1,523 with 24 skipped and 1 deselected, and I/O conformance passed 71 with 7 skipped. Full Ruff, changed-file formatting, changed-module mypy, and diff checks passed. The docs workflow prepared 66 canonical notebooks in a temporary copy, executed the changed DTTXML case study through MyST-NB, passed the public examples and JA/EN synchronization checks, and built both EN and JA HTML with notebook re-execution disabled. The all-notebook executing build was stopped after the changed case study passed because later unrelated notebooks ran for a long time. Real-sample qualification remained open at this stage.
