...
.. glossary::
   ...
   SpectrogramMatrix (ja)
      (スペクトログラム行列) 多チャネルのスペクトログラムデータを一括処理するための行列形式コンテナです。
   ...
   SeriesMatrix (ja)
      多チャネルの時系列・周波数系列・スペクトログラムを保持する行列基底クラスです。`TimeSeriesMatrix` や `FrequencySeriesMatrix` などの共通土台になります。
   ...
   ``Bruco`` (ja)
      Bruco はコヒーレンス／相関に基づくノイズ解析フレームワークです。GWexpy のドキュメントでは、クラス名と API 識別子を実装どおり `Bruco` に統一します。外部 upstream ツールやリポジトリ名を指す文脈では `BruCo` を併記する場合があります。
