...
from typing import Any, Dict, Generic, List, TypeVar, Union, cast, overload
...
def concat(left: Cycler[K, V], right: Cycler[K, U]) -> Cycler[K, V | U]:
    r"""Concatenate `Cycler`\s, as if chained using `itertools.chain`.
    ...
    if left.keys != right.keys:
        raise ValueError(
            "Keys do not match:\n"
            f"\tIntersection: {left.keys & right.keys!r}\n"
            f"\tDisjoint: {left.keys ^ right.keys!r}"
        )
