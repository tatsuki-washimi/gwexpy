...
def check_dict_parameters(waveform_dict,generic_param_dict=None):
    ...
    #Check the units of the parameteres are correct
            ...
            except: raise(TypeError( f"Parameter {k} does not have units, please pass a parameter with astropy units equivalent to u.[{pc.units_dict[default_unit_sys][k]}]"))
            assert waveform_dict[k].unit.is_equivalent(pc.units_dict[default_unit_sys][k]), f"Parameter {k} does not have proper units, units should be equivalent to u.[{pc.units_dict[default_unit_sys][k]}]"
        elif k=='condition':
            ...
            try : waveform_dict[k].unit #Check if it has units at all. Otherwise will give no clue about the parameter giving error
            except: raise(TypeError( f"Parameter {k} does not have units, please pass a parameter with astropy units equivalent to u.[{generic_param_dict[k]}]"))
            assert waveform_dict[k].unit.is_equivalent(generic_param_dict[k]), f"Parameter {k} does not have proper units, units should be equivalent to u.[{generic_param_dict[k]}]"
...
def add_params_units(waveform_dict,units_sys='S.I.',generic_param_dict=None):
    ...
    if units_sys in pc.units_dict.keys():#If units system is in units_dict we give units/convert to given units
        ...
        raise(TypeError(f'The units system specified is not available. Available systems are {[key for key in pc.units_dict.keys()]}'))
    ...
    for par in pc.mass_params_[0:4]:
        if par in dict_tmp.keys():
            if dict_tmp[par]>=2*10**30*u.solMass:
                warn_string = f"Are you sure you want to have a {par} of {u.Quantity(dict_tmp[par],u.solMass).value} Solar Masses?"
