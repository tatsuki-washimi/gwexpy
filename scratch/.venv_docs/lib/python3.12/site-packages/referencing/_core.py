...
from referencing.typing import URI, D, Mapping, Retrieve
...
class Resource(Generic[D]):
    r"""A document (deserialized JSON) with a concrete interpretation under a spec.
...
class Registry(Mapping[URI, Resource[D]]):
    r"""A registry of `Resource`\ s, each identified by their canonical URIs.
    ...
    def with_resources(
        ...
        r"""Add the given `Resource`\ s to the registry, without crawling them.
        """
        resources = self._resources
        uncrawled = self._uncrawled
    ...
    def with_contents(
        ...
        **kwargs: Any,
        ...
        r"""Add the given contents to the registry, autodetecting when necessary.
        """
