...
def to_nbytes(arg, default=None):
    ...
    if match is None:
        raise ValueError(
            ...
            f"(is your capitalisation correct?): {arg}"
        )
    ...
    try:
        ...
        raise ValueError(
            ...
            f"(is your capitalisation correct?): {unit}"
        )
    ...
    with imopen(uri, "rI", **imopen_args) as file:
        for image in file.iter(**kwargs):
            ...
            if nbytes > nbyte_limit:
                raise RuntimeError(
                    f"imageio.mimread() has read over {int(nbyte_limit)}B of "
                    "image data.\nStopped to avoid memory problems."
                    " Use imageio.get_reader(), increase threshold, or memtest=False"
                )
