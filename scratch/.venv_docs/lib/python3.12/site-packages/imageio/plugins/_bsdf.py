...
class BaseStream:
    ...
    def _activate(self, file, encode_func, decode_func):
        if self._f is not None:  # Associated with another write
            raise OSError("Stream object cannot be activated twice?")
        self._f = file
...
class ListStream(BaseStream):
    ...
    def append(self, item):
        ...
        if self._count != self._i:
            raise OSError("Can only append items to the end of the stream.")
        if self._f is None:
            raise OSError("List stream is not associated with a file yet.")
        if self._f.closed:
            raise OSError("Cannot stream to a close file.")
    ...
    def close(self, unstream=False):
        ...
        #     raise IOError('This ListStream is not in write mode.')
        if self._count != self._i:
            raise OSError("Can only close when at the end of the stream.")
        if self._f is None:
            raise OSError("ListStream is not associated with a file yet.")
        if self._f.closed:
            raise OSError("Cannot close a stream on a close file.")
        i = self._f.tell()
    ...
    def next(self):
        ...
        """
        if self._mode != "r":
            raise OSError("This ListStream in not in read mode.")
        if self._f is None:
            raise OSError("ListStream is not associated with a file yet.")
        if getattr(self._f, "closed", None):  # not present on 2.7 http req :/
            raise OSError("Cannot read a stream from a close file.")
        if self._count >= 0:
    ...
    def __iter__(self):
        if self._mode != "r":
            raise OSError("Cannot iterate: ListStream in not in read mode.")
        return self
...
class Blob:
    ...
    def seek(self, p):
        ...
        if p < 0 or p > self.allocated_size:
            raise OSError("Seek beyond blob boundaries.")
    ...
    def write(self, bb):
        ...
        if self.compression:
            raise OSError("Cannot arbitrarily write in compressed blob.")
        if self._f.tell() + len(bb) > self.end_pos:
            raise OSError("Write beyond blob boundaries.")
        self._modified = True
    ...
    def read(self, n):
        ...
        if self.compression:
            raise OSError("Cannot arbitrarily read in compressed blob.")
        if self._f.tell() + n > self.end_pos:
            raise OSError("Read beyond blob boundaries.")
