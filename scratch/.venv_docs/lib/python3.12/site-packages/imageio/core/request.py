...
from ..core import get_remote_file, urlopen
...
class Request:
    ...
    def _parse_uri(self, uri):
        ...
        if self._uri_type is None:
            ...
            if len(uri_r) > 60:
                uri_r = uri_r[:57] + "..."
            raise OSError("Cannot understand given URI: %s." % uri_r)

        # Check if this is supported
        noWriting = [URI_HTTP, URI_FTP]
        if is_write_request and self._uri_type in noWriting:
            raise OSError("imageio does not support writing to http/ftp.")
        ...
        if self._bytes is not None:
            ...
            try:
                ...
                if self._uri_type == URI_FILE:
                    raise OSError("Cannot seek back after getting firstbytes!")
