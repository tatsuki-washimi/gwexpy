...
def _setecho(fd, state):
    ...
    try:
        ...
        if err.args[0] == errno.EINVAL:
            raise OSError(err.args[0], '%s: %s.' % (err.args[1], errmsg))
        raise
...
class PtyProcess:
    ...
    def spawn(
        ...
        if pid == CHILD:
            ...
            try:
                ...
                tosend = f'OSError:{err.errno}:{str(err)}'
                if PY3:
                    tosend = tosend.encode('utf-8')
                os.write(exec_err_pipe_write, tosend)
                os.close(exec_err_pipe_write)
    ...
    def __repr__(self):
        ...
        if self.argv is not None:
            ...
            return f"{clsname}(pid={self.pid}, fd={self.fd})"
