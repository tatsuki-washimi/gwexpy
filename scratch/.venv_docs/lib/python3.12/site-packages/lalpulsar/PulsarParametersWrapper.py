...
class PulsarParametersPy:
    ...
    def values(self):
        ...
        for key in keys:
            if lalpulsar.PulsarCheckParam(self._pulsarparameters, key):
                ...
                raise ValueError(f"Could not find {key} in strcuture")
    ...
    def read(self, filename):
        ...
        if pp is None:
            raise OSError(
                f"Problem reading in pulsar parameter file '{filename}'"
            )
    ...
    def pp_to_str(self, precision=19):
        ...
        outputstr = f"{{name: <{mkl}}}{{value: <{vlb}}}{{fitflag}}\t{{error}}"
            ...
            if isinstance(tvalue, list) or isinstance(tvalue, np.ndarray):
                ...
                if key in ["WAVESIN", "WAVECOS"]:
                    ...
                    for ws, wc in zip(wavesin, wavecos):
                        precstrs = f"{{0:.{precision}f}}"  # print out float
                        if ws < 1e-6 or ws > 1e6:
                            # print out float in scientific notation
                            precstrs = f"{{0:.{precision}e}}"

                        precstrc = f"{{0:.{precision}f}}"  # print out float
                        if wc < 1e-6 or wc > 1e6:
                            # print out float in scientific notation
                            precstrc = f"{{0:.{precision}e}}"

                        outputdic = {}
                        outputdic["name"] = f"WAVE{idxsep}{idxoffset}"
                        idxoffset += 1
                        outputdic["value"] = f"{precstrs.format(ws)}\t{precstrc.format(wc)}"
                        outputdic["fitflag"] = ""
                        outputdic["error"] = ""
                    ...
                    for tv, te, tf in zip(tvalue, tevalue, fitflag):
                        precstr = f"{{0:.{precision}f}}"  # print out float
                        if tv < 1e-6 or tv > 1e6:
                            # print out float in scientific notation
                            precstr = f"{{0:.{precision}e}}"

                        precstre = f"{{0:.{precision}f}}"  # print out float
                        if te < 1e-6 or te > 1e6:
                            # print out float in scientific notation
                            precstre = f"{{0:.{precision}e}}"

                        outputdic = {}
                        outputdic["name"] = f"{key}{idxsep}{idxoffset}"
                        idxoffset += 1
    ...
    def pp_to_par(self, filename, precision=19):
        ...
        try:
            ...
            raise OSError(f"Could not open file '{filename}' for writing")
