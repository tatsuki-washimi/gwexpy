...
class knopeDAG(pipeline.CondorDAG):
    ...
    def setup_parameter_estimation(self):
        ...
        if self.pe_premade_prior_file is not None:
            if not os.path.isfile(self.pe_premade_prior_file):
                print(
                    f"Error... pre-made prior file '{self.pe_premade_prior_file}' does not exist!",
                    file=sys.stderr,
                )
                self.error_code = KNOPE_ERROR_GENERAL
                return
        ...
        if self.pe_derive_amplitude_prior:
            ...
            if self.pe_previous_posterior_files is None:
                if self.pe_previous_posteriors_file is not None:
                    try:
                        fp = open(self.pe_previous_posteriors_file)
            ...
            for prioritem in prior_options:
                ...
                if ptype != "gmm":
                    ...
                    if "nmodes" not in prior_options[prioritem]:
                        print(
                            f"Error... no 'nmodes' given for parameter '{prioritem}'",
                            file=sys.stderr,
                        )
                        self.error_code = KNOPE_ERROR_GENERAL
                        return outfile
                    ...
                    if "means" not in prior_options[prioritem]:
                        print(
                            f"Error... no 'means' given for parameter '{prioritem}'",
                            file=sys.stderr,
                        )
                        self.error_code = KNOPE_ERROR_GENERAL
                        return outfile
                    ...
                    if "covs" not in prior_options[prioritem]:
                        print(
                            f"Error... no 'covs' given for parameter '{prioritem}'",
                            file=sys.stderr,
                        )
                        self.error_code = KNOPE_ERROR_GENERAL
                        return outfile
                    ...
                    if "weights" not in prior_options[prioritem]:
                        print(
                            f"Error... no 'weights' given for parameter '{prioritem}'",
                            file=sys.stderr,
                        )
                        self.error_code = KNOPE_ERROR_GENERAL
                        return outfile
            ...
            if os.path.isfile(self.pe_amplitude_prior_file):
                ...
                if self.pe_prior_info is None:
                    try:
                        fpp = open(self.pe_amplitude_prior_file)
    ...
    def setup_heterodyne(self):
        ...
        for ifo in self.ifos:
            if ifo not in self.coarse_heterodyne_channels:
                print(
                    f"Error... could channel not specified for '{ifo}'",
                    file=sys.stderr,
                )
                self.error_code = KNOPE_ERROR_GENERAL
                return
    ...
    def setup_splinter(self):
        ...
        if self.splinter_min_seg_length > self.splinter_max_seg_length:
            print(
                f"Error... minimum segment length ({self.splinter_min_seg_length}) for SplInter is larger than maximum segment length ({self.splinter_max_seg_length})",
                file=sys.stderr,
            )
            self.error_code = KNOPE_ERROR_GENERAL
            return
        ...
        for ifo in self.ifos:
            ...
            if ifo not in frtypes:
                ...
                if isinstance(frtypes[ifo], str):
                    ...
                    print(
                        f"Error... frame types for '{ifo}' must be a single string or a list of strings",
                        file=sys.stderr,
                    )
                    self.error_code = KNOPE_ERROR_GENERAL
                    return
            ...
            for i in range(self.ndatasets[ifo]):
                ...
                if self.ndatasets[ifo] == 1:
                    ...
                    cachefile = os.path.join(
                        self.preprocessing_base_dir[ifo], f"cache_{i}.lcf"
                    )
                self.cache_files[ifo].append(cachefile)
