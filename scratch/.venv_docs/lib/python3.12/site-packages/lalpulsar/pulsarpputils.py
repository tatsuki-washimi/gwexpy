...
def antenna_response(gpsTime, ra, dec, psi, det):
    ...
    try:
        ...
        raise KeyError(f"ERROR. Key {det} is not a valid detector name.")
    ...
    if not isinstance(ra, float):
        if isinstance(ra, str):
            try:
                ...
                raise ValueError(
                    f"Right ascension string '{ra}' not formatted properly"
                )
        else:
            raise ValueError(
    ...
    if not isinstance(dec, float):
        if isinstance(dec, str):
            try:
                ...
                raise ValueError(
                    f"Declination string '{ra}' not formatted properly"
                )
        else:
            raise ValueError(
...
def inject_pulsar_signal(
    ...
    if len(freqfac) == 1 and len(detectors) != len(npsds):
        raise ValueError(
            f"Number of detectors {len(detectors)} not the same as number of noises {len(npsds)}"
        )

    if len(freqfac) == 2 and 2 * len(detectors) != len(npsds):
        raise ValueError(
            f"Number of detectors {len(detectors)} not half the number of noises {len(npsds)}"
        )
...
def detector_noise(det, f):
    ...
    if det == "AV1":  # Advanced Virgo
        ...
        raise ValueError(f"{det} is not a recognised detector")
    ...
    if fe == ".h5" or fe == ".hdf":  # HDF5 file
        if nestedsamples:
            ...
            try:
                ...
                raise OSError("Could not import nested samples")
        ...
        peparser = bppu.PEOutputParser("common")
        nsResultsObject = peparser.parse(open(postfile))
