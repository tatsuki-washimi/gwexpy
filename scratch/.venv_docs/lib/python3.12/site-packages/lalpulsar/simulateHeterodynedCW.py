...
class HeterodynedCWSimulator:
        ...
        if isinstance(det, lal.Detector):
            ...
            if not isinstance(det, str):
                ...
                try:
                    ...
                    raise ValueError(
                        f"Detector '{det}' was not a valid detector " "name."
                    )
    ...
    def units(self, units):
        ...
        if self.__units not in ["TCB", "TDB"]:
            raise ValueError(
                f"Unknown time system '{self.__units}' has been " "given."
            )
    ...
    def _read_par(self, par):
        ...
        if isinstance(par, str):
            try:
                ...
                raise OSError(f"Could not read in parameter file: '{par}'")
        else:
    ...
    def _initialise_ephemeris(self, earth_ephem, sun_ephem, time_corr):
        ...
        try:
            ...
            try:
                ...
                raise OSError(f"Could not read in ephemeris files: {e}")
        ...
        try:
            ...
            try:
                ...
                raise OSError(f"Could not read in time correction file: {e}")
