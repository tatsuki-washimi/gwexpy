...
def _extract_data_from_prior(prior):
    ...
    for i, key in enumerate(keys):
        ...
        if key not in keys:
            raise ValueError(f"Key {key}={prior[key]} not understood")
    ...
    logging.info(
        f"Computing Nstar over dimensions {lims_keys} while {scalar_keys} are scalar and {unfound_keys} unused"
    )
    lims = np.array(lims)
...
def get_Nstar_estimate(
    ...
    try:
        ...
        logging.warning(f"Encountered run-time error {e}")
