...
r"""Generate strain time series of a continuous-wave signal in the detector frame,
...
class CWSimulator:
    ...
    def get_strain(self, fs, tmin=0, tmax=None, noise_sqrt_Sh=0, noise_seed=None):
        ...
        if (tmin < 0) or (tmin >= self.__Tdata):
            raise ValueError(f"tmin must be within [0,{self.__Tdata}).")
        if tmax is None:
            tmax = self.__Tdata
        elif (tmax <= 0) or (tmax > self.__Tdata):
            raise ValueError(f"tmax must be within (0,{self.__Tdata}].")
        tspan = tmax - tmin
