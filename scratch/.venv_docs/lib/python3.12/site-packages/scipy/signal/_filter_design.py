...
def freqz_sos(sos, worN=512, whole=False, fs=2*pi):
    r"""Compute the frequency response of a digital filter in SOS format.
...
def tf2sos(b, a, pairing=None, *, analog=False):
    r"""Return second-order sections from transfer function representation
...
def sos2tf(sos):
    r"""Return a single transfer function from a series of second-order sections
...
def bilinear_zpk(z, p, k, fs):
    r"""Return a digital IIR filter from an analog one using a bilinear transform.
