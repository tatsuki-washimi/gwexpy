...
def _project(x0, lb, ub, constraints):
    ...
    if isinstance(x0, scalar_types):
        ...
        raise ValueError(f'{invoker}: UNEXPECTED ERROR: x0 should be a vector.')
    try:
        x0_c = np.asarray(x0_c, dtype=np.float64)
    except ValueError:
        raise ValueError(f'{invoker}: UNEXPECTED ERROR: x0 should contain only scalars.')
    if len(x0_c.shape) != 1:
        raise ValueError(f'{invoker}: UNEXPECTED ERROR: x0 should be a vector.')
    lenx0 = x0_c.size
    ...
    if isinstance(lb, scalar_types):
        ...
        raise ValueError(f'{invoker}: UNEXPECTED ERROR: lb should be a vector.')
    try:
        lb_c = np.asarray(lb_c, dtype=np.float64)
    except ValueError:
        raise ValueError(f'{invoker}: UNEXPECTED ERROR: lb should contain only scalars.')
    if len(lb_c.shape) != 1 or lb_c.size != lenx0:
        raise ValueError(f'{invoker}: UNEXPECTED ERROR: the size of lb is inconsistent with x0.')
    ...
    if isinstance(ub, scalar_types):
        ...
        raise ValueError(f'{invoker}: UNEXPECTED ERROR: ub should be a vector.')
    try:
        ub_c = np.asarray(ub_c, dtype=np.float64)
    except ValueError:
        raise ValueError(f'{invoker}: UNEXPECTED ERROR: ub should contain only scalars.')
    if len(ub_c.shape) != 1 or ub_c.size != lenx0:
        raise ValueError(f'{invoker}: UNEXPECTED ERROR: the size of ub is inconsistent with x0.')
    ...
    if not isinstance(constraints, dict) or not ({'linear', 'nonlinear'} <= set(constraints.keys())) or \
        ...
        raise ValueError(f'{invoker}: UNEXPECTED ERROR: The constraints are ill-defined.')
