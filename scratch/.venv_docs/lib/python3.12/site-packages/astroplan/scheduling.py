    ...
    def __repr__(self):
        ...
        if self.start_time is None or self.end_time is None:
            return orig_repr.replace('object at',
                                     f'({self.target.name}, unscheduled) at'
                                     )
        else:
            s = f'({self.target.name}, {self.start_time.iso} to {self.end_time.iso}) at'
...
class TransitionBlock:
    ...
    def __repr__(self):
        orig_repr = object.__repr__(self)
        comp_info = ', '.join([f'{c}: {t}'
                               for c, t in self.components.items()])
        if self.start_time is None or self.end_time is None:
            return orig_repr.replace('object at', f' ({comp_info}, unscheduled) at')
        else:
            s = f'({comp_info}, {self.start_time.iso} to {self.end_time.iso}) at'
            return orig_repr.replace('object at', s)
    ...
    def attempt_insert_block(self, b, new_start_time, start_time_idx):
        ...
        if not (slots_before or slots_after):
            ...
            try:
                ...
                print(f'Failed to insert {b.target.name} into schedule.\n{str(error)}')
                return False
        ...
        try:
            ...
            # this shouldn't ever happen
            print(f'Failed to insert {b.target.name} (dur: {b.duration}) into schedule.\n{new_start_time.iso}\n{str(error)}')
            return False
            ...
            if conf_name in newblock.configuration:
                ...
                if conf_times is not None:
                    ...
                    if ctime is not None:
                        s = f'{conf_name}:{old_conf} to {new_conf}'
                        components[s] = ctime
