...
class JsonIOStream:
    ...
    def read_json(self, decoder=None):
        ...
        try:
            ...
            try:
                raise OSError("Content-Length is missing or invalid:")
            except Exception:
                log_message_and_reraise_exception()
    ...
    def _handle(self):
        ...
        try:
            ...
            if result is NO_RESPONSE:
                assert self.response is None, (
                    f"Handler {util.srcnameof(handler)} for {self.describe()} must not return NO_RESPONSE if it has already "
                    "invoked request.respond()."
                )
            elif self.response is not None:
                assert result is None or result is self.response.body, (
                    f"Handler {util.srcnameof(handler)} for {self.describe()} must not return a response body if it has "
                    "already invoked request.respond()."
                )
            else:
                assert result is not None, (
                    f"Handler {util.srcnameof(handler)} for {self.describe()} must either call request.respond() before it "
...
class JsonMessageChannel:
    ...
    def _get_handler_for(self, type, name):
        ...
        raise AttributeError(
            f"handler object {util.srcnameof(handlers)} for channel {self} has no handler for {type} {name!r}"
        )
