...
HELP = f"""debugpy {debugpy.__version__}
...
def in_range(parser, start, stop):
    def parse(s):
        ...
        if start is not None and n < start:
            raise ValueError(f"must be >= {start}")
        if stop is not None and n >= stop:
            raise ValueError(f"must be < {stop}")
        return n
...
def set_config(arg, it):
    ...
    try:
        ...
        raise ValueError(f"{name!r} must be a {expected_type.__name__}")
...
def parse_args_helper(
    ...
    while True:
        ...
        try:
            ...
            raise ValueError(f"{switch}: missing {placeholder}")
        except Exception as exc:
            raise ValueError(f"invalid {switch} {placeholder}: {exc}")
    ...
    if (not options.disable_sys_remote_exec) and hasattr(sys, "remote_exec"):
        ...
        try:
            ...
            with tempfile.NamedTemporaryFile(delete=False) as tmp_file:
                ...
                tmp_file.write(
                    f"""import os;os.remove("{tmp_file_path}");""".encode()
                )
                tmp_file.flush()
                tmp_file.close()
