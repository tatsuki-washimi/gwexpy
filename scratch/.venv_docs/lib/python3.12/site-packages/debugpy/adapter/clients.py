...
class Client(components.Component):
    ...
    def __init__(self, sock):
        if sock == "stdio":
            ...
            sys.stdin = stdin = open(os.devnull)
            ...
            if self.server:
                ...
                try:
                    ...
                    self.session.finalize(
                        f"{self.server} disconnected before responding to {json.repr(request.command)}"
                    )
                    return
    ...
    def attach_request(self, request):
        ...
        if conn is None:
            if sub_pid != ():
                ...
                request.respond({})
                self.session.finalize(
                    f'No known subprocess with "subProcessId":{sub_pid}'
                )
                return
    ...
    def notify_of_subprocess(self, conn):
        ...
        body["name"] = f"Subprocess {conn.pid}"
