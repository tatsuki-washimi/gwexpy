...
def parse_log(stream, requested_files, git_dir, exclude_commits, file_dates):
    ...
    # First line is blank
    assert not line0.rstrip(), f'unexpected git output in {git_dir}: {line0}'
    ...
    while requested_files:
        ...
        assert len(pieces) == 3, f'invalid git info in {git_dir}: {line1}'
        timestamp, commit, parent_commits = pieces
        line2 = stream.readline().rstrip()
        assert line2.endswith(b'\0'), f'unexpected file list in {git_dir}: {line2}'
        line2 = line2.rstrip(b'\0')
        assert line2, f'no changed files in {git_dir} (parent commit(s): {parent_commits})'
...
def _env_updated(app, env):
    ...
    for git_dir in srcdir_iter:
        try:
            ...
            msg += f' for source files from {git_dir})'
            if e.stderr:
...
def _html_page_context(app, pagename, templatename, context, doctree):
    ...
    utc_date = datetime.fromtimestamp(int(timestamp), UTC)
    ...
    if app.config.git_last_updated_metatags:
        context['metatags'] += f"""
    <meta property="article:modified_time" content="{date.isoformat()}" />"""
