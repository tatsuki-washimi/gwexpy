...
class LIGOTimeGPS(object):
    ...
    def __init__(self, seconds, nanoseconds=0):
        ...
        if isinstance(seconds, float):
            ...
            if isinstance(seconds, (bytes, str)):
...
try:
    import lal.utils
    class CacheEntry(lal.utils.CacheEntry):   # noqa: E301
    ...
    class CacheEntry(object):
        def __init__(self, *args, **kwargs):
            raise ValueError("glue.lal.CacheEntry is deprecated and lal is not available."
                             " Use lal.utils.CacheEntry instead.")
...
class Cache(list):
    ...
    def from_urls(cls, urllist, coltype=LIGOTimeGPS):
        ...
        """
        def pfn_to_url(url):
            scheme, host, path, dummy, dummy = urlsplit(url)
            if scheme == "":
                path = os.path.abspath(path)
            return urlunsplit((scheme or "file", host or "localhost", path, "", ""))
