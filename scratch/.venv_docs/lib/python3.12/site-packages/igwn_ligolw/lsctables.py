...
def New(cls, columns=None, **kwargs):
    warnings.warn(
        "igwn_ligolw.lsctables.New() is deprecated.  use igwn_ligolw.Table.new() instead."
    )
...
def HasNonLSCTables(elem):
    ...
    """
    return any(
        t.Name not in ligolw.Table.TableByName
...
class gpsproperty:
    ...
    def __get__(self, obj, cls=None):
        ...
        if ns == 0xFFFFFFFF:
            ...
            raise ValueError(
                "unrecognized denormalized number LIGOTimeGPS(%d,%d)" % (s, ns)
            )
    ...
    def __set__(self, obj, gps):
        if gps is None:
            ...
            if abs(ns) > 999999999:
                raise ValueError(
                    "denormalized LIGOTimeGPS not allowed: LIGOTimeGPS(%d, %d)"
                    % (s, ns)
                )
...
class Process(ligolw.Table.RowType):
    ...
    def initialized(
        ...
        self = cls(
            ...
            cvs_entry_time=lal.UTCToGPS(
                time.strptime(cvs_entry_time, "%Y-%m-%d %H:%M:%S +0000")
            )
...
class ProcessParamsTable(ligolw.Table):
    ...
    def append(self, row):
        if row.type is not None and row.type not in ligolwtypes.Types:
            raise ligolw.ElementError(
                "unrecognized type '%s' for process %d param '%s'"
                % (row.type, row.process_id, row.param)
            )
...
class ProcessParams(ligolw.Table.RowType):
    ...
    def pyvalue(self, value):
        if value is None:
            ...
            self.value = (
                value
                if self.type in ligolwtypes.StringTypes
                else ligolwtypes.FormatFunc[self.type](value)
            )
...
class SearchSummaryTable(ligolw.Table):
    ...
    def get_in_segmentlistdict(self, process_ids=None):
        ...
        for row in self:
            ...
            if process_ids is None or row.process_id in process_ids:
                seglists.extend(
                    dict((ifo, segments.segmentlist([row.in_segment])) for ifo in ifos)
                )
        return seglists
    ...
    def get_out_segmentlistdict(self, process_ids=None):
        ...
        for row in self:
            ...
            if process_ids is None or row.process_id in process_ids:
                seglists.extend(
                    dict((ifo, segments.segmentlist([row.out_segment])) for ifo in ifos)
                )
        return seglists
...
class SnglBurst(ligolw.Table.RowType):
    ...
    def band(self):
        ...
        return segments.segment(
            self.central_freq - self.bandwidth / 2.0,
            self.central_freq + self.bandwidth / 2.0,
        )
...
class SnglInspiral(ligolw.Table.RowType):
    ...
    def __eq__(self, other):
        return (
            ...
            and self.mass1 == other.mass1
            and self.mass2 == other.mass2
            and self.spin1 == other.spin1
            and self.spin2 == other.spin2
...
class SimInspiral(ligolw.Table.RowType):
    ...
    time_geocent = gpsproperty_with_gmst(
        "geocent_end_time", "geocent_end_time_ns", "end_time_gmst"
    )
    ...
    def time_at_instrument(self, instrument, offsetvector):
        ...
        ra, dec = self.ra_dec
        return t_geocent + lal.TimeDelayFromEarthCenter(
            lal.cached_detector_by_prefix[instrument].location, ra, dec, t_geocent
        )
    ...
    def snr_geometry_factors(self, instruments):
        ...
        for instrument in instruments:
            ...
            snr_geometry_factors[instrument] = complex(
                -fc * cosi, fp * (1.0 + cos2i) / 2.0
            ) * cmath.exp(-2.0j * self.coa_phase)
    ...
    def effective_distances(self, instruments):
        ...
        """
        return {
            instrument: self.distance / snr_geometry_factor
            for instrument, snr_geometry_factor in self.snr_geometry_factors(
                instruments
            ).items()
        }
    ...
    def expected_snrs(self, horizon_distances):
        ...
        """
        return {
            instrument: 8.0 * horizon_distances[instrument] / effective_distance
            for instrument, effective_distance in self.effective_distances(
                horizon_distances
            ).items()
        }
...
class SimBurst(ligolw.Table.RowType):
    ...
    time_geocent = gpsproperty_with_gmst(
        "time_geocent_gps", "time_geocent_gps_ns", "time_geocent_gmst"
    )
...
class SimRingdown(ligolw.Table.RowType):
    ...
    geocent_start = gpsproperty_with_gmst(
        "geocent_start_time", "geocent_start_time_ns", "start_time_gmst"
    )
...
class SimCBC(ligolw.Table.RowType):
    ...
    def effective_distances(self, instruments):
        ...
        """
        return {
            instrument: self.d_lum / snr_geometry_factor
            for instrument, snr_geometry_factor in self.snr_geometry_factors(
                instruments
            ).items()
        }
...
class Segment(ligolw.Table.RowType):
    ...
    [<igwn_ligolw.lsctables.Segment object at 0x...>, <igwn_ligolw.lsctables.Segment object at 0x...>]
...
class SegmentSumTable(ligolw.Table):
    ...
    def get(self, segment_def_id=None):
        ...
        return segments.segmentlist(
            row.segment for row in self if row.segment_def_id == segment_def_id
        )
...
class TimeSlideTable(ligolw.Table):
    ...
    def as_dict(self):
        ...
        return dict(
            (time_slide_id, offsetvector((row.instrument, row.offset) for row in rows))
            for time_slide_id, rows in itertools.groupby(
                sorted(self, key=lambda row: row.time_slide_id),
    ...
    def get_time_slide_id(
        self, offsetdict, create_new=None, superset_ok=False, nonunique_ok=False
    ):
        """
        ...
        if superset_ok:
            ids = [
                time_slide_id
                for time_slide_id, slide in self.as_dict().items()
                if offsetdict
                == dict(
                    (instrument, offset)
                    for instrument, offset in slide.items()
                    if instrument in offsetdict
                )
            ]
        else:
            ids = [
...
class CoincDefTable(ligolw.Table):
    ...
    def get_coinc_def_id(
        self, search, search_coinc_type, create_new=True, description=None
    ):
        """
...
class CoincMapTable(ligolw.Table):
    ...
    def applyKeyMapping(self, mapping):
        ...
        for i, (table_name, old_event_id, old_coinc_event_id) in enumerate(
            zip(table_column, event_id_column, coinc_event_id_column)
        ):
            try:
...
class DQSpec(ligolw.Table.RowType):
    ...
    def apply_to_segmentlist(self, seglist):
        ...
        for i, seg in enumerate(seglist):
            seglist[i] = seg.__class__(
                seg[0] - self.low_window, seg[1] + self.high_window
            )
...
class VetoDefTable(ligolw.Table):
    ...
    def versions(self, name, category):
        ...
        """
        return set(
            row.version for row in self if row.name == name and row.category == category
        )
...
def reset_next_ids(classes):
    warnings.warn(
        "igwn_ligolw.lsctables.reset_next_ids() is deprecated.  the replace with ligolw.Table.reset_next_ids()"
    )
...
def use_in(ContentHandler):
    warnings.warn(
        "igwn_ligolw.lsctables.use_in() is deprecated.  simply importing the lsctables module automatically enables the custom table support features it provides."
    )
