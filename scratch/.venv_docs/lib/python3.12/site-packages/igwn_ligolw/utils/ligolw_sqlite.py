...
implemented in the igwn_ligolw.dbtables module.  The code here wraps the
...
def insert_from_url(url, preserve_ids=False, verbose=False, contenthandler=None):
    ...
    printed to stderr.  See igwn_ligolw.dbtables.use_in() for more
    ...
    try:
        ...
        xmldoc = ligolw_utils.load_url(
            url, verbose=verbose, contenthandler=contenthandler
        )
...
def insert_from_xmldoc(connection, source_xmldoc, preserve_ids=False, verbose=False):
    ...
    try:
        ...
        for tbl in source_xmldoc.getElementsByTagName(ligolw.Table.tagName):
            ...
            dbtbl = xmldoc.childNodes[-1].appendChild(
                cls(tbl.attributes, connection=connection)
            )
