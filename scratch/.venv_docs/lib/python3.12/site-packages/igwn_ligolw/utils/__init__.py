...
__doctest_skip__ = ["write_filename"]
...
def sort_files_by_size(filenames, verbose=False, reverse=False):
    ...
    If a filename in the list is None (used by many igwn_ligolw based codes
    ...
    return sorted(
        filenames,
        key=(
            lambda filename: os.stat(filename)[stat.ST_SIZE]
...
def load_fileobj(
    fileobj, compress=None, xmldoc=None, contenthandler=ligolw.LIGOLWContentHandler
):
    """
    ...
    use when parsing the document.  See the igwn_ligolw package
    documentation for an explanation of a typical document parsing
    scenario and the content handler it uses.  See
    igwn_ligolw.ligolw.PartialLIGOLWContentHandler and
    igwn_ligolw.ligolw.FilteringLIGOLWContentHandler for examples of custom
    ...
    if compress == False:  # noqa: E712
        ...
        fileobj = gzip.GzipFile(
            mode="rb",
            fileobj=fileobj
            if isinstance(fileobj, RewindableInputFile)
            else RewindableInputFile(fileobj),
        )
    elif compress == "xz":
...
def load_url(url, verbose=False, **kwargs):
    ...
    try:
        ...
        with contextlib.closing(
            urllib.request.urlopen(url, **urlopen_kwargs)
        ) as fileobj:
            return load_fileobj(fileobj, **kwargs)
    # handle local files, and also None (stdin)
    return load_filename(filename, verbose=verbose, **kwargs)
...
def write_fileobj(xmldoc, fileobj, compress=None, compresslevel=3, **kwargs):
    ...
    with NoCloseFlushWrapper(fileobj) as fileobj:
        ...
        if compress == False:  # noqa: E712
            ...
            fileobj = gzip.GzipFile(
                mode="wb", fileobj=fileobj, compresslevel=compresslevel
            )
        elif compress == "xz":
...
def write_filename(
    ...
    compress=None,
    with_mv=True,
    trap_signals=SignalsTrap.default_signals,
    **kwargs,
):
    """
    Writes the LIGO Light Weight document tree rooted at xmldoc to the
    ...
    >>> # write file
    >>> write_filename(xmldoc, "demo.xml")
    >>> # write gzip-compressed file (auto-select format)
    >>> write_filename(xmldoc, "demo.xml.gz")
    >>> # force compression off
    >>> write_filename(xmldoc, "demo.xml.gz", compress = False)
    """
    #
    # select format
    #
