...
class LigolwSegmentList:
    ...
    def __init__(
        self, active=(), valid=(), instruments=(), name=None, version=None, comment=None
    ):
        """
    ...
    def isfalse(self):
        ...
        result = type(self)(self)
        result.active = ~result.active & result.valid
        result.valid = segments.segmentlist(
...
class LigolwSegments(set):
    ...
    def __init__(self, xmldoc, process=None):
        ...
        self.segment_def_table = lsctables.SegmentDefTable.ensure_exists(
            xmldoc, columns=LigolwSegmentList.segment_def_columns
        )
        self.segment_sum_table = lsctables.SegmentSumTable.ensure_exists(
            xmldoc, columns=LigolwSegmentList.segment_sum_columns
        )
        self.segment_table = lsctables.SegmentTable.ensure_exists(
            xmldoc, columns=LigolwSegmentList.segment_columns
        )
        ...
        for row in self.segment_sum_table:
            try:
                ...
                raise ValueError(
                    "invalid segment_def_id in segment_summary table: %s" % e
                )
    ...
    def insert_from_segwizard(
        self, fileobj, instruments, name, version=None, comment=None
    ):
        """
        ...
        ligolw_segment_list = LigolwSegmentList(
            active=active,
            valid=segments.segmentlist([active.extent()]),
    ...
    def insert_from_segmentlistdict(self, seglists, name, version=None, comment=None):
        ...
        for instrument, active in seglists.items():
            self.add(
                LigolwSegmentList(
                    active=active,
                    valid=segments.segmentlist([seglists.extent_all()]),
    ...
    def coalesce(self):
        ...
        def key(seglist):
            return (
                tuple(sorted(seglist.instruments)),
                seglist.name,
                seglist.version,
                seglist.comment,
            )
        ...
        for _, (result, *ligolw_segment_lists) in itertools.groupby(
            ligolw_segment_lists, key=key
        ):
    ...
    def optimize(self):
        ...
        for target, source in [
            (idx_a, idx_b)
            for (idx_a, seglist_a), (idx_b, seglist_b) in itertools.combinations(
                segment_lists.items(), 2
            )
            if seglist_a.valid == seglist_b.valid
            and seglist_a.active == seglist_b.active
            and seglist_a.name == seglist_b.name
            and seglist_a.version == seglist_b.version
            and seglist_a.comment == seglist_b.comment
        ]:
            try:
        ...
        for seglist in self:
            ...
            for instrument in seglist.instruments:
                if instrument in result:
                    raise ValueError(
                        "multiple '%s' segmentlists for instrument '%s'"
                        % (name, instrument)
                    )
    ...
    def finalize(self, process_row=None):
        ...
        row_generators = []
        for ligolw_segment_list in sorted(
            self, key=lambda el: (el.name, sorted(el.instruments), el.version)
        ):
            ...
            row_generators.append(
                row_generator(
                    ligolw_segment_list.valid,
                    self.segment_sum_table,
                    process_id,
                    segment_def_row.segment_def_id,
                )
            )
            row_generators.append(
                row_generator(
                    ligolw_segment_list.active,
                    self.segment_table,
                    process_id,
                    segment_def_row.segment_def_id,
                )
...
def segmenttable_get_by_name(xmldoc, name):
    ...
    instrument_index = dict(
        (row.segment_def_id, row.instruments) for row in def_table if row.name == name
    )
    ...
    instruments = set(
        instrument
        for instruments in instrument_index.values()
