...
def merge_ligolws(elem):
    ...
    """
    ligolws = [
        child for child in elem.childNodes if child.tagName == ligolw.LIGO_LW.tagName
    ]
    if ligolws:
        dest = ligolws.pop(0)
        for src in ligolws:
...
def compare_table_cols(a, b):
    ...
    """
    return {
        (col.Name, col.Type) for col in a.getElementsByTagName(ligolw.Column.tagName)
    } != {(col.Name, col.Type) for col in b.getElementsByTagName(ligolw.Column.tagName)}
        ...
        if tables:
            ...
            for src in tables:
                ...
                if compare_table_cols(dest, src):
                    ...
                    raise ValueError(
                        "document contains %s tables with incompatible columns"
                        % dest.Name
                    )
