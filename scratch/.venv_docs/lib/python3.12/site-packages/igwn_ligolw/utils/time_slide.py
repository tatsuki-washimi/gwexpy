...
def get_time_slide_id(
    xmldoc, time_slide, create_new=None, superset_ok=False, nonunique_ok=False
):
    """
    ...
    of the igwn_ligolw.lsctables.TimeSlideTable class.  See the
    ...
    return tisitable.get_time_slide_id(
        time_slide,
        create_new=create_new,
        superset_ok=superset_ok,
        nonunique_ok=nonunique_ok,
    )
...
def time_slides_vacuum(time_slides, verbose=False):
    ...
    TimeSlideTable class in igwn_ligolw.lsctables, construct and return a
    ...
    used via the applyKeyMapping() method of igwn_ligolw.table.Table
    ...
    >>> time_slides_vacuum(offsetvector(slides))
    {0: 1}
    ...
    time_slides = dict(
        (time_slide_id, offsetvect.deltas)
...
def time_slide_list_merge(slides1, slides2):
    ...
    return slides1 + [
        offsetvect2
        for offsetvect2 in slides2
        if frozenset(offsetvect2.deltas.items()) not in deltas1
    ]
