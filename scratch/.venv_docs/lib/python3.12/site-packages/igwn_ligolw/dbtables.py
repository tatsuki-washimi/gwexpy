...
class workingcopy:
    ...
    def __init__(
        self, filename, tmp_path=None, replace_file=False, discard=False, verbose=False
    ):
        """
        ...
        self.tmp_path = (
            tmp_path
            if tmp_path != "_CONDOR_SCRATCH_DIR"
            else os.getenv("_CONDOR_SCRATCH_DIR")
        )
    ...
    def cpy(srcname, dstname, attempts=5, verbose=False):
        ...
        for i in itertools.count(1):
            try:
                ...
                if verbose:
                    sys.stderr.write(
                        "warning: attempt %d: %s: \r" % (i, errno.errorcode[e.errno])
                    )
    ...
    def __enter__(self):
        ...
        if self.tmp_path is not None:
            ...
            self.temporary_file = tempfile.NamedTemporaryFile(
                suffix=".".join(os.path.split(self.filename)[-1].split(".")[1:]),
                dir=self.tmp_path,
            )
            ...
            if database_exists:
                ...
                if self.replace_file:
                    ...
                    self.cpy(self.filename, self.target, verbose=self.verbose)
                    == self.filename
                ):
        ...
        if self.target != self.filename:
            with ligolw_utils.SignalsTrap():
                if not self.discard:
                    ...
                    if self.verbose:
                        sys.stderr.write(
                            "moving '%s' to '%s' ... " % (self.target, self.filename)
                        )
    ...
    def set_temp_store_directory(self, connection):
        ...
        if self.verbose:
            sys.stderr.write(
                "setting the sqlite temp_store_directory to %s ... " % self.tmp_path
            )
...
class idmapper:
    ...
    def __init__(self, connection):
        ...
        try:
            self.cursor.execute(
                "CREATE TEMPORARY TABLE _idmap_ (table_name TEXT NOT NULL, old INTEGER NOT NULL, new INTEGER NOT NULL, PRIMARY KEY (table_name, old))"
            )
    ...
    def get_new(cursor, table_name, old, tbl):
        ...
        """
        cursor.execute(
            "SELECT new FROM _idmap_ WHERE table_name == ? AND old == ?",
            (table_name, old),
        )
        new = cursor.fetchone()
        if new is not None:
...
_sql_create_table_pattern = re.compile(
    r"CREATE\s+TABLE\s+(?P<name>\w+)\s*\((?P<coldefs>.*)\)", re.IGNORECASE
)
...
def get_xml(connection, table_names=None):
    ...
    for table_name in table_names:
        ...
        try:
            ...
            cls = DBTable
        table_elem = cls(
            AttributesImpl({"Name": "%s:table" % table_name}), connection=connection
        )
        destrip = {}
        ...
        for column_name, column_type in table_elem.get_column_info():
            ...
            table_elem.appendChild(
                ligolw.Column(
                    AttributesImpl({"Name": column_name, "Type": column_type})
                )
            )
                ...
                AttributesImpl(
                    {
                        ...
                        "Delimiter": table_elem.Stream.Delimiter.default,
                        "Type": table_elem.Stream.Type.default,
                    }
                )
            )
        )
...
class DBTable(ligolw.Table):
    ...
    >>> tbl = DBTable(AttributesImpl({"Name": "process:table"}), connection = connection)
    ...
    If a custom igwn_ligolw.Table subclass is defined in igwn_ligolw.lsctables
    ...
    def __new__(cls, *args, **kwargs):
        ...
        if name in cls.TableByName:
            ...
            new = super(ligolw.Table, ligolw.Table).__new__(
                cls.TableByName[name], *args
            )
    ...
    def copy(self, *args, **kwargs):
        """
        This method is not implemented.  See igwn_ligolw.Table for more
        information.
        """
    ...
    def _end_of_columns(self):
        ...
        if self.loadcolumns is not None:
            self.dbcolumnnames = [
                name for name in self.columnnames if name in self.loadcolumns
            ]
            self.dbcolumntypes = [
                name
                for i, name in enumerate(self.columntypes)
                if self.columnnames[i] in self.loadcolumns
            ]
        else:
    ...
    def get_column_info(self):
        ...
        return [
            (coldef.groupdict()["name"], coldef.groupdict()["type"])
            for coldef in re.finditer(_sql_coldef_pattern, coldefs)
            if coldef.groupdict()["name"].upper() not in ("PRIMARY", "UNIQUE", "CHECK")
        ]

    def sync_next_id(self):
        if self.next_id is not None:
            maxid = self.cursor.execute(
                "SELECT MAX(%s) FROM %s" % (self.next_id.column_name, self.Name)
            ).fetchone()[0]
    ...
    def _remapping_append(self, row):
        ...
        if self.next_id is not None:
            ...
            setattr(
                ...
                idmapper.get_new(
                    self.cursor, self.Name, getattr(row, self.next_id.column_name), self
                ),
            )
        self._append(row)
    ...
    def applyKeyMapping(self):
        ...
        for colname in self.dbcolumnnames:
            ...
            if self.next_id is not None and colname == self.next_id.column_name:
                continue
            assignments.append(
                "%s = (SELECT new FROM _idmap_ WHERE _idmap_.table_name == '%s' AND _idmap_.old == %s)"
                % (colname, table_name, colname)
            )
...
class CoincMapTable(DBTable):
    ...
    def applyKeyMapping(self):
        if self.remap_first_rowid is not None:
            self.cursor.execute(
                "UPDATE coinc_event_map SET event_id = (SELECT new FROM _idmap_ WHERE _idmap_.table_name == coinc_event_map.table_name AND old == event_id), coinc_event_id = (SELECT new FROM _idmap_ WHERE _idmap_.table_name == 'coinc_event' AND old == coinc_event_id) WHERE ROWID >= ?",
                (self.remap_first_rowid,),
            )
            self.remap_first_rowid = None
        ...
        return dict(
            (
                ...
                offsetvector.offsetvector(
                    (instrument, offset) for time_slide_id, instrument, offset in values
                ),
            )
            for time_slide_id, values in itertools.groupby(
                self.cursor.execute(
                    "SELECT time_slide_id, instrument, offset FROM time_slide ORDER BY time_slide_id"
                ),
                lambda time_slide_id_instrument_offset: time_slide_id_instrument_offset[
                    0
                ],
            )
        )
...
DBTable.TableByName.update(
    {CoincMapTable.tableName: CoincMapTable, TimeSlideTable.tableName: TimeSlideTable}
)
...
def use_in(ContentHandler):
    ...
    igwn_ligolw.ligolw.LIGOLWContentHandler, to cause it to use the DBTable
