    ...
    **match
    ...
    for urlmeta in urllist:
        try:
            ...
            raise TypeError(f"unrecognised match parameter: {str(exc)}")
...
def match(
    ...
    urls = sorted(
        urls,
        key=lambda u: splitext(basename(u))[0].split("-")[::-1],
    )
    ...
    if len(matched_tags) > 1:
        ...
        raise ValueError(
            "multiple GWOSC URL tags discovered in dataset, "
            f"please select one of: {tags}"
        )
