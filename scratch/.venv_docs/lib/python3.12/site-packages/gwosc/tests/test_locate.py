...
def test_get_urls():
    ...
    for url in urls:
        assert os.path.basename(url).startswith(f"{detector[0]}-{detector}")
        assert utils.segments_overlap(utils.url_segment(url), span)
...
def test_get_urls_gw170104():
    ...
    urls = locate.get_urls("L1", 1167558912.6, 1167559012.6, version=2)
