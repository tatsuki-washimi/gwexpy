...
@mock.patch("gwosc.api.v1.fetch_json")
def test_fetch_dataset_json_local(fetch):
    start = 934000000
    end = 934100000
    api.fetch_dataset_json(start, end)
    fetch.assert_called_with(losc_url(f"archive/{start}/{end}/json/"))
...
def test_fetch_run_json_local(fetch):
    ...
    fetch.assert_called_with(losc_url("archive/links/S6/L1/934000000/934100000/json/"))
...
def test_fetch_cataloglist_json_local(fetch):
    ...
    fetch.assert_called_with(losc_url("eventapi/json/"))
...
def test_fetch_catalog_json_local(fetch):
    ...
    fetch.assert_called_with(losc_url("eventapi/json/GWTC-1-confident/"))


@pytest.mark.remote
def test_fetch_filtered_events_json():
    out = api.fetch_filtered_events_json(select=["10 <= luminosity-distance <= 200"])
    events = out["events"]
    assert "GW190425-v1" in events


@mock.patch(
    "gwosc.api.v1.fetch_allowed_params_json",
