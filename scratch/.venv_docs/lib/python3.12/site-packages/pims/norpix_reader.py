...
class NorpixSeq(FramesSequence):
    ...
    def __init__(self, filename, as_raw=False):
        ...
        if self.header_dict['magic'] != 0xFEED:
            raise OSError('The format of this .seq file is unrecognized')
        if self.header_dict['compression_format'] != 0:
            raise OSError('Only uncompressed images are supported in .seq files')
        if self.header_dict['image_format'] != 100 and not as_raw:
            raise OSError('Non-monochrome images are only supported as_raw in .seq files')
        ...
        if as_raw:
            ...
            try:
                ...
                raise OSError(self._dtype + " pixels not supported; use as_raw and convert")
...
Frame Shape: {self.frame_shape[1]}w x {self.frame_shape[0]}h
