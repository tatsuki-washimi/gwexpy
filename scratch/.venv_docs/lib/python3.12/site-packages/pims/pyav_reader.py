...
class PyAVReaderTimed(FramesSequence):
    ...
    @classmethod
    def class_exts(cls):
        return {'mov', 'avi', 'mp4'} | super().class_exts()
    ...
    def __init__(self, file, cache_size=16, fast_forward_thresh=32,
        ...
        if len(self._container.streams.video) == 0:
            raise OSError(f"No valid video stream found in {file}")
        ...
        if self.duration <= 0 or len(self) <= 0:
            raise OSError(f"Video stream {stream_index} in {file} has zero length.")
...
Frame rate: {self.frame_rate:.3f} fps
