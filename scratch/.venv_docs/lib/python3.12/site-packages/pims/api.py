...
def not_available(requirement):
    def raiser(*args, **kwargs):
        raise ImportError(
            f"This reader requires {requirement}.")
    return raiser
...
def open(sequence, **kwargs):
    ...
    if len(eligible_handlers) < 1:
        raise UnknownFormatError(
            f"Could not autodetect how to load a file of type {ext}. "
            "Try manually "
            f"specifying a loader class, e.g. Video({sequence})")
    ...
    for handler in sort_on_priority(eligible_handlers):
        try:
            ...
            messages.append(f'{str(handler)} errored: {str(e)}')
    raise UnknownFormatError(
