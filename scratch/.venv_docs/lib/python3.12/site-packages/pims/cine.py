...
SETUP_FIELDS = [
    ...
    ] + [(f'bin_name{i:d}', '11s') for i in range(8)] + [
    ('ana_option', UINT16),
    ('ana_channels', INT16),
    ('res_6', UINT8),
    ('ana_board', UINT8),
    ] + [(f'ch_option{i:d}', INT16) for i in range(8)] + [
    ] + [(f'ana_gain{i:d}', FLOAT) for i in range(8)] + [
    ] + [(f'ana_unit{i:d}', '6s') for i in range(8)] + [
    ] + [(f'ana_name{i:d}', '11s') for i in range(8)] + [
    ('i_first_image', INT32),
    ('dw_image_count', UINT32),
    ('n_q_factor', INT16),
    ('w_cine_file_type', UINT16),
    ] + [(f'sz_cine_path{i:d}', '65s') for i in range(4)] + [
    ('b_mains_freq', UINT16),
    ('b_time_code', UINT8),
    ('b_priority', UINT8),
    ...
    ('sound_dest', UINT32),
    ('frp_steps', UINT32),
    ] + [(f'frp_img_nr{i:d}', INT32) for i in range(16)] + [
    ] + [(f'frp_rate{i:d}', UINT32) for i in range(16)] + [
    ] + [(f'frp_exp{i:d}', UINT32) for i in range(16)] + [
    ('mc_cnt', INT32),
    ] + [(f'mc_percent{i:d}', FLOAT) for i in range(64)] + [
    ('ci_calib', UINT32),
    ('calib_width', UINT32),
    ('calib_height', UINT32),
    ('calib_rate', UINT32),
    ...
    ('calib_edr', UINT32),
    ('calib_temp', UINT32),
    ] + [(f'header_serial{i:d}', UINT32) for i in range(4)] + [
    ('range_code', UINT32),
    ('range_size', UINT32),
    ('decimation', UINT32),
...
Frame Shape: {self.frame_shape!r}
Pixel Datatype: {self.pixel_type}"""
