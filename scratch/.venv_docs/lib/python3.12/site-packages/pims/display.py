...
def export_pyav(sequence, filename, rate=30, bitrate=None,
    ...
    if quality is not None:
        if codec == 'libx264':
            ...
            options['qmin'] = str(quality)
            options['qmax'] = str(quality)
        else:
    ...
    output = av.open(str(filename), 'w', format=format)
    try:
        ...
        output = av.open(
            str(filename), 'w', format=format, options=options
        )
...
def export_moviepy(sequence, filename, rate=30, bitrate=None, width=None,
    ...
    ffmpeg_params = []
    for key in options:
        ffmpeg_params.extend([f'-{key}', str(options[key])])
...
<source alt="test" src="data:video/{mimetype};base64,{video_encoded}" type="video/webm">
Use Google Chrome browser.</video>"""
