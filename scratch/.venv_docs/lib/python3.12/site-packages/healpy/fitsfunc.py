...
from .utils.deprecation import deprecated, deprecated_renamed_argument
...
def read_map(
    ...
    for ff, curr_dtype in zip(field, dtype):
        ...
        if not pixelfunc.isnpixok(m.size) or (sz > 0 and sz != m.size):
            log.warning(f"nside={nside:d}, sz={sz:d}, m.size={m.size:d}")
            raise ValueError("Wrong nside parameter.")
    ...
    for unit in np.atleast_1d(hdu):
        ...
        if lmaxtot is None:
            ...
            if lmaxtot != lmax or mmaxtot != mmax:
                raise RuntimeError(
                    f"read_alm: harmonic expansion order in {filename} HDUs {unit} does not "
                    "match"
                )
        alm = almr * (0 + 0j)
...
def getformat(t):
    ...
    raise ValueError(
        f"healpy could not understand the equivalent FITS datatype of {str(t)}, please open an issue on the healpy Github repository"
    )
