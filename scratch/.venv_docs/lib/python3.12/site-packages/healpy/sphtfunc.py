...
from .pixelfunc import UNSEEN, accept_ma, ma_to_array, maptype
        ...
        if datapath is not None:
            ...
            if os.path.exists(pixel_weights_filename):
                ...
                raise RuntimeError(
                    ...
                    f"is missing at {pixel_weights_filename}"
                )
...
class Alm:
    ...
    def getlm(lmax, i=None):
        ...
        assert (
            np.max(i) < szalm
        ), f"Invalid index, it should less than the max alm array length of {szalm}"
...
def check_max_nside(nside):
    ...
    if nside > MAX_NSIDE:
        raise ValueError(
            f"nside {nside} of map cannot be larger than "
            f"MAX_NSIDE {MAX_NSIDE}"
        )
