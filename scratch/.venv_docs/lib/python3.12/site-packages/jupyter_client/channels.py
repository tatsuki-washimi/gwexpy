...
class HBChannel(Thread):
    ...
    def __init__(
        ...
        address: t.Union[tuple[str, int], str] = "",
    ) -> None:
...
class ZMQSocketChannel:
    ...
    def _recv(self, **kwargs: t.Any) -> dict[str, t.Any]:
        assert self.socket is not None
        msg = self.socket.recv_multipart(**kwargs)
        _ident, smsg = self.session.feed_identities(msg)
        return self.session.deserialize(smsg)

    def get_msg(self, timeout: float | None = None) -> dict[str, t.Any]:
...
class AsyncZMQSocketChannel(ZMQSocketChannel):
    ...
    async def _recv(self, **kwargs: t.Any) -> dict[str, t.Any]:  # type:ignore[override]
        assert self.socket is not None
    ...
    async def get_msgs(self) -> list[dict[str, t.Any]]:  # type:ignore[override]
        """Get all messages that are currently ready."""
        msgs = []
        while True:
            try:
