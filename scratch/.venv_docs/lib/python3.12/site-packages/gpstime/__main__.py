...
from . import ISO_FORMAT, GPSTimeParseAction, __version__, gpstime
...
def main():
    ...
    if not args.tz:
        ...
        utz = tzutc()
        if tz_valid:
            print(f'{tzname(ltz)}: {gt.astimezone(ltz).strftime(args.format)}')
        ...
        if args.tz == 'local':
            ...
            tz = tzutc()
        print(f'{gt.astimezone(tz).strftime(args.format)}')
