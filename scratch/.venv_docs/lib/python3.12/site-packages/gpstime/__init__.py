...
def _date_parse(string="now"):
    ...
    try:
        ...
        try:
            ...
            raise GPSTimeException(f"could not parse string '{string}'") from None
    return ts
...
class gpstime(datetime):
    ...
    def parse(cls, string='now'):
        ...
        try:
            ...
            raise TypeError(f"Time specification must be a string, not {type(string)!s}")
        if gps is not None:
            return cls.fromgps(gps)
        try:
    ...
    def __call__(self, parser, namespace, values, option_string=False):
        ...
        if hasattr(namespace, "nano") and namespace.nano:
            ...
            try:
                ...
                parser.error(f"Could not parse date/time string '{values}'")
