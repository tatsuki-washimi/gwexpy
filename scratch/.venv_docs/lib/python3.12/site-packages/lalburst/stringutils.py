...
class InstrumentBins(rate.HashableBins):
	...
	def __init__(self, names):
		super().__init__(frozenset(combo) for n in range(len(names) + 1) for combo in itertools.combinations(names, n))
...
class StringCoincParamsDistributions(snglcoinc.LnLikelihoodRatioMixin):
	ligo_lw_name_suffix = "stringcusp_coincparamsdistributions"
...
def load_likelihood_data(filenames, verbose = False):
	...
	for n, filename in enumerate(filenames, 1):
		...
		this_coinc_params = StringCoincParamsDistributions.from_xml(xmldoc, "string_cusp_likelihood")
		this_seglists = lsctables.SearchSummaryTable.get_table(xmldoc).get_out_segmentlistdict(lsctables.ProcessTable.get_table(xmldoc).get_ids_by_program("lalapps_string_meas_likelihood")).coalesce()
		xmldoc.unlink()
