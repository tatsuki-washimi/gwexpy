...
from .plot import plot_budget, plot_noise, plot_trace
...
def load_budget(name_or_path, freq=None, bname=None):
    ...
    if os.path.exists(name_or_path):
        ...
        if ext in Struct.STRUCT_EXT:
            logger.info(f"loading struct {path}...")
            ...
            raise RuntimeError(
                f"Unknown file type: {ext} (supported types: {Struct.STRUCT_EXT}).")

    else:
        if name_or_path not in IFOS:
            raise RuntimeError(f"Unknown IFO '{name_or_path}' (available IFOs: {IFOS}).")
...
def gwinc(freq, ifo, source=None, plot=False, PRfixed=True):
    ...
    if ifo.Laser.Power * prfactor != pbs:
        logger.warning(f"Thermal lensing limits input power to {pbs/prfactor} W")
