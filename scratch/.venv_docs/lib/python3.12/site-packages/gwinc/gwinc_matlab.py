...
from . import const, logger, suspension
...
class Matlab:
    def __init__(self, gwincpath=None):
        ...
        if not os.path.exists(os.path.join(gwincpath, 'gwinc.m')):
            raise OSError(f"Invalid MATLAB GWINC path: '{gwincpath}'")

        logger.info("starting MATLAB engine...")
        MATLAB_ENGINE = matlab.engine.start_matlab()

        logger.info(f"gwinc path: {gwincpath}")
    ...
    def load_array(self, var, array):
        ...
        MATLAB_ENGINE.eval(f'{var} = cell2mat({var});', nargout=0)
    ...
    def load_struct(self, var, struct):
        ...
        with tempfile.NamedTemporaryFile(suffix='.mat') as f:
            ...
            MATLAB_ENGINE.eval(f"{var} = load('{f.name}');", nargout=0)
...
def gwinc_matlab(f, ifoin, plot=False):
    ...
    cmd = f"[score, noises, ifo] = gwinc(f, [], ifo, SourceModel, {plot_flag});"
