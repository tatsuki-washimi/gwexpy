...
def _precomp_recurse_mapping(func, freq, ifo, _precomp):
    ...
    for pc_func in itertools.chain(
        ...
        logger.debug(f"precomp {pc_func}")
...
class Calibration(BudgetItem):
    ...
    def __call__(self, data):
        ...
        assert data.shape == cal.shape, \
            f"data shape does not match calibration ({data.shape} != {cal.shape})"
        return data * cal
    ...
    def __getitem__(self, name):
        ...
        try:
            ...
            try:
                ...
                raise KeyError(f"unknown noise or cal name '{name}")
        if rest:
            return o[rest]
        else:
            return o
