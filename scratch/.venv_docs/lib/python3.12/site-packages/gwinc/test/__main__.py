...
def prune_cache_dir():
    ...
    for path in expired_paths:
        logging.info(f"pruning old cache: {path}")
        shutil.rmtree(path)
...
def gen_cache(git_hash, path):
    ...
    """
    logging.info(f"creating new cache for hash {git_hash}...")
    subprocess.run(
...
def load_cache(path):
    ...
    """
    logging.info(f"loading cache {path}...")
    cache = {}
    ...
    if os.path.exists(git_hash_path):
        ...
        git_hash = None
    logging.debug(f"cache hash: {git_hash}")
...
def zip_noises(traceA, traceB, skip):
    ...
    for name, tA in traceA.walk():
        if skip and name in skip:
            logging.warning(f"SKIPPING TEST: '{name}'")
            continue
        try:
            tB = B_dict[name]
        except (KeyError, TypeError):
            logging.warning(f"MISSING B TRACE: '{name}'")
            continue
        yield name, tA, tB
...
def compare_traces(traceA, traceB, tolerance=TOLERANCE, skip=None):
    ...
    for name, tA, tB in zip_noises(traceA, traceB, skip):
        logging.debug(f"comparing {name}...")
        ampA = tA.asd
        ampB = tB.asd
        diff = ampB - ampA
        frac = abs(diff / ampA)
...
def plot_diffs(freq, diffs, styleA, styleB):
    ...
    for i, nname in enumerate(diffs):
        ...
        axr.text(max(freq)+4000, max(frac), f'{max(frac)*1e6:.1f} ppm',
...
gwinc/test/cache/<SHA1>.  Old caches are automatically pruned.""",
    ...
    parser.add_argument(
        ...
        help=f"fractional tolerance of comparison [{TOLERANCE}]")
    parser.add_argument(
    ...
    if args.git_rev:
        ...
        git_rev = f'{remote}/master'
    logging.info(f"git  rev: {git_rev}")
...
""".format(
        ...
        if not diffs:
            logging.info(f"{name} tests pass.")
            continue

        logging.warning(f"{name} tests FAIL")
        fail |= True
...
{}'''.format(name, style_ref['label'], style_cur['label'],
    ...
    if args.report:
        logging.info(f"generating report {args.report}...")
        pdf_writer = PdfFileWriter()
        for name in ifos:
            path = os.path.join(outdir.name, f'{name}.pdf')
