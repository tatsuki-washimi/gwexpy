...
class Struct:
    ...
    def __str__(self):
        return f'<GWINC Struct: {list(self.__dict__.keys())}>'
        ...
        for k, v in sorted(self.walk()):
            if isinstance(v, (int, float, complex)):
                ...
                v = np.array2string(
                    ...
                    formatter={'all': lambda x: f"{x:0.6e} "}
                )
                base = 's'
            else:
                base = 's'
            txt.write('{key}{delimiter}{value:{base}}{end}\n'.format(
    ...
    def from_file(cls, path, _pass_inherit=False):
        ...
        if ext == '.m':
            ...
            matlab.eval(f"ifo = {func_name};", nargout=0)
            ...
            with open(path) as f:
                if ext in ['.yaml', '.yml']:
                    ...
                    raise OSError(f"Unknown file type: {ext}")
