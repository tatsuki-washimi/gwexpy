...
""".format(', '.join([f"{ifo}" for ifo in IFOS]))
...
def main():
    ...
    if args.range:
        ...
        range_func = 'range'
        subtitle = r'inspiral {func} {m1}/{m2} $\mathrm{{M}}_\odot$: {fom:.0f} {unit}'.format(
            func=range_func,
            m1=H.params['m1'],
            m2=H.params['m2'],
    ...
    if args.title:
        ...
        plot_style['title'] = f"GWINC Noise Budget: {name}"
...
"""
    ...
    if out_data_files:
        for path in out_data_files:
            logger.info(f"saving budget trace: {path}")
            io.save_hdf5(
                trace=trace,
                path=path,
                ifo=ifo,
    ...
    if args.plot or out_plot_files:
        ...
        if out_plot_files:
            for path in out_plot_files:
                logger.info(f"saving budget plot: {path}")
                try:
                    fig.savefig(path)
