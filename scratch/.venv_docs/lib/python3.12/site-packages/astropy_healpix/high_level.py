...
def _get_frame(frame):
    ...
    if frame is None or isinstance(frame, BaseCoordinateFrame):
        ...
        if frame_cls is None:
            ...
            raise ValueError(f'Coordinate frame name "{frame}" is not a known '
        ...
        raise ValueError("Coordinate frame must be a frame name, frame "
                         f"instance, frame class, or None, not a '{frame.__class__.__name__}'"
                         )
...
class HEALPix:
    ...
    def interpolate_bilinear_lonlat(self, lon, lat, values):
        ...
        if len(values) != self.npix:
            raise ValueError(f'values must be an array of length {self.npix} (got {len(values)})'
                             )
