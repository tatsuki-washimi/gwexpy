...
def evolve_spins_dt(v0, m1, m2, chi1x, chi1y, chi1z, chi2x, chi2y, chi2z, Lnx, Lny, Lnz, v_final, dt):
        ...
        if abs(v_last-v_final) > delta_vf_max:
            if dvdt_current <= 0 or dvdt_current > dvdt_max:
                ...
                print(f"Warning: Integration stopped at v_max = {v_last}, more than {delta_vf_max} different from v_final = {v_final}, because dv/dt became negative or exceeded {dvdt_max}, with a value of {dvdt_current}. The final value used is {precession_eqns(t,Y[-1],m1,m2)[V_POS]}.")
            else:
                raise ValueError(f"v_max = {v_v[-1]} is more than {delta_vf} different from v_final = {v_final} and dv/dt is positive and less than {dvdt_max}")

        if abs(Lnorm_current - 1.) >= delta_Lnorm_max:
            raise ValueError(f"norm of Ln is more than {delta_Lnorm_max:e} different from 1, with distance {abs(Lnorm_current - 1.):e}")
