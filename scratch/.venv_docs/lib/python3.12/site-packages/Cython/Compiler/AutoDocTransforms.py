...
from . import Options, PyrexTypes
...
class AnnotationWriter(ExpressionWriter):
    ...
    def visit_Node(self, node):
        ...
        if self.description:
            warning(node.pos,
                    f"Failed to convert code to string representation in {self.description}", level=1)
    ...
    def visit_LambdaNode(self, node):
        ...
        if self.description:
            warning(node.pos,
                    f"Failed to convert lambda to string representation in {self.description}", level=1)
