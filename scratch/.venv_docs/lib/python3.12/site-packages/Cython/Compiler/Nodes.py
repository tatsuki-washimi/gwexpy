...
from . import Builtin, DebugFlags, Future, Naming, Options, PyrexTypes, TypeSlots
from .Code import UtilityCode
from .Errors import CannotSpecialize, CompileError, InternalError, error, warning
from .PyrexTypes import error_type, py_object_type
from .Pythran import has_np_pythran, is_pythran_buffer, pythran_type
        ...
        def put_into_closure(entry):
            if entry.in_closure:
                if entry.type.is_array:
                    ...
                    code.putln(f"memcpy({entry.cname}, {entry.original_cname}, sizeof({entry.cname}));")
                else:
                    code.putln('%s = %s;' % (entry.cname, entry.original_cname))
                if entry.type.is_memoryviewslice:
...
class PyClassDefNode(ClassDefNode):
    ...
    def analyse_expressions(self, env):
        if self.bases and not (self.bases.is_sequence_constructor and len(self.bases.args) == 0):
            from .ExprNodes import CloneNode, PythonCapiCallNode
        ...
        if not from_range and self.py_loopvar_node:
            ...
            if self.target.entry.is_pyglobal:
                ...
                if self.target.entry.scope.is_module_scope:
                    ...
                    lookup_func = f'__Pyx_GetNameInClass(%s, {self.target.entry.scope.namespace_cname}, %s); %s'
...
class CriticalSectionStatNode(TryFinallyStatNode):
    ...
    def check_for_yields(self):
        ...
        if collector.yields:
            error(self.pos, "Cannot yield while in a cython.critical_section.")
