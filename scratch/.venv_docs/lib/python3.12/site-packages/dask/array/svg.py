...
def svg_2d(chunks, offset=(0, 0), skew=(0, 0), size=200, sizes=None):
    ...
    header = f'<svg width="{int(max_x + 50)}" height="{int(max_y + 50)}" style="stroke:rgb(0,0,0);stroke-width:1" >\n'
    footer = "\n</svg>"
...
def svg_3d(chunks, size=200, sizes=None, offset=(0, 0)):
    ...
    header = f'<svg width="{int(max_z + 50)}" height="{int(max_y + 50)}" style="stroke:rgb(0,0,0);stroke-width:1" >\n'
    footer = "\n</svg>"
...
def svg_nd(chunks, size=200):
    ...
    header = f'<svg width="{int(left)}" height="{int(total_height)}" style="stroke:rgb(0,0,0);stroke-width:1" >\n'
    footer = "\n</svg>"
    return header + "\n\n".join(out) + footer
...
def svg_lines(x1, y1, x2, y2, max_n=20):
    ...
    lines = [
        f'  <line x1="{int(x1[i])}" y1="{int(y1[i])}" x2="{int(x2[i])}" y2="{int(y2[i])}" />'
        for i in indices
    ]
