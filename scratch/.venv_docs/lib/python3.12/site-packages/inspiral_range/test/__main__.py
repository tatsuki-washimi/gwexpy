...
def git_extract_hash(git_hash, path):
    ...
    hash_path = os.path.join(path, f'.hash-{git_hash}')
    if os.path.exists(hash_path):
        return
    if os.path.exists(path):
        shutil.rmtree(path)
    logger.info(f"extracting code from git hash {git_hash}...")
    os.makedirs(path)
    try:
        subprocess.check_call(
            f'git archive {git_hash} | tar -x -C {path}',
            shell=True)
...
""",
        epilog=f"""Default IFO list: {list(IFOS.keys())}
""",
