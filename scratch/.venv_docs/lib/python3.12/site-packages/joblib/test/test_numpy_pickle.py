...
def test_compress_level_error(wrong_compress):
    ...
    exception_msg = f'Non valid compress level given: "{wrong_compress}"'
    ...
    if pickle_reading_protocol >= pickle_writing_protocol:
        try:
            ...
            for w in deprecation_warnings:
                assert (
                    str(w.message)
                    == f"The file '{filename}' has been generated with a joblib "
        ...
        try:
            ...
            message = f"unsupported pickle protocol: {pickle_writing_protocol}"
...
@parametrize(
    ...
    [
        ...
        # More bytes
        10000 * f"{random.randint(0, 1000) * 1000}".encode("latin-1"),
    ],
...
def test_binary_zlibfile_bad_compression_levels(tmpdir, bad_value):
    ...
    pattern = re.escape(
        ...
        f"You provided 'compresslevel={bad_value}'"
    )
...
def test_register_compressor_already_registered():
    ...
    excinfo.match(f"Compressor '{compressor_name}' already registered.")
