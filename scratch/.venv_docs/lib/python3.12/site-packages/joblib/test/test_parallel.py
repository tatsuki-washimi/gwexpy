...
class MyBackend(joblib._parallel_backends.LokyBackend):
    ...
    def get_nested_backend(
        ...
        return super().get_nested_backend()[0]
...
INTERACTIVE_DEFINED_FUNCTION_AND_CLASS_SCRIPT_CONTENT = f"""\
import sys
import faulthandler
# Make sure that joblib is importable in the subprocess launching this
# script. This is needed in case we run the tests from the joblib root
# folder without having installed joblib
sys.path.insert(0, {os.path.dirname(os.path.dirname(joblib.__file__))!r})
...
INTERACTIVELY_DEFINED_SUBCLASS_WITH_METHOD_SCRIPT_CONTENT = f"""\
import sys
# Make sure that joblib is importable in the subprocess launching this
# script. This is needed in case we run the tests from the joblib root
# folder without having installed joblib
...
def _parent_max_num_threads_for(child_module, parent_info):
    ...
    raise ValueError(
        f"An unexpected module was loaded in child:\n{child_module}"
    )
