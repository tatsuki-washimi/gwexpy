...
def test_resource_tracker_retries_when_permissionerror(tmpdir):
    ...
    assert out == b""
    msg = f"tried to unlink {filename}, got PermissionError"
...
def test_permission_error_windows_memmap_sent_to_parent(backend):
    ...
    cmd = f"""if 1:
        ...
        if __name__ == '__main__':
            ...
            _ = Parallel(n_jobs=2, verbose=5, backend='{backend}')(
                delayed(id)(i) for i in range(20))

            time.sleep(0.5)

            slice_of_data = Parallel(n_jobs=2, verbose=5, backend='{backend}')(
        ...
        if __name__ == '__main__':
            for i in range(5):
                slice_of_data = Parallel(
                    n_jobs=2, max_nbytes=1, backend='{backend}')(
    ...
    try:
        ...
        for i in range(10):
            ...
            raise AssertionError(
                f"temporary folder {p._temp_folder} was not deleted"
            )
        del p
...
def test_child_raises_parent_exits_cleanly(backend):
    ...
    cmd = f"""if 1:
        ...
        if __name__ == "__main__":
            try:
                with Parallel(n_jobs=2, backend="{backend}", max_nbytes=100) as p:
                    temp_folder = get_temp_folder(p, "{backend}")
