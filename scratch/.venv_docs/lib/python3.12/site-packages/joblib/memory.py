...
def register_store_backend(backend_name, backend):
    ...
    if not isinstance(backend_name, str):
        raise ValueError(
            f"Store backend name should be a string, '{backend_name}' given."
        )
    if backend is None or not issubclass(backend, StoreBackendBase):
        raise ValueError(
            f"Store backend should inherit StoreBackendBase, '{backend}' given."
        )
...
def _store_backend_factory(backend, location, verbose=0, backend_options=None):
    ...
    if isinstance(location, StoreBackendBase):
        ...
        if obj is None:
            raise TypeError(
                f"Unknown location {location} or backend {backend}"
            )
        ...
        warnings.warn(
            f"Instantiating a backend using a {location.__class__.__name__} as a location is not "
            "supported by joblib. Returning None instead.",
            UserWarning,
        )
    ...
    def get(self):
        ...
        try:
            ...
            new_exc = KeyError(
                ...
                f"It seems that this folder is corrupted : {os.path.join(self.store_backend.location, *self._call_id)}"
            )
...
class MemorizedFunc(Logger):
    ...
    def _cached_call(self, args, kwargs, shelving):
        ...
        if self._is_in_cache_and_valid(call_id):
            ...
            try:
                ...
                _, signature = format_signature(self.func, *args, **kwargs)
                self.warn(
                    f"Exception while loading results for {signature}\n {traceback.format_exc()}"
                )
    ...
    def _check_previous_func_code(self, stacklevel=2):
        ...
        except OSError:  # some backend can also raise OSError
        ...
        if old_first_line == first_line == -1 or func_name == "<lambda>":
            if not first_line == -1:
                func_description = f"{func_name} ({source_file}:{first_line})"
            else:
                func_description = func_name
            warnings.warn(
                JobLibCollisionWarning(
                    f"Cannot detect name collisions for function '{func_description}'"
                ),
        ...
        if self._verbose > 10:
            _, func_name = get_func_name(self.func, resolv_alias=False)
            self.warn(
                f"Function {func_name} (identified by {self.func_id}) has changed."
            )
    ...
    def __repr__(self):
        return f"{self.__class__.__name__}(func={self.func}, location={self.store_backend.location})"
...
class Memory(Logger):
    ...
    def __repr__(self):
        return f"{self.__class__.__name__}(location={None if self.store_backend is None else self.store_backend.location})"
