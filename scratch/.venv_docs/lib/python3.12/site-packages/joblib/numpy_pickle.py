...
def dump(value, filename, compress=0, protocol=None):
    ...
    if compress is True:
        ...
        if len(compress) != 2:
            raise ValueError(
                ...
                f"(compress method, compress level), you passed {compress}"
            )
    ...
    if (
        ...
        raise ValueError(
            f'Non valid compress level given: "{compress_level}". Possible values are {list(range(10))}.'
        )

    if compress_method not in _COMPRESSORS:
        # Raising an error if an unsupported compression method is given.
        raise ValueError(
            f'Non valid compression method given: "{compress_method}". Possible values are {_COMPRESSORS}.'
        )
