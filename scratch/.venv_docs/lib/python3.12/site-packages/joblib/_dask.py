...
class DaskDistributedBackend(AutoBatchingMixin, ParallelBackendBase):
    ...
    def effective_n_jobs(self, n_jobs):
        ...
        try:
            ...
            error_msg = (
                f"DaskDistributedBackend has no worker after {self.wait_for_workers_timeout} seconds. "
                "Make sure that workers are started and can properly connect "
                "to the scheduler and increase the joblib/dask connection "
                "timeout with:\n\n"
                f"parallel_config(backend='dask', wait_for_workers_timeout={max(10, 2 * self.wait_for_workers_timeout)})"
            )
