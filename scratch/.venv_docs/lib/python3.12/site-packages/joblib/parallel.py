        ...
        **backend_kwargs,
        ...
        if not isinstance(backend, SequentialBackend):
            if self.return_generator and not backend.supports_return_generator:
                raise ValueError(
                    f"Backend {backend} does not support return_as={return_as}"
                )
    ...
    def _initialize_backend(self):
        ...
        try:
            ...
            if self.timeout is not None and not self._backend.supports_timeout:
                warnings.warn(
                    f"The backend class {self._backend.__class__.__name__!r} does not support timeout. "
                    f"You have set 'timeout={self.timeout}' in Parallel but "
                    "the 'timeout' parameter will not be used."
                )
