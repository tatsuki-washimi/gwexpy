...
def _log_and_unlink(filename):
    ...
    util.debug(
        f"[FINALIZER CALL] object mapping to {os.path.basename(filename)} about to be deleted,"
        f" decrementing the refcount of the file (pid: {os.getpid()})"
    )
...
def add_maybe_unlink_finalizer(memmap):
    util.debug(
        f"[FINALIZER ADD] adding finalizer to {type(memmap)} (id {id(memmap)}, filename {os.path.basename(memmap.filename)}, pid  {os.getpid()})"
    )
...
def unlink_file(filename):
    ...
    for retry_no in range(1, NUM_RETRIES + 1):
        try:
            ...
            util.debug(
                f"[ResourceTracker] tried to unlink {filename}, got PermissionError"
            )
...
def _reduce_memmap_backed(a, m):
    ...
    util.debug(
        f"[MEMMAP REDUCE] reducing a memmap-backed array (shape, {a.shape}, pid: {os.getpid()})"
    )
    try:
    ...
    def __call__(self, a):
        ...
        if (
            ...
            try:
                ...
                basename = f"{os.getpid()}-{id(threading.current_thread())}-{uuid4().hex}.pkl"
            ...
            if not os.path.exists(filename):
                util.debug(
                    f"[ARRAY DUMP] Pickling new array (shape={a.shape}, dtype={a.dtype}) "
                    f"creating a new memmap at {filename}"
                )
                ...
                util.debug(
                    f"[ARRAY DUMP] Pickling known array (shape={a.shape}, dtype={a.dtype}) "
                    f"reusing memmap file: {os.path.basename(filename)}"
                )
            ...
            util.debug(
                f"[ARRAY DUMP] Pickling array (NO MEMMAPPING) (shape={a.shape}, "
                f" dtype={a.dtype})."
            )
...
class TemporaryResourcesManager:
    ...
    def register_new_context(self, context_id):
        ...
        if context_id in self._cached_temp_folders:
            ...
            new_folder_name = f"joblib_memmapping_folder_{os.getpid()}_{self._id}_{context_id}"
    ...
    def register_folder_finalizer(self, pool_subfolder, context_id):
        ...
        def _cleanup():
            ...
            try:
                ...
                warnings.warn(
                    f"Failed to delete temporary folder: {pool_subfolder}"
                )
