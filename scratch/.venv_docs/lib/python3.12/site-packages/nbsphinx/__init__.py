...
class Exporter(nbconvert.RSTExporter):
    ...
    def __init__(self, execute='auto', kernel_name='', execute_arguments=[],
        ...
        def replace_attachments(text):
            for filename, bundle in attachment_storage:
                ...
                text = re.sub(
                    rf'^(\s*\.\. ((\|[^|]*\| )?image|figure)::) attachment:{filename}$'
                        ,
                    rf'\1 data:{mime_type};base64,{data}',
    ...
    def from_notebook_node(self, nb, resources=None, **kw):
        ...
        if execute not in ('always', 'never', 'auto'):
            raise ValueError(f'invalid execute option: {execute!r}')
        auto_execute = (
            execute == 'auto' and
        ...
        rststr, resources = super().from_notebook_node(
            nb, resources, **kw)
        ...
        if orphan is True:
            ...
            raise ValueError(f'invalid orphan option: {orphan!r}')
    ...
    def parse(self, inputstring, document):
        ...
        *inputstring* is either the JSON representation of a notebook,
        ...
        for format, converter in formats.items():
            ...
            raise NotebookError(
                f'No converter was found for {srcfile!r}')
        ...
        try:
            ...
            lines = str(e).split('\n')
            lines[0] = f'CellExecutionError in {env.doc2path(env.docname, base=False)}:'
...
def _extract_gallery_or_toctree(cell):
    ...
    try:
        ...
        raise ValueError(
            f'invalid nbsphinx-[link-]gallery/nbsphinx-toctree option: {options!r}'
            )
...
def depart_gallery_html(self, node):
    ...
    for title, uri, filename, tooltip in node['entries']:
        if tooltip:
            tooltip = f' title="{html.escape(tooltip)}"'
        self.body.append(f"""\
<a class="reference internal" href="{html.escape(uri)}"{tooltip}>
  <div><img alt="" src="{html.escape(filename)}"></div>
