...
def build_REAL4FrequencySeries(series, comment=None, encoding="Text"):
    ...
    return _build_series(series, ("Frequency,Real", "Frequency"), comment, 'deltaF', 's^-1', encoding=encoding)
...
def build_COMPLEX8FrequencySeries(series, comment=None, encoding="Text"):
    ...
    return _build_series(series, ("Frequency,Real,Imaginary", "Frequency"), comment, 'deltaF', 's^-1', encoding=encoding)
...
def build_REAL4TimeSeries(series, comment=None, encoding="Text"):
    ...
    return _build_series(series, ("Time,Real", "Time"), comment, 'deltaT', 's', encoding=encoding)
...
def build_COMPLEX8TimeSeries(series, comment=None, encoding="Text"):
    ...
    return _build_series(series, ("Time,Real,Imaginary", "Time"), comment, 'deltaT', 's', encoding=encoding)
...
def read_psd_xmldoc(xmldoc, root_name = "psd"):
    ...
    if root_name is not None:
        xmldoc, = (elem for elem in xmldoc.getElementsByTagName(ligolw.LIGO_LW.tagName) if elem.hasAttribute("Name") and elem.Name == root_name)
    result = dict((ligolw.Param.get_param(elem, "instrument").value, parse_REAL8FrequencySeries(elem)) for elem in xmldoc.getElementsByTagName(ligolw.LIGO_LW.tagName) if elem.hasAttribute("Name") and elem.Name == "REAL8FrequencySeries")
