...
try:
    ...
    swig_redirect_standard_output_error(True)
    @contextmanager
    def no_swig_redirect_standard_output_error():
        state = swig_redirect_standard_output_error(False)
        yield
        swig_redirect_standard_output_error(state)
    import warnings
...
def unpickle_vector(data):
    ...
    creator = globals()[f'Create{lal_type}Vector']
...
def unpickle_series(attrs):
    ...
    creator = globals()[f'{lal_type}{kind}Series']
    result = creator()
...
for datatype in numpy_to_lal_types.values():
    clazz = globals().get(f'{datatype}Vector')
    if clazz:
        copyreg.pickle(clazz, pickle_vector)
    clazz = globals().get(f'{datatype}FrequencySeries')
    if clazz:
        copyreg.pickle(clazz, pickle_series)
    clazz = globals().get(f'{datatype}TimeSeries')
    if clazz:
        copyreg.pickle(clazz, pickle_series)
