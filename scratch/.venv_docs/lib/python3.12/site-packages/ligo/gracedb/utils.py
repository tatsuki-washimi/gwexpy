...
def content_id(filecontents):
    """Generate a content-based UUID from file contents.

    Computes SHA-256 of the raw bytes and truncates to 128 bits to
    produce a deterministic UUID.  The same file contents will always
    produce the same UUID.

    Args:
        filecontents (bytes): raw file contents

    Returns:
        uuid.UUID: content-derived identifier

    """
    digest = hashlib.sha256(filecontents).digest()[:16]
...
def handle_str_or_list_arg(arg, arg_name):
    if arg:
        if isinstance(arg, str):
            ...
            raise TypeError(f"{arg_name} arg is {type(arg)}, should be str or list"
                            )
    return arg
