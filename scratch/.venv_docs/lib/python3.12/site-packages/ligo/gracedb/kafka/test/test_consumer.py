...
@mock.patch('ligo.gracedb.kafka.consumer.KAFKA_AVAILABLE', True)
class TestConsumerHeaderDecode:
    """Test header decoding with fallback on invalid UTF-8."""

    def _make_consumer(self):
        consumer, self.mock_consumer = _make_consumer()
        return consumer

    def _make_msg(self, headers):
        m = mock.Mock()
        m.error.return_value = None
        m.value.return_value = b'"data"'
        m.key.return_value = None
        m.headers.return_value = headers
        m.topic.return_value = 'test'
        m.partition.return_value = 0
        m.offset.return_value = 0
        m.timestamp.return_value = (0, None)
    ...
    def test_valid_utf8_headers(self):
        """Headers with valid UTF-8 bytes are decoded to strings."""
        consumer = self._make_consumer()
        headers = [('key1', b'value1'), ('key2', b'value2')]
        self.mock_consumer.poll.return_value = self._make_msg(headers)

        result = consumer.poll()
        assert result['headers'] == {'key1': 'value1', 'key2': 'value2'}

    def test_invalid_utf8_headers_fallback(self):
        """Headers with invalid UTF-8 fall back to raw bytes."""
        consumer = self._make_consumer()
        bad_bytes = b'\xff\xfe\x80'
        headers = [('key1', bad_bytes), ('key2', b'ok')]
        self.mock_consumer.poll.return_value = self._make_msg(headers)

        result = consumer.poll()
        assert result['headers'] == {'key1': bad_bytes, 'key2': b'ok'}

    def test_none_headers(self):
        """None headers remain None."""
        consumer = self._make_consumer()
        self.mock_consumer.poll.return_value = self._make_msg(None)
...
class TestConsumerAuthError:
    ...
    def test_poll_raises_on_prior_auth_error(self):
        ...
        # poll() is still called; _check_auth runs after
