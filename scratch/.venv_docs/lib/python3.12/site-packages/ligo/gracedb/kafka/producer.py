...
class GraceDbKafkaProducer:
    ...
    Args:
        ...
        guaranteed_delivery (bool): When ``True`` (default), configures the
            producer for reliable delivery: ``acks=all``, retries on
            transient failures, and delivery verification via callback.
            Set to ``False`` for higher throughput at the risk of
            silent message loss.
    ...
    def __init__(self, bootstrap_servers, topic=None,
        ...
        if guaranteed_delivery:
            # Require broker acknowledgment and retry transient failures.
            config.update({
                'acks': 'all',
                'retries': 5,
    ...
    def produce(self, value, topic=None, key=None, headers=None,
        ...
        # Wrap callback to always track pending message count
        def _tracking_callback(err, msg):
        ...
        try:
            ...
            try:
                ...
                log.error(f"Failed to produce message after retry: {e}")
                raise KafkaProducerError(
                    f"Message production failed after retry: {e}"
                ) from e
        except KafkaException as e:
    ...
    def ping(self, topic=None, timeout=10):
        ...
        if topic is not None:
            ...
            topic = self.default_topic or make_topic_name(
