...
def test_create_gstlal(client, test_data_dir):
    ...
    response = client.create_event('Test', 'gstlal', filename)
...
def test_create_mbta(client, test_data_dir):
    ...
    response = client.create_event('Test', 'MBTAOnline', filename)
    assert response.status_code == 201
    data = response.json()
...
def test_create_spiir(client, test_data_dir):
    ...
    response = client.create_event('Test', 'spiir', filename)
    assert response.status_code == 201
    data = response.json()
...
def test_create_cwb(client, test_data_dir):
    ...
    response = client.create_event('Test', 'CWB', filename)
    assert response.status_code == 201
    data = response.json()
...
def test_create_external(client, test_data_dir):
    ...
    response = client.create_event('Test', 'Fermi', filename, search='GRB')
    assert response.status_code == 201
    data = response.json()
...
def test_creation_with_offline(client, test_data_dir, offline):
    filename = os.path.join(test_data_dir, 'cbc-lm.xml')
    response = client.create_event('Test', 'gstlal', filename, offline=offline)
    assert response.status_code == 201
    data = response.json()
...
def test_creation_with_labels(client, test_data_dir):
    labels = ['INJ', 'DQV']
    filename = os.path.join(test_data_dir, 'cbc-lm.xml')
    response = client.create_event('Test', 'gstlal', filename, labels=labels)
    assert response.status_code == 201
    data = response.json()
...
def test_replace_event(client, test_data_dir):
    ...
    response = client.replace_event(graceid, new_data_file)
