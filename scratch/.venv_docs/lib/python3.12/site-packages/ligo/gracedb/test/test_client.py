...
@mock.patch(
    ...
    lambda cert, timeleft: True,
)
@mock.patch.dict(os.environ)
def test_x509_lookup_cert_key_from_envvars():
    """Test lookup of X509 cert and key from environment variables"""
    # FIXME: use tmp_path fixture when pytest min_version is 3.9.0
    with tempfile.TemporaryDirectory() as tmpdir:
        tmppath = Path(tmpdir)

        # Setup
        cert_file = tmppath / "cert_file"
        key_file = tmppath / "key_file"
        cert_file.touch()  # file needs to exist
        key_file.touch()  # file needs to exist
        os.environ["X509_USER_CERT"] = str(cert_file)
        os.environ["X509_USER_KEY"] = str(key_file)
    ...
    assert len(g.cert) == 2
    assert 'x509' in g.auth_type
    assert creds['x509'].get('cert_file') == str(cert_file)
    assert creds['x509'].get('key_file') == str(key_file)
...
def test_x509_lookup_proxy_from_envvars():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        # Setup
        proxy_file = Path(tmpdir) / "proxy_file"
        proxy_file.touch()
    ...
    assert isinstance(g.cert, str)
    assert 'x509' in g.auth_type
    assert creds['x509'].get('cert_file') == str(proxy_file)
    assert creds['x509'].get('key_file') == str(proxy_file)
