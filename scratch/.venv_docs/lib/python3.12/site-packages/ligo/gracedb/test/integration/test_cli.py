...
def run_CLI_test(cmd):
    cmd += f' --service-url={TEST_SERVICE}'
...
def test_update_event():
    ...
    cmd = f'update event {gid} {filename}'
    response = run_CLI_test(cmd)
...
def test_logs_files():
    ...
    # Test output
    assert response.status_code == 200
    comments = [log['comment'] for log in out['log']]
    assert comment in comments

    # Get file ----------------------------------------------------------------
    cmd = f'get file {gid} {fname}'
    response = run_CLI_test(cmd)
    with open(filename, 'rb') as fh:
        assert response == fh.read()

    # List files -------------------------------------------------------------
    cmd = f'list files {gid}'
    response = run_CLI_test(cmd)
    out = response.json()
    assert response.status_code == 200
    assert len(out) >= 2
...
def test_tags():
    ...
    # Remove tag from log -----------------------------------------------------
    cmd = f'remove tag {gid} {log_N} {tag}'
    response = run_CLI_test(cmd)
    assert response.status_code == 204
...
def test_create_superevent():
    ...
    # Create superevent
    cmd = (f'create superevent 1 2 3 {gid1} --labels=INJ,EM_READY '
           f'--category=Test --events={gid2}')
    response = run_CLI_test(cmd)
    out = response.json()
...
def test_get_superevent():
    ...
    cmd = f'create superevent 1 2 3 {gid} --category=Test '
    response = run_CLI_test(cmd)
    out = response.json()
    sid = out['superevent_id']

    # Get superevent
    cmd = f'get superevent {sid}'
    response = run_CLI_test(cmd)
    out = response.json()
...
def test_add_event_to_superevent():
    ...
    cmd = f'create superevent 1 2 3 {gid1} --category=Test '
    response = run_CLI_test(cmd)
    out = response.json()
    sid = out['superevent_id']
...
def test_remove_event_from_superevent():
    ...
    cmd = (f'create superevent 1 2 3 {gid1} --category=Test --events={gid2}')
    response = run_CLI_test(cmd)
    out = response.json()
    sid = out['superevent_id']
...
def test_update_superevent():
    ...
    # Update superevent -------------------------------------------------------
    cmd = (f'update superevent {sid} --t-start=4 --t-0=5 --t-end=6 '
           f'--preferred-event={gid2}')
    response = run_CLI_test(cmd)
    out = response.json()
...
def test_update_grbevent():
    ...
    ra = 12.34
    cmd = (f'update grbevent {gid} --redshift={redshift} --designation="{designation}" '
           f'--ra={ra}')
    try:
        response = run_CLI_test(cmd)
...
def test_search_superevents():
    ...
    # Setup: create superevent
    cmd = (f'create superevent 1 2 3 {gid} --labels=INJ,EM_READY '
           '--category=Test')
    response = run_CLI_test(cmd)
    out = response.json()
    assert response.status_code == 201
...
def test_voevents():
    ...
    filename = os.path.join(TEST_DATA_DIR, 'test_file.txt')
    cmd = f'create log {gid} "fake skymap" {filename}'
    response = run_CLI_test(cmd)
    assert response.status_code == 201

    # Creation ----------------------------------------------------------------
    cmd = (f'create voevent {gid} UP --skymap-filename={os.path.basename(filename)} --hardware-inj '
           f' --skymap-type=update --prob-has-ns={0.1} --bbh={0.3}')
    response = run_CLI_test(cmd)
    out = response.json()
    voevent_N = out['N']
    ...
    # Get ---------------------------------------------------------------------
    cmd = f'get voevent {gid} {voevent_N}'
    response = run_CLI_test(cmd)
    out = response.json()
...
def test_emobservations():
    ...
    cmd = (f'create emobservation {gid} {group} {join_s(ra_list)} {join_s(ra_width)} '
           f'{join_s(dec_list)} {join_s(dec_width)} {join_s(start_time_list)} {join_s(duration)}')
    ...
    # Get ---------------------------------------------------------------------
    cmd = f'get emobservation {gid} {emo_N}'
    response = run_CLI_test(cmd)
    out = response.json()
...
def test_signoffs():
    ...
    cmd = f'add label {sid} ADVREQ'
    response = run_CLI_test(cmd)
    out = response.json()
    assert response.status_code == 201

    # Create signoff ----------------------------------------------------------
    cmd = f'create signoff {sid} ADV OK "looks good"'
    try:
        response = run_CLI_test(cmd)
        out = response.json()
    ...
    if authorized_user:
        ...
        # Update signoff ---------------------------------------------------
        cmd = f'update signoff {sid} ADV --status=NO --comment="new"'
        response = run_CLI_test(cmd)
        out = response.json()
...
def test_ping():
    ...
    assert response.startswith(f'Response from {TEST_SERVICE}')
