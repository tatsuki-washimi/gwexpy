...
from .utils import content_id, dict_to_form_encoded, event_or_superevent, get_mimetype
...
class GraceDb(GraceDBClient):
    ...
    preference unless overridden by the ``use_auth`` variable. Lookup of
    user credentials is done in the following order:

    #. A scitoken, usually located in ``/tmp/bt_u${UID}``, or the \
        location provided by the ``SCITOKEN_FILE`` environment variable.
    ...
    def __init__(self, service_url=DEFAULT_SERVICE_URL,
                 ...
                 reload_certificate=False, reload_cred=False,
                 reload_buffer=300, use_auth='all',
                ...
                if (e.status == 404):
                    ...
                    if available_api_versions:
                        err_msg = ('Bad API version. Available versions for '
                                   f'this server are: {available_api_versions}')
                    else:
    ...
    @staticmethod
    def _read_event_file(filename, filecontents=None):
        """Read event file contents, handling stdin and file paths.

        Args:
            filename (str): path to event file, or '-' for stdin.
            filecontents: optional pre-read file contents.
        ...
        if filecontents is None:
            if filename == '-':
                ...
                filecontents = builtins.open(filename, 'rb')
        return filename, filecontents

    def _reserve_graceid(self, group, pipeline, search=None,
                         file_content_id=None):
        """Reserve a GraceDB ID for a future event.

        This is an internal method that reserves a graceid without creating
        a full event. Only available in API v2.

        Args:
            group (str): name of the analysis group.
            pipeline (str): name of the analysis pipeline.
            search (:obj:`str`, optional): type of search.
            file_content_id (:obj:`uuid.UUID`, optional): content-based
                identifier derived from the event file contents.

        Returns:
            :class:`requests.models.Response`

        Raises:
            NotImplementedError: if the API version is not v2 or the server
                does not support graceid reservation.
        ...
        if self._api_version != 'v2':
            raise NotImplementedError(
                "_reserve_graceid() is only implemented in API v2"
            )

        try:
            uri = self.templates['event-reserve-template']
        except (KeyError, TypeError):
            raise NotImplementedError(
                "graceid reservation is not yet implemented on "
                f"{self._service_url}"
            )
        ...
        if file_content_id is not None:
            body['content_id'] = str(file_content_id)

        return self.post(uri, data=body)

    def _create_event_via_kafka(self, group, pipeline, filename, filecontents,
                                search=None, kafka=None, http_fallback=False,
                                gracedb_upload=True, labels=None,
                                skip_supervisor=False):
        """Create an event via Kafka.

        When gracedb_upload is True, reserves a graceid on the server first
        and includes it in the Kafka message. When gracedb_upload is False,
        publishes directly to Kafka without reserving a graceid.

        The Kafka topic is determined automatically from the event's group
        and pipeline:

        - ``group="Test"`` → ``{instance}.test``
        - Any other group → ``{instance}.{pipeline}`` (lowercased)

        Args:
            group (str): analysis group name.
            pipeline (str): analysis pipeline name.
            filename (str): event filename.
            filecontents: file contents (bytes, str, or file-like object).
            search (str, optional): search type.
            kafka: Kafka producer instance.
            http_fallback (bool): if True, return None on reservation failure
                to allow caller to fall back to HTTP.
            gracedb_upload (bool): whether the server should upload/process
                the event. When False, no graceid is reserved.
            labels (list, optional): labels to attach to the event.
            skip_supervisor (bool): if True, the supervisor should not
                process this message. Default False.

        Returns:
            :class:`requests.models.Response` or None (if fallback triggered
            or gracedb_upload is False).
        ...
        # Normalize filecontents to bytes early so we can compute content_id
        if hasattr(filecontents, 'read'):
            raw = filecontents.read()
            if isinstance(raw, str):
                raw = raw.encode('utf-8')
        elif isinstance(filecontents, str):
            raw = filecontents.encode('utf-8')
        else:
            raw = filecontents

        file_content_id = content_id(raw)

        if gracedb_upload:
            # Reserve a graceid
            try:
                reservation = self._reserve_graceid(
                    group, pipeline, search=search,
                    file_content_id=file_content_id,
                )
            except Exception as exc:
                # Auth/permission errors won't be helped by HTTP fallback
                if isinstance(exc, HTTPError) and exc.status_code in (401, 403):
                    raise
                if not http_fallback:
                    raise
                warnings.warn(
                    "Kafka submission failed during graceid reservation; "
            ...
            graceid = reservation_data['graceid']
            submitter = reservation_data['submitter']

        encoded_contents = base64.b64encode(raw).decode('ascii')

        # Build Kafka message
        import time as _time
        message = {
            'type': 'upload',
            'gracedb_server': self._service_url,
            'graceid': graceid,
            'submitter': submitter,
            'pipeline': pipeline,
            'gracedb_upload': gracedb_upload,
            'skip_supervisor': skip_supervisor,
            'labels': labels,
            'eventFile': [filename, encoded_contents],
            'content_id': str(file_content_id),
            '_producer_timestamp': _time.time(),
        }

        # Route to the correct topic based on group/pipeline
        if group.lower() == 'test':
            kafka_topic = 'test'
        else:
            kafka_topic = pipeline.lower()
        ...
        try:
            ...
            def _on_delivery(err, msg):
                if err is not None:
                    _delivery_error[0] = err

            kafka.produce(value=message, key=graceid, topic=kafka_topic,
                          on_delivery=_on_delivery)
            remaining = kafka.flush(timeout=kafka.flush_timeout)
            if remaining > 0:
                raise RuntimeError(
                    f"Kafka flush timed out with {remaining} message(s) "
                    f"still in queue after {kafka.flush_timeout}s"
                )
            if _delivery_error[0] is not None:
                raise RuntimeError(
                    f"Kafka delivery failed: {_delivery_error[0]}"
                )
        except Exception:
            if not gracedb_upload or not http_fallback:
                raise
            # We have a reserved graceid but Kafka delivery failed;
            # fall back to uploading directly via HTTP.
            warnings.warn(
                "Kafka produce failed after graceid reservation; "
                "falling back to upload_reserved_event via HTTP.",
                RuntimeWarning,
            )
            return self.upload_reserved_event(
                graceid, filename,
                filecontents=raw,
                labels=labels,
                kafka_server="kafka delivery failed, reverted to http",
                kafka_topic=None,
            )
    ...
    def create_event(self, group, pipeline, filename, search=None, labels=None,
                     offline=False, filecontents=None, kafka=None,
                     http_fallback=False, gracedb_upload=True,
                     skip_supervisor=False, **kwargs):
        ...
        Args:
            ...
            kafka: optional Kafka producer instance. When provided, the event
                is submitted via Kafka after reserving a graceid.
            http_fallback (:obj:`bool`, optional): if ``True`` and the Kafka
                reservation fails, fall back to the HTTP POST path.
            gracedb_upload (:obj:`bool`, optional): included in the Kafka
                message to indicate whether the server should upload/process
                the event. Default ``True``. Only used with the Kafka path.
            skip_supervisor (:obj:`bool`, optional): if ``True``, the
                supervisor will not process this message (i.e., will not
                call upload-reserved-event). Default ``False``. Only used
                with the Kafka path.
        ...
        if labels is not None:
            if isinstance(labels, str):
                ...
                raise TypeError(f"labels arg is {type(labels)}, should be str or list"
                                )
            # Check labels against those in database
            for label in labels:
                if label not in self.allowed_labels:
                    raise ValueError(f"Label '{label}' does not exist in the "
                                     "database")
        filename, filecontents = self._read_event_file(filename, filecontents)

        if kafka is not None:
            result = self._create_event_via_kafka(
                group, pipeline, filename, filecontents,
                search=search, kafka=kafka, http_fallback=http_fallback,
                gracedb_upload=gracedb_upload, labels=labels,
                skip_supervisor=skip_supervisor,
            )
            # result is None when gracedb_upload=False (no reservation)
            # or when http_fallback triggered. Only fall through to HTTP
            # POST when fallback was actually triggered.
            if result is not None or not gracedb_upload:
                return result
            # Fallback was triggered; reset file position if needed
            if hasattr(filecontents, 'seek'):
                filecontents.seek(0)

        # Compute content_id from file contents
        if hasattr(filecontents, 'read'):
            raw = filecontents.read()
            if isinstance(raw, str):
                raw = raw.encode('utf-8')
    ...
    def upload_reserved_event(self, graceid, filename, filecontents=None,
                              labels=None, kafka_server=None,
                              kafka_topic=None):
        """Upload event data for a previously reserved graceid.

        Completes a reserved event by uploading its event file, optional
        labels, and the Kafka provenance metadata.  This is the server-side
        counterpart to the Kafka-based event submission path.

        Args:
            graceid (str): GraceDB ID of the reserved event.
            filename (str): path to the event file, or a filename when
                *filecontents* is provided.
            filecontents (:obj:`bytes`, optional): event file contents.
                If ``None``, the file at *filename* is read from disk.
            labels (:obj:`str` or :obj:`list`, optional): label or list of
                labels to apply to the event.
            kafka_server (str, optional): Kafka broker address that
                delivered the message.
            kafka_topic (str, optional): Kafka topic the message arrived
                on.
        ...
        Raises:
            NotImplementedError: if the server does not expose the
                ``event-upload-template`` endpoint.
            ligo.gracedb.exceptions.HTTPError: if the response has a
                status code >= 400.
        ...
        if self._api_version != 'v2':
            raise NotImplementedError(
                "upload_reserved_event is only implemented in API v2"
            )

        try:
            uri = self.templates['event-upload-template'].format(
                graceid=graceid
            )
        except (KeyError, TypeError):
            raise NotImplementedError(
                "upload_reserved_event is not yet implemented on "
                f"{self._service_url}"
            )
        ...
        # Normalise labels to a list (or None)
        if isinstance(labels, str):
            labels = [labels]
        ...
        fields = {'graceid': graceid}
        if labels:
            fields['labels'] = labels
        if kafka_server is not None:
            fields['kafka_server'] = kafka_server
        if kafka_topic is not None:
            fields['kafka_topic'] = kafka_topic
    ...
    def create_superevent(self, t_start, t_0, t_end, preferred_event,
        ...
        if events:
            if isinstance(events, str):
                ...
                raise TypeError(f"events arg is {type(events)}, should be str or list"
                                )

        # Validate category, convert to short form if necessary
        if not isinstance(category, str):
            err_msg = f"category arg is {type(category)}, should be a string"
            raise TypeError(err_msg)
        ...
        # superevent category
        if not category:
            raise ValueError(f"category must be one of: {list(self.superevent_categories.items())}")
    ...
    def write_log(self, object_id, message, filename=None, filecontents=None,
        ...
        if label:
            if not isinstance(label, str):
                raise TypeError(f"label arg is {type(label)}, should be "
                                " str")
    ...
    def labels(self, object_id, label="", *args, **kwargs):
        ...
        if label and label not in self.allowed_labels:
            raise NameError(f"Label '{label}' does not exist in the "
                            "database")
    ...
    def create_kafka_producer(self, bootstrap_servers, topic="test",
                             ca_cert_path=None, skip_hostname_verification=False,
        ...
        This method creates a Kafka producer configured to use SciToken
        authentication. The producer will automatically discover and use
        SciTokens for authentication with the Kafka broker.
        ...
        Args:
            ...
            ca_cert_path (str, optional): Path to CA certificate for SSL verification.
                Required if broker uses custom CA certificates.
            skip_hostname_verification (bool): Skip SSL hostname verification.
                Use for development/testing when cert CN doesn't match hostname.
                Default: False
            token_scope (str): Required SciToken scope. Default: "gracedb.read"
            reload_cred (bool): Proactively reload tokens before expiry.
                Default: True
            reload_buffer (int): Seconds before expiry to trigger reload.
                Default: 300
        ...
        Returns:
            GraceDbKafkaProducer: Configured Kafka producer instance

        Raises:
            ImportError: If confluent-kafka is not installed
            KafkaConnectionError: If connection to broker fails
            KafkaTokenError: If no valid SciToken can be found
        ...
        Note:
            This method requires the confluent-kafka package. Install it with:
        ...
        from .kafka import GraceDbKafkaProducer

        return GraceDbKafkaProducer(
