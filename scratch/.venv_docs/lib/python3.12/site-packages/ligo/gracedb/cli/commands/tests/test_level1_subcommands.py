...
def test_subcommand_help_content(CLI, cmd):
    ...
    # Check usage
    assert help_text.startswith(f'usage: {CLI.parser.prog} {cmd}')
...
def test_credentials_subcommand(CLI, cmd, cli_func, kw):
    ...
    func = f'ligo.gracedb.rest.GraceDb.{cli_func}'
    with mock.patch(func) as mock_cli_func:
        CLI(['credentials', cmd])
...
def test_ping_subcommand(CLI):
    ...
    # Check output
    assert output == f'Response from {CLI.client._service_url}: {mock_cli_func().status_code}'
