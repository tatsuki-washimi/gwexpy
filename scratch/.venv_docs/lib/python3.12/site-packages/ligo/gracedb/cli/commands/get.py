...
from ..parsers import graceid_parser, object_id_parser, superevent_id_parser
    ...
    def run(self, client, args):
        ...
        if args.destination != '-':
            ...
            if not os.access(full_dir_path, os.W_OK):
                raise OSError(f"{args.destination}: permission denied")
        ...
        if response.status_code == 200:
            if args.destination == '-':
                ...
                return f"File '{args.filename}' for {args.object_id} saved at {args.destination}"
        else:
