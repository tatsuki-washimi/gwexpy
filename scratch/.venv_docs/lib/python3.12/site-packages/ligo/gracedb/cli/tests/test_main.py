...
def test_main_with_ok_response(main_tester, capsys, output_type):
    ...
    if output_type == 'json':
        ...
        assert stdout.rstrip() == f'Server returned {status}: {reason}'
...
def test_main_with_bad_response(main_tester, capsys, response_msg):
    ...
    if len(response_msg) < 1000:
        assert stdout.rstrip() == f'{status} {reason}: {response_msg}'
    else:
        assert stdout.rstrip() == f'{status} {reason}'
...
def test_main_with_exception_in_CLI_call(main_tester, capsys, exc):
    ...
    if isinstance(exc, HTTPError):
        assert stdout.rstrip() == f'Error: {exc.status_code} {exc.reason}. {exc.text}.'
    elif isinstance(exc, Exception):
        assert stdout.rstrip() == f'Error: {str(exc)}'
...
def test_main_with_unexpected_response_type(main_tester, capsys):
    ...
    # Test output
    expected = f"Unexpected response type {type(response)}\nResponse: {str(response)}"
