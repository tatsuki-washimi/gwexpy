...
def test_update_superevent_subcommand(CLI, t_start, t_0, t_end, pref_ev):
    ...
    # Generate command
    cmd = f"update superevent {s_id}"
    if t_start is not None:
        cmd += f" --t-start={t_start}"
    if t_0 is not None:
        cmd += f" --t-0={t_0}"
    if t_end is not None:
        cmd += f" --t-end={t_end}"
    if pref_ev is not None:
        cmd += f" --preferred-event={pref_ev}"
...
def test_update_grbevent_subcommand(
    ...
    # Generate command
    cmd = f"update grbevent {gid}"
    if ra is not None:
        cmd += f" --ra={ra}"
    if dec is not None:
        cmd += f" --dec={dec}"
    if error_radius is not None:
        cmd += f" --error-radius={error_radius}"
    if t90 is not None:
        cmd += f" --t90={t90}"
    if redshift is not None:
        cmd += f" --redshift={redshift}"
    if designation is not None:
        cmd += f" --designation={designation}"
