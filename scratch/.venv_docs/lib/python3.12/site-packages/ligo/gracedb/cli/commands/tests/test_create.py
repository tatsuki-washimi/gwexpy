...
def test_create_emobservation_subcommand(CLI, ra_width, dec_width, duration,
    ...
    cmd = (f'create emobservation {s_id} {group} {join_s(ra_list)} {join_s(ra_width)} '
...
def test_create_voevent_subcommand(
    ...
    if skymap_type is not None:
        cmd += f" --skymap-type={skymap_type}"
    if skymap_filename is not None:
        cmd += f" --skymap-filename={skymap_filename}"
    if external:
        cmd += " --external"
    if open_alert:
        cmd += " --open-alert"
    ...
    if prob_has_ns is not None:
        cmd += f" --prob-has-ns={prob_has_ns}"
    if prob_has_remnant is not None:
        cmd += f" --prob-has-remnant={prob_has_remnant}"
    if bns is not None:
        cmd += f" --bns={bns}"
    if nsbh is not None:
        cmd += f" --nsbh={nsbh}"
    ...
    if terrestrial is not None:
        cmd += f" --terrestrial={terrestrial}"
    if mass_gap is not None:
        cmd += f" --mass-gap={mass_gap}"
