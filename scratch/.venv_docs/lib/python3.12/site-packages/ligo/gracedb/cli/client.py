...
from ligo.gracedb.cli.commands.base import CommandBase, command_registry
from ligo.gracedb.exceptions import HTTPError
from ligo.gracedb.rest import DEFAULT_SERVICE_URL, GraceDb
from requests import Response

# Version string
VERSION_STRING = f"GraceDB Client {__version__}"
    ...
    def set_up_client(self, args):
        ...
        if creds:
            ...
            if len(creds) == 1:
                ...
                print(f"Malformed 'creds' argument: {args.creds}")
                sys.exit(1)
...
def main(args=None):
    ...
    try:
        ...
        print(f'Error: {e.status_code} {e.reason}. {e.text}.')
        sys.exit(1)
    ...
    if isinstance(response, Response):
        ...
        try:
            ...
            print('WARNING: problem decoding binary data. Please '
                  'specify a destination filename.')
    elif isinstance(response, int):
        # Command returned an exit code (e.g., Kafka commands)
        # 0 = success, non-zero = error
        sys.exit(response)
    elif response is None:
        # Command completed without explicit return value
        pass
    else:
