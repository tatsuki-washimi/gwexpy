...
def condition_prior(
    ...
    if min_distance == 0 and prior_distance_power < 0:
        raise ValueError(
            f"Prior is a power law r^k with k={prior_distance_power}, undefined at min_distance=0"
        )
...
def localize(
    ...
    argstr = inspect.formatargvalues(
        *inspect.getargvalues(frame), formatvalue=formatvalue
    )

    stopwatch = Stopwatch()
    stopwatch.start()

    epoch, sample_rate, toas, snrs, responses, locations, horizons = condition(
        event,
        waveform=waveform,
        f_low=f_low,
    ...
    min_distance, max_distance, prior_distance_power, cosmology = condition_prior(
        horizons, min_distance, max_distance, prior_distance_power, cosmology
    )
    ...
    skymap.meta["history"] = [
        ...
        *wrap(f"{__name__}.{frame.f_code.co_name}{argstr}", 72),
        "",
...
def rasterize(skymap, order=None):
    ...
    if downsampling:
        ...
        distmean, diststd, _ = distance.parameters_to_moments(distmu, distsigma)
        distmean[bad] = np.nan
        diststd[bad] = np.nan
        skymap["DISTMEAN"] = probdensity * distmean
        skymap["DISTVAR"] = probdensity * (np.square(diststd) + np.square(distmean))

    skymap = Table(moc.rasterize(skymap, order=order), meta=skymap.meta, copy=False)

    # If we are downsampling, then convert back to distance parameters.
    if downsampling:
        distmean = skymap.columns.pop("DISTMEAN") / skymap["PROBDENSITY"]
        diststd = np.sqrt(
            skymap.columns.pop("DISTVAR") / skymap["PROBDENSITY"] - np.square(distmean)
        )
        skymap["DISTMU"], skymap["DISTSIGMA"], skymap["DISTNORM"] = (
            distance.moments_to_parameters(distmean, diststd)
        )
