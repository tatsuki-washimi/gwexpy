...
from . import ArgumentParser, get_random_parser, register_to_xmldoc, write_fileobj
...
def cell_max(values):
    r"""Find pairwise max of consecutive elements across all axes of an array.
...
def parser():
    ...
    group.add_argument(
        "--distribution",
        help="Use a preset distribution",
        choices=(
            "bns_astro",
            "bns_broad",
            "nsbh_astro",
    ...
    parser.add_argument(
        "--f-low", type=float, default=25.0, help="Low frequency cutoff in Hz"
    )
    parser.add_argument(
        "--snr-threshold", type=float, default=4.0, help="Single-detector SNR threshold"
    )
    parser.add_argument(
        "--min-triggers",
        type=int,
    ...
    parser.add_argument(
        "--waveform", default="o2-uberbank", help="Waveform approximant"
    )
    parser.add_argument(
        "--nsamples", type=int, default=100000, help="Output this many injections"
    )
    parser.add_argument(
        "-o",
        "--output",
        ...
        default="-",
        metavar="INJ.xml[.gz]",
        help="Output file, optionally gzip-compressed",
    )
    parser.add_argument(
        "-j",
        "--jobs",
...
def main(args=None):
    ...
    with p.parse_args(args) as args:
        ...
        cols["mass1"][swap], cols["mass2"][swap] = (
            cols["mass2"][swap].copy(),
            cols["mass1"][swap].copy(),
        )
        cols["spin1z"][swap], cols["spin2z"][swap] = (
            cols["spin2z"][swap].copy(),
            cols["spin1z"][swap].copy(),
        )
