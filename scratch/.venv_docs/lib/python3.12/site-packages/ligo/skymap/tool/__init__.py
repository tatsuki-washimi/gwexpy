...
class GlobAction(argparse._StoreAction):
    ...
    def __call__(self, parser, namespace, values, *args, **kwargs):
        values = list(itertools.chain.from_iterable(glob.iglob(s) for s in values))
        if values:
...
def get_waveform_parser():
    ...
    group.add_argument(
        "--f-low", type=float, metavar="Hz", default=30, help="Low frequency cutoff"
    )
    group.add_argument(
        "--f-high-truncate",
        type=float,
        default=0.95,
        help="Truncate waveform at this fraction of the maximum frequency of the PSD",
    )
    group.add_argument(
        "--waveform",
        default="o2-uberbank",
...
class ArgumentParser(argparse.ArgumentParser):
    ...
    def __init__(
        ...
        self.add_argument("--version", action="version", version=version_string)
...
_compress_arg_map = {".bz2": "bz2", ".gz": "gz", ".xz": "xz", ".zst": "zst"}
