...
from . import ArgumentParser, DirType, EnableAction, get_random_parser
...
def parser():
    ...
    parser.add_argument(
        "samples",
        type=FileType("rb"),
        metavar="SAMPLES.hdf5",
        help="posterior samples file",
    )
    # Only present for backward compatibility with --samples syntax
    parser.add_argument(
        "--samples", action="store_false", dest="_ignored", help=SUPPRESS
    )
    parser.add_argument(
        "--outdir",
        "-o",
        default=".",
        type=DirType(create=True),
    ...
    parser.add_argument(
        ...
        default=False,
        action=EnableAction,
        help="Use a DPGMM instead of KDE for density estimation.",
    )
    return parser
...
def main(args=None):
    ...
    with _parser.parse_args(args) as args:
        ...
        from ..kde import Clustered2DSkyKDE, Clustered2Plus1DSkyKDE
        from ..util.stopwatch import Stopwatch

        if args.enable_dpgmm:
            if args.enable_distance_map:
                raise NotImplementedError(
                    "--enable-distance-map is not yetsupported with --enable-dpgmm"
                )
            else:
        ...
        io.write_sky_map(os.path.join(args.outdir, args.fitsoutname), hpmap, nest=True)
