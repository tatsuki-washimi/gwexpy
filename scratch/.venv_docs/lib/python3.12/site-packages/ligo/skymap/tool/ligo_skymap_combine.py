...
def main(args=None):
    with parser().parse_args(args) as args:
        ...
        for input_file in args.input:
            ...
            if has_distance:
                if dist_mu is not None:
                    raise RuntimeError(
                        "only one input localization can have distance information"
                    )
                dist_mu = data[1]
                dist_sigma = data[2]
                dist_norm = data[3]
        ...
        for nside, prob, _, _ in input_skymaps:
            if nside < max_nside:
                prob = hp.ud_grade(
                    prob, max_nside, order_in="NESTED", order_out="NESTED"
                )
        ...
        if dist_mu is not None:
            if ah.npix_to_nside(len(dist_mu)) < max_nside:
                dist_mu = hp.ud_grade(
                    dist_mu, max_nside, order_in="NESTED", order_out="NESTED"
                )
                dist_sigma = hp.ud_grade(
                    dist_sigma, max_nside, order_in="NESTED", order_out="NESTED"
                )
                dist_norm = hp.ud_grade(
                    dist_norm, max_nside, order_in="NESTED", order_out="NESTED"
                )
        ...
        for i, x in enumerate(input_skymaps):
            ...
            out_kwargs["HISTORY"].append("")
            for line in (
                x[3].tostring(sep="\n", endcard=False, padding=False).split("\n")
            ):
