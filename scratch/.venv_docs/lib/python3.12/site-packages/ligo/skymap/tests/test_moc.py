...
def input_skymap(order1, d_order, fraction, dtype=np.float64, extra_fields=True):
    ...
    if extra_fields:
        fields["VALUE2"] = np.pi * np.repeat(ipix1, npix2 // npix1).astype(dtype)
...
def test_rasterize_downsample(
    order_in, d_order_in, fraction_in, order_out, dtype, extra_fields
):
    npix_in = ah.nside_to_npix(ah.level_to_nside(order_in))
    npix_out = ah.nside_to_npix(ah.level_to_nside(order_out))
    skymap_in = input_skymap(order_in, d_order_in, fraction_in, dtype, extra_fields)
    skymap_out = moc.rasterize(skymap_in, order_out)

    assert len(skymap_out) == npix_out
    reps = npix_in // npix_out
    expected = np.mean(np.arange(npix_in).reshape(-1, reps), axis=1).astype(dtype)
...
def test_bayestar_adaptive_grid(order, rounds, benchmark):
    ...
    skymap_out = benchmark(
        moc.bayestar_adaptive_grid, func, top_nside=top_nside, rounds=rounds
    )

    assert len(skymap_out) == ah.nside_to_npix(top_nside) * (1 + 0.75 * rounds)
...
def test_bayestar_adaptive_grid_refinement(order):
    ...
    high_prob_ipix = np.random.choice(top_hpx.npix, top_hpx.npix // 4, replace=False)
    ...
    skymap_out = moc.bayestar_adaptive_grid(func, top_nside=top_nside, rounds=1)
    skymap_out.sort("UNIQ")

    for key in ["UNIQ", "PROBDENSITY"]:
        np.testing.assert_array_almost_equal(skymap_out[key], skymap_expected[key])
