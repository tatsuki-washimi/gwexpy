    ...
    *files : list of str, file-like object, or `h5py.File` objects
    ...
    def __init__(self, *files, **kwargs):
        ...
        try:
            ...
            raise ValueError("You must provide exactly one template bank file.")
        try:
        ...
        for f in trigger_files:
            ((ifo, group),) = f.items()
            triggers[ifo] = [group["snr"], group["coa_phase"], group["end_time"]]
    ...
    def singles(self):
        return tuple(
            HDFSingleEvent(
                ...
                self._id,
                i,
                trigger_ids[self._id],
                self._source._timeslide_interval,
                triggers,
                self._source._timeslide_ids,
                psds,
            )
            for i, (ifo, trigger_ids, triggers, psds) in enumerate(
                zip(
                    self._source._ifos,
                    self._source._trigger_ids,
                    self._source._triggers,
                    self._source._psds,
                )
            )
        )
...
class HDFSingleEvent(SingleEvent):
    ...
    def psd(self):
        ...
        fseries = lal.CreateREAL8FrequencySeries(
            "psd", 0, kmin * df, df, lal.DimensionlessUnit, len(psd) - kmin
        )
