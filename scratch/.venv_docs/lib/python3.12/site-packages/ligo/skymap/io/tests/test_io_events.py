...
def ligolw_assertions(source):
    ...
    assert event.singles[0].time == event.singles[0].zerolag_time == 967328914.866107842
    ...
    assert event.singles[1].time == event.singles[1].zerolag_time == 967328914.866513726
    psd = event.singles[0].psd
    assert psd.f0 == 0.0
    assert psd.deltaF == 0.125
...
def test_ligolw():
    ...
    assert (
        str(single_event)
        == "<LigoLWSingleEvent(detector='H1', snr=12.035994, phase=-1.0371021, time=970976257.4808338)>"
    )
    assert (
        repr(single_event)
        == "<LigoLWSingleEvent(detector='H1', snr=12.035994, phase=-1.0371021, time=970976257.4808338)>"
...
def test_ligolw_document():
    ...
    xmldoc = load_filename(
        os.path.join(DATA_PATH, "2016_subset.xml.gz"), contenthandler=ContentHandler
    )
...
def test_sqlite(tmpdir):
    ...
    subprocess.check_call(["igwn_ligolw_sqlite", "-p", xmlfilename, "-d", dbfilename])

    # Symbolicly link the PSD directory into the temporary directory.
    os.symlink(
        os.path.join(DATA_PATH, "gstlal_reference_psd"),
...
def test_gracedb(mock_gracedb):
    ...
    for i, (event_id, event) in enumerate(source.items()):
        if i == 0:
            ...
            assert (
                event.singles[0].time
                == event.singles[0].zerolag_time
            ...
            assert (
                event.singles[1].time
                == event.singles[1].zerolag_time
...
def test_detector_disabled(mock_gracedb):
    ...
    expected_message = (
        "Disabling detectors {H1, L1} would have no effect on "
    ...
    # Now test that exceptions are raised when they are called for.
    expected_message = (
        "Disabling detectors {H1 L1 V1} would exclude all "
...
def test_hdf(tmpdir):
    ...
    with h5py.File(filename, "w") as coinc_file:
        ...
        for i, ifo in enumerate(ifos):
            ...
            with h5py.File(filename, "w") as psd_file:
                ...
                for j in range(5):
                    psd_group[str(j)] = (
                        np.concatenate((np.zeros(5), np.arange(5) ** 2)) * np.e**2
                    )
                    psd_group[str(j)].attrs["delta_f"] = 2.0

    with pytest.raises(ValueError, match="You must provide exactly one coinc file."):
        source = events.open(*filenames[1:])

    with pytest.raises(
        ValueError, match="You must provide exactly one template bank file."
    ):
        source = events.open(*filenames[:1])

    with pytest.raises(ValueError, match="You must provide PSD files."):
        source = events.open(*filenames[:2])

    with pytest.raises(ValueError, match="You must provide trigger files."):
    ...
    for coinc_id, coinc in source.items():
        for i, (ifo, single) in enumerate(zip(ifos, coinc.singles)):
            ...
            assert single.time == (
                single.zerolag_time + coinc_id * 10 * np.e * (-0.5 if i == 0 else +0.5)
            )
    ...
    with ExitStack() as stack:
        events.open(
            *(stack.enter_context(open(filename, "rb")) for filename in filenames)
        )
