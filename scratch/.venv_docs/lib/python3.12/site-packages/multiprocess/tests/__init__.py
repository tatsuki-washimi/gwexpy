...
from multiprocess.connection import AuthenticationError, wait
...
class _TestLock(BaseTestCase):
    ...
    def test_repr_lock(self):
        ...
        lock = self.Lock()
        self.assertEqual('<Lock(owner=None)>', repr(lock))

        lock.acquire()
        self.assertEqual('<Lock(owner=MainProcess)>', repr(lock))
        lock.release()
        ...
        p.start()
        event.wait()
        self.assertEqual('<Lock(owner=SomeOtherProcess)>', repr(lock))
        p.terminate()
...
class TestFlags(unittest.TestCase):
    ...
    def _test_flags(self):
        ...
        prog = (
            'from multiprocess.tests import TestFlags; '
...
class _TestSpawnedSysPath(BaseTestCase):
    ...
    def test_forkserver_preload_imports_sys_path(self):
        ...
        self._ctx.set_forkserver_preload(
            ["test_multiprocessing_forkserver", self._mod_name])
        q = self._ctx.Queue()
