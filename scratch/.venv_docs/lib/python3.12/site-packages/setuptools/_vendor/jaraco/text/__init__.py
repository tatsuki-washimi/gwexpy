...
def is_decodable(value):
    r"""Return True if the supplied value is decodable (using the default
    encoding).
...
def is_binary(value):
    r"""Return True if the value appears to be binary (that is, it's a byte
...
def trim(s):
    r"""Trim something like a docstring to remove the whitespace that
...
def unwrap(s):
    r"""Given a multi-line string, return an unwrapped version.
...
class Stripper:
    r"""Given a series of lines, find the common prefix and strip it from them.
...
def normalize_newlines(text):
    r"""Replace alternate newlines with the canonical newline.
...
def yield_lines(iterable):
    r"""Yield valid lines of a string or iterable.
...
def join_continuation(lines):
    r"""Join lines continued by a trailing backslash.
...
def read_newlines(filename, limit=1024):
    r""">>> tmp_path = getfixture('tmp_path')
