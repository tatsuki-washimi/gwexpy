...
def forecast_cov(ma_coefs, sigma_u, steps):
    r"""Compute theoretical forecast error variance matrices
...
def var_loglike(resid, omega, nobs):
    r"""Returns the value of the VAR(p) log-likelihood.
...
class VAR(TimeSeriesModel):
    r"""Fit VAR(p) process and do lag order selection
...
class VARResults(VARProcess):
    ...
    def plot_acorr(self, nlags=10, resid=True, linewidth=8):
        r"""Plot autocorrelation of sample (endog) or residuals
    ...
    @cache_readonly
    def detomega(self):
        r"""Return determinant of white noise covariance with degrees of freedom
        correction:
