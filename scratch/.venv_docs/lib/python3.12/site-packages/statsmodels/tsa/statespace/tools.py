...
def diff(series, k_diff=1, k_seasonal_diff=None, seasonal_periods=1):
    r"""Difference a series simply and/or seasonally along the zero-th axis.
...
def is_invertible(polynomial, threshold=1 - 1e-10):
    r"""Determine if a polynomial is invertible.
...
def solve_discrete_lyapunov(a, q, complex_step=False):
    r"""Solves the discrete Lyapunov equation using a bilinear transformation.
...
def constrain_stationary_multivariate_python(unconstrained, error_variance,
                                             ...
                                             prefix=None):
    r"""Transform unconstrained parameters used by the optimizer to constrained
...
def _compute_multivariate_sample_acovf(endog, maxlag):
    r"""Computer multivariate sample autocovariances
...
def _compute_multivariate_acovf_from_coefficients(
    ...
    r"""Compute multivariate autocovariances from vector autoregression coefficient
    matrices
...
def _compute_multivariate_pacf_from_coefficients(constrained, error_variance,
                                                 order=None, k_endog=None):
    r"""Transform matrices corresponding to a stationary (or invertible) process
...
def compute_smoothed_state_weights(results, compute_t=None, compute_j=None,
    ...
    r"""Construct the weights of observations and the prior on the smoothed state
