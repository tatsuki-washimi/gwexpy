...
class UnobservedComponents(MLEModel):
    r"""Univariate unobserved components time series model
    ...
    def param_names(self):
        ...
        for key in self.parameters.keys():
            if key == 'irregular_var':
                ...
                freq_seasonal_name = f"{repr(periodicity)}({repr(harmonics)})"
                param_names.append(
        ...
        if spec.freq_seasonal:
            ...
            for ix, h in enumerate(spec.freq_seasonal_harmonics):
                ...
                item = Bunch(
                    ...
                    smoothed=None, smoothed_cov=None,
                    offset=offset,
                    pretty_name=f'seasonal {repr(period)}({repr(h)})')
                if self.smoothed_state is not None:
    ...
    def summary(self, alpha=.05, start=None):
        ...
        if self.specification.freq_seasonal:
            for ix, is_stochastic in enumerate(
                ...
                freq_seasonal_name = f"freq_seasonal({repr(periodicity)}({repr(harmonics)}))"
                if is_stochastic:
