...
class TestPoissonOffset(CompareMixin):
    ...
    def setup_class(cls):
        ...
        cls.res_discrete = Poisson(data_endog, data_exog[:, 1:], offset=offset).fit(
            disp=0
        )
...
class TestPoissonZi(CompareMixin):
    ...
    def test_params(self):
        ...
        assert_almost_equal(
            self.res.params[:-1], self.res_discrete.params, self.decimal
        )
    ...
    def test_cov_params(self):
        ...
        assert_almost_equal(
            self.res.bsejac[:-1], self.res_glm.bse[1:], self.decimal - 2
        )
        assert_almost_equal(
            self.res.bsejac[:-1], self.res_discrete.bse, self.decimal - 2
        )
