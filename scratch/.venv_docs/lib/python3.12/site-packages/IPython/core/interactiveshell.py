...
from IPython.core import magic, oinspect, page, ultratb
...
class InteractiveShell(SingletonConfigurable):
    ...
    def run_cell_magic(self, magic_name, line, cell):
        ...
        if fn is None:
            ...
            extra = '' if lm is None else (f' (But line magic `%{magic_name}` exists, '
    ...
    def system_raw(self, cmd):
        ...
        if main_cmd in has_magic_alternatives:
            warnings.warn(
                    ...
                    f"You executed the system command !{main_cmd} which may not work "
                    f"as expected. Try the IPython magic %{main_cmd} instead."
        ...
        for tgt in potential_target :
            if os.path.isfile(tgt):                        # Read file
                try :
                    ...
                    if not py_only :
                        with open(tgt, encoding='latin1') as f :
                            return f.read()
                    raise ValueError(("'%s' seem to be unreadable.") % target) from e
