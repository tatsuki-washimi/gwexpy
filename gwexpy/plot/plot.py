    ...
    **kwargs
    ...
    def show(
        ...
        *,
        ...
        close : bool, keyword-only, optional
            Explicit GWexpy extension. If True, close the figure after the
            GWpy display path returns. The default is False so that plain
            ``show()`` has the same lifecycle as GWpy.

        Examples
        --------
        >>> plot = Plot(data)
        >>> plot.show(block=False)  # Non-blocking, keep figure
        >>> plot.savefig("output.png")  # Works because figure is still open

        Notes
        -----
        ``warn`` and ``block`` retain GWpy's positional layout. Use the
        keyword-only ``close=True`` extension in notebooks when explicitly
        closing after display is desired.

        """
        super().show(warn=warn, block=block)
        if close:
