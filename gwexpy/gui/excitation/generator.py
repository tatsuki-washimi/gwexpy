...
class SignalGenerator:
    ...
    def generate(self, times: np.ndarray, params: GeneratorParams) -> np.ndarray:
        ...
        if "none" in wtype or wtype == "none":
            ...
            return params.offset + params.amplitude * np.sin(2 * np.pi * params.frequency * t + phi)

        elif "square" in wtype:
            # Phase shift for square?
            phi = np.deg2rad(params.phase)
            return params.offset + params.amplitude * scipy.signal.square(2 * np.pi * params.frequency * t + phi)

        elif "ramp" in wtype:
             # Sawtooth is essentially a ramp
             phi = np.deg2rad(params.phase)
             return params.offset + params.amplitude * scipy.signal.sawtooth(2 * np.pi * params.frequency * t + phi)

        elif "triangle" in wtype:
             # Triangle is sawtooth with width=0.5
             phi = np.deg2rad(params.phase)
             return params.offset + params.amplitude * scipy.signal.sawtooth(2 * np.pi * params.frequency * t + phi, width=0.5)
            ...
            return params.offset + params.amplitude * (scipy.signal.square(2 * np.pi * params.frequency * t + phi, duty=0.01) + 1)/2 * 2 # Rescale 0/1 to -1/1? Impulse is usually 0 to Amp.
            # "Impulse" usually means 0 everywhere except peak. 
            # Adjusted: 0 to Amp.
            duty = 0.05
            sq = scipy.signal.square(2 * np.pi * params.frequency * t + phi, duty=duty) # returns +1 and -1.
            # We want +1 for duty time, 0 otherwise? Or -1?
            # Let's assume bipolar impulse? Or Unipolar?
            # Standard: Unipolar pulse train.
            return params.offset + params.amplitude * ((sq > (1-2*duty)).astype(float))

        elif "offset" in wtype:
             return np.full_like(t, params.offset + params.amplitude) # Maybe Amp is the offset value here? Or Amp + Offset? usually just Offset. Let's use Amp as the value.
             ...
             if f_low > 0:
                 if f_high > f_low and f_high < nyq:
                     ...
                     btype = 'high'
                     Wn = f_low
                 elif f_high <= f_low: # Invalid range or intended HP? Treat as HP > Start
                     apply_filter = True
                     btype = 'high'
                     Wn = f_low
             ...
             if apply_filter:
                 ...
                 if key not in self.filter_states:
                     self.filter_states[key] = {'zi': None, 'last_t': -np.inf, 'cfg': None}
                 ...
                 is_continuous = (abs(time_gap - dt) < dt*0.5) if len(t) > 1 else False # simple check
                 ...
                 # Apply Filter
                 filtered_noise, new_zi = scipy.signal.sosfilt(state['sos'], raw_noise, zi=state['zi'])
                 state['zi'] = new_zi
                 state['last_t'] = t[-1]
            ...
            return params.offset + params.amplitude * scipy.signal.chirp(
                t_loc, f0=params.start_freq, t1=period, f1=params.stop_freq, method='linear'
            )
