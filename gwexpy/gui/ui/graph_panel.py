...
class GraphPanel(QtWidgets.QFrame):
    ...
    def _init_ui(self):
        ...
        vg = QtWidgets.QGroupBox("Values"); vgl = QtWidgets.QGridLayout(vg); txt_x1 = QtWidgets.QLineEdit(); txt_y1 = QtWidgets.QLineEdit(); txt_x2 = QtWidgets.QLineEdit(); txt_y2 = QtWidgets.QLineEdit()
        lab_x1 = QtWidgets.QLabel("X1:"); lab_y1 = QtWidgets.QLabel("Y1:"); lab_x2 = QtWidgets.QLabel("X2:"); lab_y2 = QtWidgets.QLabel("Y2:")
        vgl.addWidget(lab_x1,0,0); vgl.addWidget(txt_x1,0,1); vgl.addWidget(lab_y1,0,2); vgl.addWidget(txt_y1,0,3)
        vgl.addWidget(lab_x2,1,0); vgl.addWidget(txt_x2,1,1); vgl.addWidget(lab_y2,1,2); vgl.addWidget(txt_y2,1,3)
        cl.addWidget(vg)

        # Cursor Logic
        cursors = {'c1_v': pg.InfiniteLine(angle=90, movable=True, pen=pg.mkPen('g', width=1.5)), 
        ...
        def update_cursor_values():
            ...
            def get_snapped_val(val, vertical=True):
                if x_data is None or len(x_data) == 0: return val, 0.0
                if vertical:
            ...
            self.cursor_states['y1'] = y1_snap if rb_style_cross.isChecked() else cursors['c1_h'].value()
            if rb_style_cross.isChecked(): cursors['c1_h'].setValue(y1_snap)
            
            # Update C2
            x2_snap, y2_snap = get_snapped_val(cursors['c2_v'].value())
            self.cursor_states['x2'] = x2_snap
            self.cursor_states['y2'] = y2_snap if rb_style_cross.isChecked() else cursors['c2_h'].value()
            if rb_style_cross.isChecked(): cursors['c2_h'].setValue(y2_snap)
            ...
            if is_delta:
                txt_x2.setText(f"{(self.cursor_states['x2'] - self.cursor_states['x1']):.6g}")
                txt_y2.setText(f"{(self.cursor_states['y2'] - self.cursor_states['y1']):.6g}")
            else:
        ...
        def update_cursor_visibility():
            ...
            if ctrl['line_chk'].isChecked(): c_idx = ctrl['line_c'].currentIndex()
            elif ctrl['bar_chk'].isChecked(): c_idx = ctrl['bar_c'].currentIndex()
            elif ctrl['sym_chk'].isChecked(): c_idx = ctrl['sym_c'].currentIndex()
            else: c_idx = 0 # Fallback Red
            ...
            cursors['c1_v'].setPen(pen1); cursors['c1_h'].setPen(pen1)
            cursors['c2_v'].setPen(pen2); cursors['c2_h'].setPen(pen2)
            # -----------------
        ...
        for w in [cur_act1, cur_act2, rb_style_none, rb_style_vert, rb_style_cross, rb_style_horiz, rb_type_abs, rb_type_delta]:
        ...
        rb_left = QtWidgets.QRadioButton("Left"); rb_center = QtWidgets.QRadioButton("Center"); rb_right = QtWidgets.QRadioButton("Right")
        rb_center.setChecked(True)
        align_layout.addWidget(rb_left); align_layout.addWidget(rb_center); align_layout.addWidget(rb_right)
        ...
        def apply_title_style():
            ...
            colors_list = [('#FF0000'), ('#0000FF'), ('#00FF00'), ('#000000'), ('#FF00FF'), ('#00FFFF'), ('#FFFF00'), ('#FFA500')]
            ...
            # Alignment
            justify = "center"
            if rb_left.isChecked(): justify = "left"
            elif rb_right.isChecked(): justify = "right"
            
            self.target_plot.setTitle(text, color=color, size=size, bold=is_bold, italic=is_italic, justify=justify, family=family)
        ...
        def create_axis_tab_ui(default_title):
            ...
            # Title Group
            tg = QtWidgets.QGroupBox("Title")
            tgl = QtWidgets.QGridLayout(tg); tgl.setContentsMargins(4,6,4,4); tgl.setSpacing(2)
            title_edit = QtWidgets.QLineEdit(default_title)
            tgl.addWidget(title_edit, 0, 0, 1, 6)
            tgl.addWidget(QtWidgets.QLabel("Size:"), 1, 0)
            t_size = _small_spin_dbl(3, 60, 0, 1, 0.001); t_size.setValue(0.042); tgl.addWidget(t_size, 1, 1)
            tgl.addWidget(QtWidgets.QLabel("Offset:"), 1, 2)
            t_off = _small_spin_dbl(2, 60, -10, 10, 0.1); t_off.setValue(1.40); tgl.addWidget(t_off, 1, 3)
            t_clr = _clr_box(); t_clr.setCurrentIndex(3); tgl.addWidget(t_clr, 1, 5, QtCore.Qt.AlignRight)
            layout.addWidget(tg)

            # Ticks/Axis Group
            tag = QtWidgets.QGroupBox("Ticks/Axis")
            tagl = QtWidgets.QGridLayout(tag); tagl.setContentsMargins(4,6,4,4); tagl.setSpacing(2)
            tagl.addWidget(QtWidgets.QLabel("Length:"), 0, 0)
            tk_len = _small_spin_dbl(3, 60, 0, 1, 0.001); tk_len.setValue(0.030); tagl.addWidget(tk_len, 0, 1)
            both_chk = QtWidgets.QCheckBox("Both sides"); both_chk.setChecked(True); tagl.addWidget(both_chk, 0, 3)
            grid_chk = QtWidgets.QCheckBox("Grid"); grid_chk.setChecked(True); tagl.addWidget(grid_chk, 0, 4)
            tagl.addWidget(QtWidgets.QLabel("Divisions:"), 1, 0)
            div1 = _small_spin_int(0, 100, 40); div1.setValue(10); tagl.addWidget(div1, 1, 1)
            div2 = _small_spin_int(0, 100, 40); div2.setValue(5); tagl.addWidget(div2, 1, 2)
            div3 = _small_spin_int(0, 100, 40); div3.setValue(0); tagl.addWidget(div3, 1, 3)
            tk_clr = _clr_box(); tk_clr.setCurrentIndex(3); tagl.addWidget(tk_clr, 1, 5, QtCore.Qt.AlignRight)
            layout.addWidget(tag)

            # Labels Group
            lg = QtWidgets.QGroupBox("Labels")
            lgl = QtWidgets.QGridLayout(lg); lgl.setContentsMargins(4,6,4,4); lgl.setSpacing(2)
            lgl.addWidget(QtWidgets.QLabel("Size:"), 0, 0)
            l_size = _small_spin_dbl(3, 60, 0, 1, 0.001); l_size.setValue(0.040); lgl.addWidget(l_size, 0, 1)
            lgl.addWidget(QtWidgets.QLabel("Offset:"), 0, 2)
            l_off = _small_spin_dbl(3, 60, -1, 1, 0.001); l_off.setValue(0.005); lgl.addWidget(l_off, 0, 3)
            l_clr = _clr_box(); l_clr.setCurrentIndex(3); lgl.addWidget(l_clr, 0, 5, QtCore.Qt.AlignRight)
            layout.addWidget(lg)

            # Font Group
            fg = QtWidgets.QGroupBox("Font")
            fgl = QtWidgets.QHBoxLayout(fg); fgl.setContentsMargins(4,6,4,4); fgl.setSpacing(4)
            font_fam = QtWidgets.QComboBox(); font_fam.addItems(["Helvetica", "Times", "Courier", "Arial"])
            font_weight = QtWidgets.QComboBox(); font_weight.addItems(["normal", "bold", "italic"]); font_weight.setCurrentText("bold")
            fgl.addWidget(font_fam); fgl.addWidget(font_weight); fgl.addStretch(1)
            center_chk = QtWidgets.QCheckBox("Center"); center_chk.setChecked(True); fgl.addWidget(center_chk)
            layout.addWidget(fg)
            layout.addStretch(1)

            ctrls = {
                'title': title_edit, 't_size': t_size, 't_off': t_off, 't_clr': t_clr,
                'tk_len': tk_len, 'both': both_chk, 'grid': grid_chk, 'divs': [div1, div2, div3], 'tk_clr': tk_clr,
                'l_size': l_size, 'l_off': l_off, 'l_clr': l_clr, 'font_fam': font_fam, 'font_weight': font_weight, 'center': center_chk
            }
            return tab, ctrls
        ...
        def apply_axis_style(axis_pos):
            ...
            label_style = {'color': t_clr, 'font-size': t_px, 'font-family': family, 'font-weight': weight}
            ...
            elif weight == "italic": font.setItalic(True)
            ...
            if show_opp:
                ...
                opp_axis.setTicks([]) # Usually opposite side just has the line or minor ticks in DTT
            
        def update_xaxis(): apply_axis_style('bottom')
        def update_yaxis(): apply_axis_style('left')

        # Connect all signals
        for ctrls, upd in [(self.xaxis_ctrls, update_xaxis), (self.yaxis_ctrls, update_yaxis)]:
            ...
            for d in ctrls['divs']: d.valueChanged.connect(upd)
        ...
        pgl = QtWidgets.QGridLayout(pg_group); pgl.setContentsMargins(4,6,4,4); pgl.setSpacing(2)
        rb_tl = QtWidgets.QRadioButton("Top left"); rb_tr = QtWidgets.QRadioButton("Top Right")
        rb_bl = QtWidgets.QRadioButton("Bottom left"); rb_br = QtWidgets.QRadioButton("Bottom Right")
        rb_tr.setChecked(True)
        pgl.addWidget(rb_tl, 0, 0); pgl.addWidget(rb_tr, 0, 1)
        pgl.addWidget(rb_bl, 1, 0); pgl.addWidget(rb_br, 1, 1)
        
        pos_ly = QtWidgets.QHBoxLayout()
        pos_ly.addWidget(QtWidgets.QLabel("X:")); l_x = _small_spin_dbl(2, 60, -10, 10, 0.01)
        pos_ly.addWidget(l_x); pos_ly.addSpacing(10)
        pos_ly.addWidget(QtWidgets.QLabel("Y:")); l_y = _small_spin_dbl(2, 60, -10, 10, 0.01)
        pos_ly.addWidget(l_y); pos_ly.addSpacing(10)
        pos_ly.addWidget(QtWidgets.QLabel("Size:")); l_size = _small_spin_dbl(1, 60, 0.1, 10, 0.1); l_size.setValue(1.0)
        pos_ly.addWidget(l_size); pos_ly.addStretch(1)
        pgl.addLayout(pos_ly, 2, 0, 1, 2)
        ll.addWidget(pg_group)

        # Symbol style Group
        ss_group = QtWidgets.QGroupBox("Symbol style")
        ssl = QtWidgets.QHBoxLayout(ss_group); ssl.setContentsMargins(4,6,4,4)
        rb_sym_same = QtWidgets.QRadioButton("Same as trace"); rb_sym_none = QtWidgets.QRadioButton("None")
        rb_sym_same.setChecked(True)
        ssl.addWidget(rb_sym_same); ssl.addWidget(rb_sym_none); ssl.addStretch(1)
        ll.addWidget(ss_group)

        # Text Group
        txt_group = QtWidgets.QGroupBox("Text")
        txl = QtWidgets.QVBoxLayout(txt_group); txl.setContentsMargins(4,6,4,4); txl.setSpacing(4)
        tx_modes = QtWidgets.QHBoxLayout()
        rb_txt_auto = QtWidgets.QRadioButton("Auto"); rb_txt_user = QtWidgets.QRadioButton("User")
        rb_txt_auto.setChecked(True); tx_modes.addWidget(rb_txt_auto); tx_modes.addWidget(rb_txt_user); tx_modes.addStretch(1)
        txl.addLayout(tx_modes)
        
        trace_sel_tab = QtWidgets.QTabBar(); trace_sel_tab.setExpanding(False)
        for i in range(8): trace_sel_tab.addTab(str(i))
        ...
        def update_legend():
            ...
            # Anchor placement
            anchor = (1, 0) # TR
            if rb_tl.isChecked(): anchor = (0, 0)
            elif rb_bl.isChecked(): anchor = (0, 1)
            elif rb_br.isChecked(): anchor = (1, 1)
            
            # Correct positioning for pyqtgraph LegendItem
            # anchor(itemPos, parentPos, offset)
            target_legend.anchor(itemPos=anchor, parentPos=anchor, offset=(l_x.value() * 20, l_y.value() * 20))
            
            # Refresh items based on mode
            target_legend.clear()
            for i, ctrl in enumerate(self.trace_controls):
                if not ctrl['active'].isChecked(): continue
        ...
        for rb in [rb_tl, rb_tr, rb_bl, rb_br, rb_sym_same, rb_sym_none, rb_txt_auto, rb_txt_user]:
        ...
        def on_user_txt_edited():
            ...
            if rb_txt_user.isChecked(): update_legend()
        ...
        tfl = QtWidgets.QHBoxLayout(tf_group); tfl.setContentsMargins(4,6,4,4)
        rb_tf_utc = QtWidgets.QRadioButton("Date/time UTC"); rb_tf_gps = QtWidgets.QRadioButton("GPS seconds")
        rb_tf_utc.setChecked(True); tfl.addWidget(rb_tf_utc); tfl.addWidget(rb_tf_gps); tfl.addStretch(1)
        pl.addWidget(tf_group)
        
        # Variable Group
        var_group = QtWidgets.QGroupBox("Variable")
        varl = QtWidgets.QVBoxLayout(var_group); varl.setContentsMargins(4,6,4,4); varl.setSpacing(2)
        v_start = QtWidgets.QCheckBox("Start time"); v_start.setChecked(True)
        v_avgs = QtWidgets.QCheckBox("Number of averages"); v_avgs.setChecked(True)
        v_third = QtWidgets.QCheckBox("Third parameter"); v_third.setChecked(True)
        v_stats = QtWidgets.QCheckBox("Statistics"); v_stats.setChecked(True)
        v_hist = QtWidgets.QCheckBox("Histogram Under/Overflow"); v_hist.setChecked(True)
        for w in [v_start, v_avgs, v_third, v_stats, v_hist]: varl.addWidget(w)
        ...
        self.param_text = pg.TextItem("", color='k', anchor=(0,1), border='k', fill=(255, 255, 255, 220))
        ...
        self.meta_info = {'start_time': 0, 'avgs': 1, 'bw': 0} # To be updated by engine/main
        ...
        def update_params_display():
            ...
            if v_start.isChecked():
                ...
                if rb_tf_utc.isChecked():
                    import datetime
                    ts_str = datetime.datetime.fromtimestamp(t0, datetime.timezone.utc).strftime('%Y-%m-%d %H:%M:%S UTC')
            ...
            if v_third.isChecked():
                ...
                if any(x in g_type for x in ["Series", "Density", "Coherence", "Function"]):
                    ...
                    if val: header_lines.append(f"<b>BW:</b> {val:.3g} Hz")
            ...
            if v_stats.isChecked():
                ...
                for i, ctrl in enumerate(self.trace_controls):
                    ...
                    if x is not None and len(x) > 0:
                        ...
                        pk_pk = np.ptp(y)
                        rows.append(f"<tr><td>{ch_name}</td><td align='right'>{mean:.2e}</td><td align='right'>{rms:.2e}</td><td align='right'>{pk_pk:.2e}</td></tr>")
        ...
        self.cfg_plot_settings = QtWidgets.QCheckBox("Plot settings"); self.cfg_plot_settings.setChecked(True)
        # Indented checkbox
        self.cfg_respect_user = QtWidgets.QCheckBox("Respect user selection"); self.cfg_respect_user.setChecked(True)
        self.cfg_respect_user.setContentsMargins(20, 0, 0, 0)
        
        self.cfg_axes_title = QtWidgets.QCheckBox("Axes title"); self.cfg_axes_title.setChecked(True)
        self.cfg_bin = QtWidgets.QCheckBox("Bin"); self.cfg_bin.setChecked(True)
        self.cfg_time_adjust = QtWidgets.QCheckBox("Time Adjust"); self.cfg_time_adjust.setChecked(True)

        for w in [self.cfg_plot_settings, self.cfg_respect_user, self.cfg_axes_title, self.cfg_bin, self.cfg_time_adjust]:
            acl.addWidget(w)
        ...
        btn_layout.addWidget(btn_store); btn_layout.addWidget(btn_restore)
        ...
        def update_axis_labels():
            ...
            if txt == "Time Series":
                ...
                rb_x_lin.setChecked(True); rb_y_log.setChecked(True)
                self.display_y_combo.setCurrentText("Magnitude")
            elif "Coherence" in txt:
                self.xaxis_ctrls['title'].setText("Frequency")
                self.yaxis_ctrls['title'].setText("|Coherence|" if "Squared" not in txt else "Coherence^2")
                rb_x_log.setChecked(True); rb_y_lin.setChecked(True)
