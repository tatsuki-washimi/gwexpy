...
class StabilityChecker:
    ...
    def _check_uncached_setlink(self, lines: List[str], filepath: Path) -> None:
        ...
        for i, line in enumerate(lines):
            ...
            if re.search(r'getattr\(self,\s*["\']_.*linked|_linked\s*[!=]', line, re.IGNORECASE):
                has_cache_check = True
                
    def _check_autorange_in_update(self, lines: List[str], filepath: Path, content: str) -> None:
        """Check for enableAutoRange inside update_* functions."""
        # Find update functions
        update_func_pattern = r'def\s+(update_\w+|on_\w+_changed)\s*\('
    ...
    def _check_frequent_setdata(self, lines: List[str], filepath: Path, content: str) -> None:
            ...
            if match and match.group(1) in timer_callbacks:
                ...
                for j in range(i, min(i + 100, len(lines))):
                    if re.search(r'\.setData\(|\.setImage\(', lines[j]):
                        ...
                        throttle_found = any(
                            re.search(r'time\.time\(\)|_last_update|throttle|debounce', lines[k])
                            for k in range(i, j)
                        )
    ...
    def run(self) -> int:
        ...
        if not self.issues and not self.warnings:
            ...
            print(f"{RED}Found {len(self.issues)} critical issues and {len(self.warnings)} warnings.{RESET}")
            return 1
        else:
            print(f"{YELLOW}Found {len(self.warnings)} warnings (no critical issues).{RESET}")
            return 0
