...
def _rolling_1d(arr, window: int, op: str, center: bool, min_count: int, nan_policy: str, ddof: int):
    n = arr.shape[0]
    ...
    for i in range(n):
        ...
        if nan_policy == "propagate" and np.isnan(window_vals).any():
            continue
        valid = window_vals if nan_policy == "propagate" else window_vals[~np.isnan(window_vals)]
...
def _rolling_numpy(data, window: int, op: str, center: bool, min_count: int, nan_policy: str, ddof: int):
    ...
    for idx in range(reshaped.shape[0]):
        out[idx] = _rolling_1d(reshaped[idx], window, op, center, min_count, nan_policy, ddof)
    return out.reshape(flat.shape)


def _rolling_backend(data, window: int, op: str, center: bool, min_count: int, nan_policy: str, ddof: int, backend: str):
    if backend not in ("auto", "bottleneck", "numpy"):
        raise ValueError("backend must be 'auto', 'bottleneck', or 'numpy'")

    use_bn = backend in ("auto", "bottleneck") and _BN_AVAILABLE and nan_policy == "omit"
    if use_bn:
        bn_name = f"move_{op}"
...
def _apply_matrix_op(matrix: TimeSeriesMatrix, window, op: str, center: bool, min_count: int, nan_policy: str, ddof: int, backend: str):
    win = _resolve_window(window, matrix.dt)
    vals = _rolling_backend(matrix.value, win, op, center, min_count, nan_policy, ddof, backend)
    new_mat = matrix.copy()
    new_mat.value[:] = vals
    return new_mat


def _apply_timeseries_op(ts: TimeSeries, window, op: str, center: bool, min_count: int, nan_policy: str, ddof: int, backend: str):
    win = _resolve_window(window, ts.dt)
    vals = _rolling_backend(ts.value, win, op, center, min_count, nan_policy, ddof, backend)
    return ts.__class__(vals, t0=ts.t0, dt=ts.dt, unit=ts.unit, name=ts.name, channel=getattr(ts, "channel", None))
