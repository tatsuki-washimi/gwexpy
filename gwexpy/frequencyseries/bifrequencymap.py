...
class BifrequencyMap(Array2D):
    ...
    def plot(self, method="imshow", **kwargs):
        ...
        **kwargs
        ...
        if method == "imshow":
            ...
            layer = ax.pcolormesh(self.frequency1.value, self.frequency2.value, val_to_plot, **kwargs)
        else:
    ...
    def diagonal(self, method="mean", bins=None, absolute=False, **kwargs):
        ...
        **kwargs
        ...
        if bins is None:
            ...
            df1 = np.mean(np.diff(self.frequency1.value)) if len(self.frequency1) > 1 else 1.0
            df2 = np.mean(np.diff(self.frequency2.value)) if len(self.frequency2) > 1 else 1.0
        ...
        if method == "mean":
            ...
            statistic_func = lambda x: np.nanpercentile(x, p)
        else:
    ...
    def convolute(self, input_spectrum, interpolate=True, fill_value=0):
        ...
        res = self.propagate(input_spectrum, interpolate=interpolate, fill_value=fill_value)
    ...
    def plot_lines(self, xaxis='f1', color='f2', num_lines=None, ax=None, cmap=None, **kwargs):
        """
        ...
        **kwargs
        ...
        if color == 'f2':
            ...
            raise NotImplementedError(f"Coloring/Slicing by '{color}' is not yet implemented using slice iteration.")
        ...
        lc = LineCollection(segments, cmap=cmap, norm=plt.Normalize(iter_axis_vals.min(), iter_axis_vals.max()), **kwargs)
        ...
        # But we need figure.
        if ax.figure:
            cbar = ax.figure.colorbar(lc, ax=ax, label=cbar_label)
