...
**作成日**: 2026-04-03  
**ステータス**: ~~ドラフト（ブレインストーミング結果）~~ **Superseded (issue #483)**  
**目的**: gwexpy プロジェクト固有の知識・過去資産・既知の課題を `.harness/` に体系的に注入し、AI エージェントとの協働品質を向上させる。

> **Superseded:** 本ドキュメントは「`.harness/` は公開リポジトリの追跡対象である」という
> 前提で書かれているが、issue #483 により `.harness/` はメンテナ専用の外部central storageへの
> symlinkとなり、公開リポジトリの追跡対象ではなくなった。以下の増強計画・現状構成表は当時の
> スナップショットとして経緯記録に留め、現行の`.harness/`構成の参照先としては使用しないこと。
