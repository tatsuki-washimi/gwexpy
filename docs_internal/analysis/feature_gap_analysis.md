...
| **品質ゲート** | mypy は設定済みだが、CI の主要ジョブで実行（強制）されていない。 | 型の退行が検知されず、メンテナンスコストが増大する。 | CI に mypy ジョブ（最小構成）を追加。 |
| **マルチOS保証** | CI が Ubuntu に依存しており、Win/Mac での動作保証が未明示。 | ユーザー層が限定され、環境依存バグの発見が遅れる。 | CI に Windows/macOS の smoke test を追加。 |
| **コミュニティ** | `CODE_OF_CONDUCT.md` や Issue/PR テンプレートが未整備。 | 外部協力の“入口摩擦”が増え、貢献の品質が安定しない。 | 行動規範とテンプレートの導入。 |
...
1. **行動規範 (CoC) とテンプレート**: `CODE_OF_CONDUCT.md` と Issue/PR テンプレートの導入。
2. **マルチOS Smoke Test**: Windows/macOS での最小限 host 動作確認ジョブを CI に追加。
3. **CI での型チェック強制**: `mypy` ジョブを CI に統合し、型の退行を防止。
4. **pre-commit 導入**: `ruff`, `mypy` などをコミット前にローカルで実行可能にする。
