...
| スキル名 | 説明 |
| :--- | :--- |
| `setup_plan` | ユーザーの要求から具体的な作業計画を作成し、モデル・スキル・工数を提案する。複数エージェント計画では global gate を参照する |
...
| `gh_pr_comment` | `gh pr comment` を使い、PRレビュー結果や検証結果をGitHub PRへ投稿する |
...
| `audit_api_elements` | gwexpy 内のクラス・関数を抽出し、公開 API / 内部実装として分類する |
